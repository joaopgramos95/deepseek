...
\begin{abstract}
This digest records the current state of Plan~2 / Route $\delta$ toward
...
\CLOSED{} or \OPEN{} tags.  The May~14 re-audit found that the route is
\emph{not} closed as previously stated.  The endpoint-window issue can
be repaired by the good-endpoint variant, and the bad-$I$ weighted
action can be paid for by a crude perimeter-square estimate.  The
remaining load-bearing obstruction is the bad-$(-t')$ Lebesgue kinetic
bound: small Lebesgue measure of the bad set is not enough without a
valid substitute for the old perimeter-free metric Lipschitz claim.
\end{abstract}

\paragraph{Update (2026-05-14 re-audit).}
The table below should be read as the current status.  The earlier
``closed'' marking of Steps~5--6 used two invalid shortcuts: (i) a
small-window Markov argument that produced only an $O(\sqrt{\delta_T})$
metric trace, and (ii) the old dilation-based uniform metric Lipschitz
bound (A0).  The first has a clean conditional repair; the second is
...
6$'$. Good Borel endpoint $\rhad$ near $\rdel$ & \texttt{GoodEndpointTrace} (new) & conditional $^{\dagger}$ \\
...
$\dagger$~The order-one trace plus good-endpoint selection fixes the
endpoint placement, and the bad-$I$ weighted action is removable by the
perimeter-square estimate in Section~\ref{sec:bad-tau}.  What remains
open is the bad-$(-t')$ Lebesgue kinetic estimate needed to transport
metric closeness across the low-density set.
...
\end{align*}
\textbf{Status.}  \OPEN{} in the current no-graph finite-perimeter
route.  On $G_{I}=\{D_{I}\le\eta_{I}\}$, the chain
[Step~2]+[Step~3]+[Step~4] gives the pointwise
$|\dot F_{\rho}|^{2}\le C(D_{H}+D_{I})$.  The weighted bad-$I$ part can
also be paid for without A0; see Section~\ref{sec:bad-tau}.  The
unresolved part is the Lebesgue-form kinetic bound on
$B_{\tau}=\{-t'(\rho)<\rho_{*}/(2n)\}$.  The old proof used a
perimeter-free metric Lipschitz estimate under dilations; bounded
support and nestedness do not imply such a bound for rough measurable
sets.
...
\textbf{Status.}  Conditional on the missing bad-$\tau$ kinetic
estimate.  The correct endpoint mechanism is not Markov on an
$O(\sqrt{\delta_T})$ window (that gives only $O(\sqrt{\delta_T})$ for
$d_{\X}^{2}$).  Instead, run the trace argument on an order-one window
to obtain a uniform bound, then choose a Borel good level
$\rhad\in[\rdel-C_{1}\delta_{T},\rdel]\cap G_{I}\cap G_{\tau}$ using
the bad-set Lebesgue bounds
$\mathcal L^{1}(B_{\tau})\le C\delta_{T}$ and
$\mathcal L^{1}(B_{I}\cap G_{\tau})\le C\delta_{T}$.  This fixes the
boundary-layer interface but does not remove the bad-$\tau$ kinetic
input needed for the uniform trace.
...
\section{The current obstruction: bad-$(-t')$ kinetic control}
\label{sec:bad-tau}

The useful simplification from the re-audit is that the
\emph{bad-isoperimetric} weighted action is not the essential problem.
Indeed, on $B_I=\{\mathcal I(\rho)>\eta_I\}$ choose the harmless centre
$z=0$ and $a=0$ in the metric first-variation estimate.  Since
$\Om\subset B_R$,
\[
   |\dot F_\rho|_{\X}
   \le \rho_*^{-n}\int_{\partial^*E_\rho}|V_\rho-H_{0,\rho}|\,d\Hn
   \le C(n,R,\rho_*)\bigl(P(B_\rho)+P(E_\rho)\bigr),
\]
using $\int_{\partial^*E_\rho}V_\rho\,d\Hn=P(B_\rho)$ and
$|H_{0,\rho}|\le R/\rho_*$.  Therefore
\[
   \int_{B_I}|\dot F_\rho|_{\X}^{2}\,d\mu
   \le C\int_{B_I}\bigl(1+P(E_\rho)^2\bigr)\,d\mu
   \le C\int_{B_I}\bigl(1+\mathcal I(\rho)\bigr)\,d\mu
   \le C\delta_T,
\]
because $P(E_\rho)^2=P(B_\rho)^2+\mathcal I(\rho)$ and
$\mu(B_I)\le\eta_I^{-1}\int\mathcal I\,d\mu\le C\delta_T$.

The remaining estimate is different:
\[
   \int_{B_{\tau}}|\dot F_\rho|_{\X}^{2}\,d\rho\le C\delta_T,
   \qquad
   B_\tau:=\{-t'(\rho)<\rho_*/(2n)\}.
\]
Talenti gives only $|B_\tau|\le C\delta_T$.  To exploit this measure
bound one would need either a valid pointwise metric-Lipschitz bound
for $\rho\mapsto F_\rho$ or another estimate controlling the metric
variation on $B_\tau$.  The old dilation argument for such a bound is
not valid: for a rough bounded measurable set $A$, bounded support and
volume do not imply
$|\lambda A\Delta A|\le C|\lambda-1|$.  A perimeter or BV-type bound is
needed, and precisely this can fail on the low-density bad set.

Thus the bad set \emph{does} matter.  The endpoint can be placed
correctly by \texttt{GoodEndpointTrace}, but the no-graph Route
$\delta$ still needs a genuine substitute for the discredited A0
estimate on $B_\tau$.

\section{The historical homothetic-trace obstruction}\label{sec:gap}

\textbf{Historical open question.}  In the pre-closure version of the
chain, everything reduced to:
...
This is a weighted-$\mu$ statement.  It does not by itself discharge
the bad-$\tau$ Lebesgue kinetic estimate needed for endpoint
transport.
...
\section{Consequences of closing the historical gap}\label{sec:if-closed}

In the older architecture, closing \eqref{eq:hom-target} together with
the bad-level kinetic bookkeeping would yield:
...
The homothetic-trace bottleneck is no longer the cleanest formulation
of the gap.  After the good-endpoint repair and the bad-$I$ estimate of
Section~\ref{sec:bad-tau}, the remaining missing input is the
bad-$\tau$ Lebesgue kinetic bound.  Until that is proved, the bounded
Saint--Venant estimate and the Plan~2 proof of sharp Faber--Krahn do
not follow from this route alone.

\section{Historical suggestions for next attempts}\label{sec:suggestions}
...
\fbox{\eqref{eq:hom-target}: integrated homothetic-trace}
   & \longrightarrow & \int(\int|H_{z}-\bar V|)^{2}d\mu\le C\delta_{T} & & conditional \\
\text{Step 5: \texttt{WeightedMetricTrace}}
   & \Longleftarrow & \text{Steps 1, 2, 4$'$, \eqref{eq:hom-target}, and bad-$\tau$ kinetic} & & \OPEN \\
\text{Step 6: trace at endpoint}
   & \Longleftarrow & \text{Step 5 plus good-endpoint selection} & & \OPEN \\
...
\medskip
Historically, the single \OPEN{} input \eqref{eq:hom-target}
propagated downward to Steps~5 and~6.  The re-audit separates that
old bottleneck from the present one: the endpoint selection can be
made near $\rdel$, but the bad-$\tau$ transport estimate is still
missing.
...
\section{Why the historical gap was difficult}\label{sec:why}
...
was a \emph{quantitative geometric} statement about how rough a
torsion super-level set can be when its perimeter is large.

The reason the fourth was more obdurate is that the first three are
...
had to come from a \emph{global} balance: bad levels are rare in
$\mu$-measure, and a refined Cauchy--Schwarz must exploit this
rareness.  The current route achieves part of this at the weighted
level, but it still lacks the unweighted bad-$\tau$ control needed for
metric transport.

\section{Summary}\label{sec:summary}

The Plan~2 / Route~$\delta$ proof is not closed by the current
finite-perimeter argument.  The re-audit identifies the remaining
obstruction precisely: prove
$\int_{B_\tau}|\dot F_\rho|_{\X}^{2}d\rho\le C\delta_T$, or replace
the Cauchy--Schwarz transport step by an argument that avoids crossing
$B_\tau$.  The six candidate routes recorded above remain useful as
diagnostics: they explain why the bottleneck is subtle, which
approaches lose a square root, and why any successful repair must
square at the slicing level rather than after a naive
Cauchy--Schwarz step.
