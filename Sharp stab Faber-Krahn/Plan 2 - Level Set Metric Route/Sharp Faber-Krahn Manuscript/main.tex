...
\begin{assumption}[Constructive Serrin strong-form input]
...
for the torsion superlevel sets of the bounded core.  For a.e.\ radius
\(\rho\in[\rstar,\rdel]\) with
\[
   D_H(t(\rho))+D_I(t(\rho))\le\eta_{\rm Ser},
\]
there is a centre \(z_\rho\in\R^n\) satisfying
\begin{equation}\label{eq:constructive-strong}
   |E_\rho\Delta B_\rho(z_\rho)|^2
   +
   \int_{\partial^*E_\rho}
      \left|\nu_\rho(x)-\frac{x-z_\rho}{|x-z_\rho|}\right|^2d\Hn(x)
   \le
   C_{\rm Ser}(n,R,\rstar)
      \bigl(D_H(t(\rho))+D_I(t(\rho))\bigr).
\end{equation}
\end{assumption}

\begin{remark}[Status of the constructive input]
\label{rem:constructive-strong-status}
Assumption~\ref{ass:constructive-strong} is the exact replacement for the
Fusco--Julin strong form formerly used here.  It is not a consequence of the
compactness-built Fusco--Julin theorem.  The intended proof is the constructive
Reilly--Weinberger/Serrin route described in
\texttt{CONSTRUCTIVE\_STRONG\_FORM\_STRATEGY.md}: for
\(v=u-t(\rho)\), the torsion function of \(E_\rho\), prove a weighted
Reilly--Weinberger estimate controlling
\(D^2v+I/n\) linearly by the boundary Cauchy--Schwarz defect \(D_H\), then
convert the resulting bulk deviation into the boundary normal term in
\eqref{eq:constructive-strong}.  The sign convention in this manuscript is
\(v>0\) in \(E_\rho\), so the weighted identity must use the positive torsion
weight \(v\), not \(-v\).  Until this assumption is discharged, the constructive
version of the manuscript is conditional on this single input.
\end{remark}
...
\begin{theorem}[Same-centre good-level geometric package]
\label{inp:samecenter}
Assume the constructive strong-form input
Assumption~\ref{ass:constructive-strong}.  There exists
\(C_{\rm sc}=C_{\rm sc}(n,R,\rstar)\) such that the following holds.  Define
\[
   G_{\rm sc}:=\bigl\{\rho\in[\rstar,\rdel]:
      D_H(t(\rho))+D_I(t(\rho))\le\eta_{\rm Ser}\bigr\}.
\]
For a.e.\ \(\rho\in G_{\rm sc}\) there is a centre
\(z_\rho^{\rm sc}\) such that
...
\begin{proof}
Let \(\rho\in G_{\rm sc}\) be a finite-perimeter level where
Assumption~\ref{ass:constructive-strong} applies.  Let
\(z=z_\rho^{\rm sc}\) be the centre supplied by
\eqref{eq:constructive-strong}.  Then
\begin{equation}\label{eq:sc-proof-constructive}
   |E_\rho\Delta B_\rho(z)|^2
   +
   \int_{\partial^*E_\rho}|\nu_\rho-e_z|^2\,d\Hn
   \le C(n,R,\rstar)\bigl(D_H(t(\rho))+D_I(t(\rho))\bigr).
\end{equation}
Reducing \(\eta_{\rm Ser}\) if necessary, the first term in
\eqref{eq:sc-proof-constructive} implies
\[
...
The radial trace lemma and \eqref{eq:sc-proof-constructive} give
\[
   \int_{\partial^*E_\rho}
      \left|\frac{|x-z|}{\rho}-1\right|\,d\Hn
   \le
   C(n,R,\rstar)
   \bigl(D_H(t(\rho))+D_I(t(\rho))\bigr)^{1/2},
\]
after reducing \(\eta_{\rm Ser}\le1\).  For
\(H_{z,\rho}=(|x-z|/\rho)(e_z\cdot\nu_\rho)\),
\[
   |H_{z,\rho}-1|
   \le
   ...
   +\frac12|\nu_\rho-e_z|^2 .
\]
Together with \eqref{eq:sc-proof-constructive}, this gives
\begin{equation}\label{eq:Hminus1-DH-DI}
   \int_{\partial^*E_\rho}|H_{z,\rho}-1|\,d\Hn
   \le
   C(n,R,\rstar)\bigl(D_H(t(\rho))+D_I(t(\rho))\bigr)^{1/2}.
\end{equation}
Finally,
\[
...
\]
Combining this with \eqref{eq:Hminus1-DH-DI} proves the homothetic-trace
part of \eqref{eq:sc-input}; the volume part is the first term of
\eqref{eq:sc-proof-constructive}.  Squaring completes the proof.
\end{proof}
...
\begin{lemma}[Transfer to a distance-minimising centre]
\label{lem:center-transfer}
After reducing \(\eta_{\rm Ser}=\eta_{\rm Ser}(n,R,\rstar)\) if necessary,
the following holds.  Let \(\rho\in G_{\rm sc}\) be a level where
...
\[
   ...
   \le C \bigl(D_H(t(\rho))+D_I(t(\rho))\bigr)^{1/2}.
\]
Thus
\[
...
Choose \(\eta_{\rm Ser}\) so small that the preceding upper bound forces the
linear branch \(d<\rho\).  Since \(\rho\ge\rstar\),
\[
   |z_\rho^{\min}-z_\rho^{\rm sc}|
   \le C(n,\rstar)\bigl(D_H(t(\rho))+D_I(t(\rho))\bigr)^{1/2}.
\]
...
\]
On \(G_{\rm sc}\), the perimeter is uniformly bounded because
\(D_I(t(\rho))\le D_H(t(\rho))+D_I(t(\rho))\le\eta_{\rm Ser}\) and
\(\rho\in[\rstar,1]\).  Hence
\[
   \int_{\partial^*E_\rho}
   |H_{z_\rho^{\min},\rho}-H_{z_\rho^{\rm sc},\rho}|\,d\Hn
   \le C(n,R,\rstar)\bigl(D_H(t(\rho))+D_I(t(\rho))\bigr)^{1/2}.
\]
...
\[
   ...
   \le C(n,R,\rstar)\bigl(D_H(t(\rho))+D_I(t(\rho))\bigr).
\]
Indeed \(q^2\le C(D_H+D_I)\) on \(G_{\rm sc}\), and
\[
\begin{aligned}
...
\]
On \(G\), Theorem~\ref{inp:samecenter} gives
\[
...
\input{parts/sec_bounded_reduction_body.tex}

\input{parts/sec_kohler_jobin_body.tex}
...
\(C_{\rm sc}(n,R,\rstar)\) and threshold \(\eta_{\rm Ser}(n,R,\rstar)\)
