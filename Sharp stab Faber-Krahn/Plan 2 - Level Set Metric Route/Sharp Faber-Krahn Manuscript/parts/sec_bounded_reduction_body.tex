...
Fix a finite-perimeter level $\rho$ and apply Fusco--Julin
(Theorem~\ref{thm:FJ-strong}) to $E_\rho$ with $r_E=\rho$; discarding the
boundary-oscillation summand and the outer square,
$\abs{E_\rho\Delta B_\rho(z)}^2\le C_{\rm FJ}(n)\,\rho^{\,n+1}
(\Per(E_\rho)-\Per(B_\rho))$ for the optimal centre $z$. Since
...
The only external ingredient in this proof is the Fusco--Julin quantitative
isoperimetric inequality (Theorem~\ref{thm:FJ-strong}), already used in the
manuscript; everything else -- the deficit identity, the profile-gap
identity, and the superlevel transfer -- is proved here. Thus the bounded
reduction depends on no stability estimate beyond the ones the paper
establishes. The exponent $4$ (indeed the $3$ obtained in the proof) is far
