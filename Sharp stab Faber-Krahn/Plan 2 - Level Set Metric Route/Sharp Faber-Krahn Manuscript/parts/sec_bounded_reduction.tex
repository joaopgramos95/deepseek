%=====================================================================
%  PREAMBLE  (strip everything between here and \begin{document}
%             when \input-ing this file into the master document)
%=====================================================================
...
\begin{document}
%=====================================================================
%  END PREAMBLE  --  the section proper begins here
%=====================================================================

\section{Bounded reduction}\label{sec:bdd-red}

The purpose of this section is to remove the boundedness hypothesis from
the sharp Saint--Venant stability inequality. We assume given the
following \emph{bounded} estimate, whose proof occupies the earlier part
of the manuscript and which we treat here as a black box.
...
There is a dimensional constant $C_\flat=C_\flat(n)>0$ such that for
every open $\Om$ with $\abs{\Om}=\omega_n$ and every $R\ge1$,
...
\end{equation}
with an absolute constant $C_1$ (one checks $C_1=4$ works after expanding
...
$w\in H^1_0(\R^2)$ (e.g.\ via the $n=2$ Ladyzhenskaya/Sobolev embedding,
valid on bounded support). Repeating the Hölder step with exponent $p$
replaces the exponent $2/n=1$ by $1-2/p\in(0,1)$ in
\eqref{eq:sobolev}--\eqref{eq:mu-recursion}; the recursion
\eqref{eq:mu-recursion} becomes
$\mu_{k+1}\le C\,16^k\,\mu_k^{1+(1-2/p)}$ and Lemma~\ref{lem:DG-iteration2}
applies verbatim with $\beta=1-2/p>0$. The final exponent on
$\abs{\Om\setminus B_R}$ in \eqref{eq:Linfty} stays $1/n=1/2$ because it
is fixed by the level scale $s_\infty=M\abs{\Om\setminus B_R}^{1/n}$,
which we are free to choose; only the dimensional constant $C_\flat$
changes. Thus \eqref{eq:Linfty} holds for all $n\ge2$.
\end{remark}

\subsection{A suboptimal stability seed}\label{ssec:seed}

The truncation iteration needs one quantitative input that is genuinely
\emph{not} produced by the De~Giorgi machinery: a (possibly far from
sharp) quantitative control of the asymmetry by the deficit, used only to
guarantee that the tail mass is already small at the first radius and to
bound the number of truncation steps. We state it with the exponent~$4$;
this is the one ingredient of the present section that we cite rather than
prove, and we record this honestly.

\begin{lemma}[Suboptimal stability, cited]\label{lem:seed}
...
\begin{remark}[Status of Lemma~\ref{lem:seed}]\label{rem:seed-status}
Inequality \eqref{eq:seed} is the torsional stability estimate of
Fusco--Maggi--Pratelli \cite{FMP}, obtained from the
P\'olya--Szeg\H{o} inequality and the quantitative isoperimetric
inequality; via $E(\Om)=-1/(2\lambda_{2,1}(\Om))$ it is exactly
\cite[Lemma~5.2]{BDV}. Its proof is rearrangement-based (no compactness,
no contradiction), but it lies outside the De~Giorgi--truncation circle
that is the subject of this section, so we import it. Only a positive
power of $D$ is used below; the exponent $4$ is not essential, any fixed
exponent would do.
\end{remark}
...
\emph{Tail at the stopping radius.} By the maximality \eqref{eq:K-def},
$b_{K+1}<2\widehat C D(\Om)$; we record this for the next step.
...
\end{align*}
For the first term, $r^{-(n+2)}-1\ge0$ and $E(B_1)<0$ would make it
negative—good for an upper bound—but we must be careful, since a negative
contribution only helps. Quantitatively, by \eqref{eq:r-near-1},
...
in particular Lemmas~5.1, 5.2 and 5.3.
