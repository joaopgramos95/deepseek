...
There is one separate attempt not closed here:
...
\section{Attempt IV: transport and restricted geometric classes}
\label{sec:transport}

This final attempt is shorter because, at present, it produces a programme
rather than a new estimate.  The sharp quantitative Fraenkel inequality can
be proved constructively by transport or symmetrisation methods
\cite{FMP}.  It supplies a ball centre \(z\) with
\[
   |E\mathbin{\triangle}B_\rho(z)|^2
   \le C_{\rm iso}(n)\rho^{n+1}\Def(E).
\tag{6.1}\label{eq:FMP-volume}
\]
Estimate \eqref{eq:FMP-volume} controls a volume displacement.  Fusco--Julin
requires, in addition, the boundary direction term
\[
   \int_{\partial^*E}|\nu_E-e_z|^2\,d\Hn.
\tag{6.2}\label{eq:normal-needed}
\]
For a general finite-perimeter target set, no boundary regularity of a
transport map is available from \eqref{eq:FMP-volume}.  Therefore an argument
which simply differentiates a transport map on \(\partial B_\rho\) is not a
proof of \eqref{eq:normal-needed}.

There are, however, restricted settings in which this approach may become
effective.  If an independent hypothesis gives a small radial
\(W^{1,\infty}\) representation with the transport/Fraenkel centre, then
Corollary~\ref{cor:scaled-graph} already supplies the required normal term.
Convexity or a uniform interior-and-exterior ball condition are natural
sources of such a representation, but the quantitative implication must be
proved before it is used.

\begin{problem}[Geometric bridge]\label{prob:bridge}
Prove a computable statement of the following form.  Given a specified
geometric class \(\mathcal C\), find \(\eta_{\mathcal C}(n)>0\) such that if
\(E\in\mathcal C\), \(|E|=|B_\rho|\), and
\(\Def(E)\le\eta_{\mathcal C}(n)\rho^{n-1}\), then there is a centre \(z\)
for which
\[
   \partial E=\{z+\rho(1+u(\theta))\theta:\theta\in S\},
   \qquad
   \norm{u}_{W^{1,\infty}(S)}\le\frac1{100n^3}.
\]
Once this is proved, Corollary~\ref{cor:scaled-graph} makes the strong form
explicit on \(\mathcal C\).
\end{problem}
