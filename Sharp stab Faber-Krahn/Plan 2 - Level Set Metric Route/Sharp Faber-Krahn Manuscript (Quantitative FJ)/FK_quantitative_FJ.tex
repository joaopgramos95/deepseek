...
% Wrapper for flagging passages identified, on two independent passes,
% as not load-bearing for the proof of the main theorem.  These
% passages are kept for context (motivation, comparisons, scope
% remarks, dependency certificates) but are typeset in red so that a
% reader can excise them without affecting the logical chain.
\newcommand{\unused}[1]{{\color{red}#1}}
...
(Theorem~\ref{thm:FJ-strong}), used to extract a same-centre control of
volume asymmetry and boundary-normal oscillation, is itself reproved
internally in \S\ref{sec:FJ-quantitative} by a regularising selection
that uses only the \(\eps\)-regularity for almost-minimisers of
perimeter, so the manuscript is self-contained modulo standard tools for
sets of finite perimeter and the classical rearrangement results.
\end{abstract}
...
\subsection{History}\label{sec:intro-history}

For an open set \(\Om\subset\R^n\) (\(n\ge2\)) with \(0<|\Om|<\infty\), let
\(\lambda_1(\Om)\) denote its first Dirichlet eigenvalue and \(\Bstar\) the
ball of the same volume.  The \emph{Faber--Krahn inequality}
\begin{equation}\label{eq:FK-classical}
   \lambda_1(\Om)\ge\lambda_1(\Bstar),\qquad
   \text{equality if and only if \(\Om\) is a ball},
\end{equation}
goes back to a 1877 conjecture of Lord Rayleigh \cite{Rayleigh1877} and was
proved independently by Faber \cite{Faber1923} and Krahn \cite{Krahn1925} in
the 1920s, by Schwarz symmetrisation of the Rayleigh quotient
\eqref{eq:rayleigh}.  It is the spectral analogue of the classical
isoperimetric inequality and is the prototype for an entire family of
spectral shape inequalities; for the modern theory we refer to the
monograph of Henrot \cite{Henrot2006}, the survey of Brasco--De~Philippis
\cite{BraDeP2017}, and the references therein.

\medskip
The qualitative statement \eqref{eq:FK-classical} is silent on the rate at
which an almost-extremal domain must resemble a ball.  Quantitative
versions were first obtained by Hansen--Nadirashvili \cite{HansenNadirashvili1994},
Melas \cite{Melas1992}, Bhattacharya--Weitsman \cite{BhattacharyaWeitsman1999},
and Fusco--Maggi--Pratelli \cite{FuscoMaggiPratelli2009Eigenvalue}, each
producing an inequality of the form
\[
   \deltaFK(\Om)
   :=\Bigl(\tfrac{|\Om|}{\omega_n}\Bigr)^{2/n}
     \bigl(\lambda_1(\Om)-\lambda_1(\Bstar)\bigr)
   \ge c(n)\,\Fr(\Om)^p
\]
for some exponent \(p>2\) (\(p=4\) in \cite{FuscoMaggiPratelli2009Eigenvalue}),
where
\(\Fr(\Om):=\inf_{x_0\in\R^n}|\Om\Delta(x_0+\Bstar)|/|\Bstar|\) is the
\emph{Fraenkel asymmetry}.  Smooth ellipsoidal perturbations of the ball
show that no exponent \(p<2\) is possible
\cite{Brasco2014,BraDeP2017}, so the sharp question asks for \(p=2\).
The sharp form was proved by Brasco--De~Philippis--Velichkov \cite{BDV} in
the form
\begin{equation}\label{eq:BDV}
   \deltaFK(\Om)\ge c_{\mathrm{FK}}(n)\,\Fr(\Om)^2,
\end{equation}
using a Cicalese--Leonardi-style selection principle
\cite{CicaleseLeonardi2012} adapted to a penalised Bernoulli functional,
with the Alt--Caffarelli free-boundary regularity theory
\cite{AltCaffarelli1981} providing the smoothing step.  Their argument
proceeds by a sequential compactness/contradiction scheme, and the
resulting constant \(c_{\mathrm{FK}}(n)\) is not explicit.  This was
pointed out by Brasco and De~Philippis in their survey
\cite[Open~Problem~2, p.~22]{BraDeP2017}, where the question of giving
a sharp quantitative Faber--Krahn inequality with a \emph{computable}
dimensional constant -- equivalently, with no compactness selection --
is formally posed and listed as open; the same issue is reiterated in
Allen--Kriventsov--Neumayer \cite[\S1]{Allen2021}, who also gave a
related linear-implies-nonlinear stability result via the
Alt--Caffarelli--Friedman monotonicity formula, with a constant that is
likewise not explicit.  Prior to the present work, an explicit
constant had only been obtained in the \emph{non-sharp} regime, with
exponent $4$ on the asymmetry, in dimension two by Melas
\cite{Melas1992} and in general dimension by Brasco--De~Philippis
\cite[Thm.~2.10]{BraDeP2017}.  The present manuscript answers Open
Problem~2 of \cite{BraDeP2017} in the affirmative.  For related
Stekloff- and spectrum-type stability results see
\cite{Nitsch2014,DePhilippisVelichkov2020}.

\medskip
The development on the perimeter side runs in parallel.  The qualitative
isoperimetric inequality, with its full equality cases, was settled by
De~Giorgi \cite{DeGiorgi1958} in the class of sets of finite perimeter.
Quantitative stability was first established by Fuglede \cite{Fuglede1989}
for nearly-spherical sets, then in full generality by Fusco--Maggi--Pratelli
\cite{FMP}, and refined via mass transport by Figalli--Maggi--Pratelli
\cite{FMP2010} -- both with the \emph{volume} asymmetry \(\Fr\) on the
right-hand side.  Fusco and Julin \cite{FJ2014} subsequently obtained the
stronger \emph{normal-oscillation} form: there is a centre \(z_E\) about
which \emph{both} the volume asymmetry and the squared deviation of the
unit normal from the radial direction are controlled by a single perimeter
deficit.  Their proof is a regularising selection in the class of additive
perimeter almost-minimisers, closed by the \(\eps\)-regularity for
quasi-minimal hypersurfaces of Almgren \cite{Almgren1976} and Tamanini
\cite{Tamanini1984}, followed by a nearly-spherical second variation in
the spirit of Fuglede and of Acerbi--Fusco--Morini
\cite{AcerbiFuscoMorini2013}; see also the survey \cite{Krummel2014} for a
unified account.

The Fusco--Julin same-centre control is, as we shall see in
\S\ref{sec:samecentre}, the input that makes the level-set route to
\eqref{eq:BDV} go through.
...
\begin{equation}\label{eq:FK-main}
   ...
   \;\ge\; c_{\mathrm{FK}}(n)\,\Fr(\Om)^2,
\end{equation}
where \(\Fr(\Om):=\inf_{x_0\in\R^n}|\Om\Delta(x_0+\Bstar)|/|\Bstar|\) is the
Fraenkel asymmetry.  The exponent \(2\) is sharp.
\end{theorem}

This is the theorem of Brasco--De~Philippis--Velichkov \cite{BDV}; the
novelty here is in the proof.  The constant \(c_{\mathrm{FK}}(n)\) is
\emph{computable}, in the sense of \cite[Open~Problem~2]{BraDeP2017}: it
is exhibited as an explicit finite composition of dimensional constants,
with no compactness selection, no sequential contradiction, and no
Bernoulli free-boundary input.  Theorem~\ref{thm:FK-main} thus answers
in the affirmative the open problem of producing a sharp quantitative
Faber--Krahn inequality with a computable constant, as raised in
\cite[p.~22]{BraDeP2017} and reiterated in
\cite[\S1]{Allen2021}.  In particular the Fusco--Julin
strong-form inequality, used as an internal ingredient
(Theorem~\ref{thm:FJ-strong}), is reproved \emph{from scratch} in
\S\ref{sec:FJ-quantitative}, so the entire manuscript is self-contained
modulo the classical theory of sets of finite perimeter
\cite{AFP2000,Maggi2012} and the Talenti--P\'olya--Szeg\H{o} rearrangement
results.

\subsection{Strategy of proof}\label{sec:intro-strategy}

\subsubsection*{Reaching the eigenvalue through the torsion function}

The eigenvalue \(\lambda_1\) is reached only at the very end.  The
backbone is the torsion function
\(u=u_\Om\in H^1_0(\Om)\) of \(-\Delta u=1\) and its Saint--Venant deficit
\[
   \dT(\Om):=E(\Om)-E(\Bstar),
   \qquad E(\Om)=-\tfrac12\int_\Om u\,dx .
\]
The classical Saint--Venant inequality \(\dT(\Om)\ge0\) is the torsion
analogue of \eqref{eq:FK-classical} and, by the
Kohler--Jobin~\cite{KJ1,KJ2,KJ3,Brasco2014} rearrangement, comparison of
the Faber--Krahn deficit with the Saint--Venant deficit is essentially
free; we reproduce a fully elementary, scale-tracked version of this
transfer in \S\ref{sec:kj}.  The whole problem is therefore to establish
the \emph{sharp Saint--Venant stability inequality}
\begin{equation}\label{eq:sharpSV}
   \dT(\Om)\;\ge\; c_{\mathrm{SV}}(n)\,\Fr(\Om)^2,
\end{equation}
which is the heart of the paper.

\subsubsection*{Why we do not follow the route via free-boundary regularity}

One \emph{theoretically plausible} quantification of the route of
Brasco--De~Philippis--Velichkov to \eqref{eq:sharpSV} would replace their
selection principle by a single-set, penalised selection map
\(\Om\mapsto\widetilde\Om\) preserving the deficit-to-asymmetry ratio
without sequential compactness, then close as in \cite{BDV}.  The hard
inputs of such a route would still be those of \cite{BDV}: a
Bernoulli-type penalised functional whose minimisers have an
Alt--Caffarelli flatness improvement, a bootstrap from \(C^{1,\gamma}\)
to a \emph{small} \(C^{2,\gamma_0}\) graph on which the nearly-spherical
second variation can be invoked, and the second variation itself.
Quantifying every constant along that chain demands the full
Alt--Caffarelli theory~\cite{AltCaffarelli1981} -- existence and regularity
for one-phase Bernoulli free boundaries -- both for the
penalised-minimiser construction (density and non-degeneracy lemmas of
\cite{BDV}) and for the flatness improvement underlying the
\(C^{1,\gamma}\) entry.  The hidden cost is then the
\(C^{1,\gamma}\to\) small \(C^{2,\gamma_0}\) bootstrap, where the
deficit only controls a half-derivative of the boundary; closing the
gap requires uniform Schauder estimates for the hodograph problem and a
careful interpolation against a high-order fixed bound.

We chose not to pursue that route, although it would be theoretically
viable, because its quantification rests on machinery -- the
Alt--Caffarelli existence and regularity theory and a quantitative
Bernoulli bootstrap -- whose effective form is precisely the part one
would need to redo by hand.  Our route is, by contrast, \emph{more
instructive and relies on a more reasonable input}: it builds directly
on the geometry of the torsion superlevels and on the only sharp
quantitative result for which a fully constructive proof is available
without recourse to Bernoulli free boundaries, namely the Fusco--Julin
strong form of the quantitative isoperimetric inequality.  At the level
of regularity theory we therefore use only the De~Giorgi--Almgren--Tamanini
\(\eps\)-regularity for almost-minimal sets of finite perimeter
\cite{Almgren1976,Tamanini1984,Maggi2012}, not the Alt--Caffarelli theory
for one-phase free boundaries; this is the intrinsic reason for the
particular architecture of our proof, and the reason we believe it is
worth recording.

\subsubsection*{The level-set viewpoint and the central distance}

We work directly with the superlevel sets of the torsion function.  Write
\(t(\rho)\) for the unique level such that
\(E_\rho:=\{u>t(\rho)\}\) has volume \(\omega_n\rho^n\); the family
\(\{E_\rho\}_{0<\rho<1}\) is nested and decreasing in \(\rho\).  The
geometric object that does the work is the \emph{unscaled moving-ball
distance}
\begin{equation}\label{eq:q-intro}
   q(\rho):=\inf_{z\in\R^n}|E_\rho\Delta B_\rho(z)|.
\end{equation}
Because \(\{E_\rho\}\) is nested, \(q\) is uniformly Lipschitz in
\(\rho\) (Lemma~\ref{lem:qLip}), with a constant depending only on the
dimension.  This Lipschitz bound is what eventually lets the bad-density
levels be charged by \emph{Lebesgue measure}, hence by the deficit only
indirectly, rather than by the deficit pointwise.

\subsubsection*{The exact level-set deficit identity}

The starting point of the analysis is an exact identity that decomposes
the Saint--Venant deficit into a non-negative sum of two reduced-boundary
defects.  For a.e.\ level \(t\in(0,\|u\|_\infty)\) set
\[
   \alpha(t):=\int_{\partial^*\{u>t\}}\frac{d\Hn}{|\nabla u|},
   \qquad
   \beta(t):=\int_{\partial^*\{u>t\}}|\nabla u|\,d\Hn,
\]
and define the Cauchy--Schwarz defect
\(D_H(t):=\alpha(t)\beta(t)-\Per(E_t)^2\) and the squared-perimeter
isoperimetric defect \(D_I(t):=\Per(E_t)^2-c_n m(t)^{2-2/n}\), with
\(c_n=n^2\omega_n^{2/n}\) and \(m(t)=|E_t|\).  Cauchy--Schwarz on
\(\partial^*E_t\) applied to \((|\nabla u|^{1/2},|\nabla u|^{-1/2})\) and
the De~Giorgi isoperimetric inequality \cite{DeGiorgi1958} give
\(D_H,D_I\ge0\).  Then
\begin{equation}\label{eq:identity-intro}
   2\,\dT(\Om)
   =\int_0^{\|u\|_\infty}
      \frac{D_H(t)+D_I(t)}{c_n\,m(t)^{1-2/n}}\,dt
\end{equation}
(Theorem~\ref{thm:id-main}).  Identity \eqref{eq:identity-intro} is an
exact equality, valid on the De~Giorgi reduced boundary of any open
\(\Om\), with no smoothness, no graph parametrisation, and no pointwise
lower bound on \(|\nabla u|\).  In particular, levels at which
\(|\nabla u|\) is far from constant on \(\partial^*E_t\) pay through
\(D_H\), and levels at which \(E_t\) is far from a ball pay through
\(D_I\); a small deficit therefore forces both defects to be small on
most levels.

\subsubsection*{The Fusco--Julin input and same-centre control}

The Fusco--Julin strong form (Theorem~\ref{thm:FJ-strong}) provides,
for each finite-perimeter set \(E\) with \(|E|=|B_\rho|\), a
\emph{single} centre \(z_E\) controlling simultaneously the volume
asymmetry and the boundary-normal oscillation:
\[
   \frac{|E\Delta B_\rho(z_E)|^2}{\rho^{2n}}
   +
   \frac1{\rho^{n-1}}\int_{\partial^*E}|\nu_E-e_{z_E}|^2\,d\Hn
   \le
   C_{\rm FJ}(n)\,\frac{\Per(E)-\Per(B_\rho)}{\rho^{n-1}}.
\]
This same-centre feature is essential, and is not given by the weaker
Fusco--Maggi--Pratelli theorem alone (which controls only the volume
asymmetry, possibly about a different centre).  Combined with an oriented
radial trace identity (Lemma~\ref{lem:oriented-radial-trace}), it
converts a small \(D_I\) on a level into homothetic-trace control of
\(E_\rho\) about the corresponding optimal centre
(Theorem~\ref{inp:samecenter}); a Cauchy--Schwarz computation on the
coarea derivative converts a small \(D_H\) into a small velocity defect
(Lemma~\ref{lem:velocity}).

We have made it a point that the Fusco--Julin theorem itself is proved
internally, in \S\ref{sec:FJ-quantitative}, in a sharp truly quantitative
form with a finite, explicit dimensional constant.  The argument splits
in two stages: \S\ref{app-sec:graphs} establishes an elementary
Fuglede-type nearly-spherical inequality with a displayed constant, and
\S\ref{app-sec:selection} runs a regularising selection that reduces an
arbitrary finite-perimeter set to the nearly-spherical regime, using
only the \(\eps\)-regularity for additive almost-minimisers of
perimeter.  Crucially -- this is the point about machinery emphasised
above -- no Alt--Caffarelli theory, no one-phase Bernoulli regularity,
and no contradiction argument is used; the input is the classical
De~Giorgi--Almgren--Tamanini regularity for almost-minimal hypersurfaces.

\subsubsection*{The affine first variation and the positive kinetic estimate}

The next step is to convert the same-centre good-level control into a
quantitative first variation of \(q\).  The key technical tool is an
\emph{affine moving-ball first variation}: at almost every level a
finite-perimeter version of the formula
\(D^+q(\rho)\le -\text{(something positive)}+\text{(error)}\) holds, with
the error controlled by the level-set defects \(D_H+D_I\) and the
\emph{something positive} -- the \emph{positive kinetic estimate} of
Proposition~\ref{prop:positivekinetic} -- bounded below by a multiple of
\(q(\rho)\) itself.  After bookkeeping for the bad-density levels (which
are charged by Lebesgue measure via the Lipschitz bound on \(q\)) and a
moving-ball endpoint trace (Lemma~\ref{lem:endpoint}), this closes the
bounded Saint--Venant stability inequality
\(\Fr(\Om)^2\le C(n,R)\dT(\Om)\) for sets confined to a fixed ball
\(B_R\) (Theorem~\ref{thm:boundedSV}).

\subsubsection*{Bounded reduction and the Kohler--Jobin transfer}

The confinement \(\Om\subset B_R\) is removed by a constructive De~Giorgi
\(L^\infty\)-truncation argument in the spirit of
Brasco--De~Philippis--Velichkov \cite[\S5]{BDV} and Figalli--Maggi--Pratelli
\cite{FMP2010}: at the cost of slightly increasing the deficit and
slightly decreasing the asymmetry, one replaces \(\Om\) by a bounded
surrogate of dimensional diameter, applies
Theorem~\ref{thm:boundedSV}, and transfers the resulting estimate back.
The complementary large-deficit regime is trivial because \(\Fr<2\).
This yields the global sharp Saint--Venant inequality
\(\dT(\Om)\ge c_{\mathrm{SV}}^{\mathrm{glob}}(n)\Fr(\Om)^2\)
(Theorem~\ref{thm:SV-global}).

Finally, the Kohler--Jobin rearrangement is proved in \S\ref{sec:kj} from
scratch, in the modern formulation of Brasco \cite{Brasco2014}
specialised to the linear case, producing the scale-invariant
spectral-shape inequality
\(T(\Om)\lambda_1(\Om)^{(n+2)/2}\ge T_n L_n^{(n+2)/2}\);
combined with the global Saint--Venant inequality and an elementary
one-variable estimate (Lemma~\ref{lem:onevar}), it transfers
\eqref{eq:sharpSV} into \eqref{eq:FK-main} and completes the proof.

\subsubsection*{Outline of the paper}

Section~\ref{sec:preliminaries} gathers standing notation and the technical
inputs used in the proof: the constructive Fraenkel and Fusco--Julin
same-centre packages (\S\ref{sec:samecentre}), the coarea differentiation
framework on shell-admissible radii (\S\ref{sec:coarea}), and the
Kohler--Jobin inequality (\S\ref{sec:kj}).
Section~\ref{sec:FJ-quantitative} carries out the internal, computable
proof of the Fusco--Julin strong form (Theorem~\ref{thm:FJ-strong}).
Section~\ref{sec:main-proof} then assembles the main theorem in five
movements: the exact level-set deficit identity and its bad-density
consequences (\S\ref{sec:new-FK-proof}); the bounded reduction
(\S\ref{sec:part-bddred}); the affine moving-ball first variation
(\S\ref{sec:part-shell}); the endpoint trace (\S\ref{sec:part-endpoint});
the positive kinetic estimate (\S\ref{sec:part-kinetic}); and the closing
Kohler--Jobin transfer (\S\ref{sec:part-putting-together}).


\section{Setting and technical inputs}\label{sec:preliminaries}

This section fixes the standing notation, then prepares the four
technical ingredients fed into the main proof: the constructive Fraenkel
and Fusco--Julin same-centre packages on level sets
(\S\ref{sec:samecentre}); the coarea differentiation framework and the
shell-admissible radii on which the first variation will be carried out
(\S\ref{sec:coarea}); and the Kohler--Jobin spectral-shape inequality,
reproved from scratch (\S\ref{sec:kj}).
...
\S\ref{sec:FJ-quantitative}, with a dimensional constant
...
In \S\ref{sec:FJ-quantitative} the result is stated in the
normalisation \(|E|=|B_\rho|\) for the combined Fusco--Julin asymmetry
...
\eqref{eq:FJ-exact} above is the same statement with \(r_E\) in place
of \(\rho\).  The rescaling between the two normalisations is the
standard one and is the same as in the proof of
Corollary~\ref{cor:FJ-unnormalised} below.
\end{proof}
...
\section{An internal, computable proof of the Fusco--Julin inequality}\label{sec:FJ-quantitative}\label{app:FJ-quantitative}


This section gives a self-contained, computable proof of the Fusco--Julin
sharp quantitative isoperimetric inequality used in \S\ref{sec:samecentre}
(Theorem~\ref{thm:FJ-strong} and Corollary~\ref{cor:FJ-unnormalised}); the
dimensional constant \(C_{\rm FJ}(n)\) will be exhibited as a finite
composition of explicit dimensional constants, with no compactness
selection and no contradiction step.  The argument proceeds in three
blocks.  In \S\ref{app-sec:identity} we record the finite-perimeter radial
Gauss--Green identity that singles out the normal-oscillation index
\(\beta(E)\), and we reduce the problem to the small-deficit regime.  In
\S\ref{app-sec:graphs} we prove a Fuglede-type estimate with displayed
constants: a balanced \(W^{1,\infty}\)-small radial graph is controlled,
about one fixed centre, in both volume asymmetry and radial normal
oscillation by the perimeter deficit.  In \S\ref{app-sec:selection} we
run the Fusco--Julin regularising selection, prove the resulting set is
an additive perimeter almost-minimiser via quantitative annular trapping
and localisation of the potential centres, certify computability of the
graph-entry constant by a finite spectral truncation argument, and close
the global theorem on arbitrary finite-perimeter sets through the
Fusco--Julin confinement step.  The combined-asymmetry form of
\cite{FJ2014}, used as Theorem~\ref{thm:FJ-strong}, then follows.

\medskip\noindent\textbf{Notation warning.}
Throughout this section, the symbol \(\beta\) denotes the
\emph{radial-normal oscillation index} of Definition~\ref{app-def:beta},
that is, \(\beta(E)^2:=\inf_z\osc_z(E)\).  This is \emph{not} the
boundary-perimeter density function \(\beta(t)\) used elsewhere in the
manuscript.  The two objects share a letter for historical reasons but
live in different sections and never appear together in the same
display.
...
\subsection{Radial Gauss--Green identity and reduction to small deficit}\label{app-sec:identity}
...
\subsection{Nearly-spherical sets: an explicit Fuglede-type estimate}\label{app-sec:graphs}
...
\subsection{Regularising selection and the general case}
...
With the technical inputs of \S\ref{sec:preliminaries} and the internal
Fusco--Julin estimate of \S\ref{sec:FJ-quantitative} now in place, we are
ready to assemble Theorem~\ref{thm:FK-main}.  The argument runs in five
movements.  Section~\ref{sec:new-FK-proof} establishes the exact level-set
deficit identity, which writes \(\dT(\Om)\) as a reduced-boundary
integral of the Cauchy--Schwarz defect \(D_H\) and the squared-perimeter
isoperimetric defect \(D_I\), and derives its profile-gap and
bad-density consequences.  Section~\ref{sec:part-bddred} carries out the
De~Giorgi truncation that removes the boundedness hypothesis, taking the
bounded Saint--Venant stability inequality (Theorem~\ref{thm:SV-bounded}
below) as input.  Sections~\ref{sec:part-shell}--\ref{sec:part-kinetic}
then prove that bounded stability by developing the affine moving-ball
first variation, the endpoint trace, and the positive kinetic estimate.
Finally, \S\ref{sec:part-putting-together} closes the bounded estimate
and fits it together with the Kohler--Jobin transfer of \S\ref{sec:kj}
to produce Theorem~\ref{thm:FK-main}.

\subsection{The exact level-set deficit identity}\label{sec:new-FK-proof}
...
\subsection{Removing the boundedness hypothesis}\label{sec:part-bddred}\label{sec:bddred}
...
\subsubsection{Local notation and the tools used in this section}
\label{ssec:notation}
...
\item the \emph{suboptimal} torsional stability bound
   ...
   the truncation iteration; this seed is proved here from the level-set
   identity, the profile-gap identity, and the constructive Fraenkel
   inequality (Theorem~\ref{thm:constructive-fraenkel}).
\end{itemize}
...
\subsubsection{The De~Giorgi \texorpdfstring{$L^\infty$}{L-infinity} tail estimate}\label{ssec:Linfty}
...
\subsection{Affine first variation of the moving-ball distance}\label{sec:part-shell}\label{sec:shell}

We now turn to the proof of the bounded Saint--Venant inequality
(Theorem~\ref{thm:SV-bounded}), the input used in
\S\ref{sec:part-bddred}.  The technical engine is a finite-perimeter
one-sided derivative for the moving-ball distance \(q(\rho)\): pushing
the superlevel \(E_\rho\) forward by a homothety plus an affine
translation produces a competitor for \(q(\rho+h)\), whose
symmetric-difference cost is bounded by a single boundary integral
involving the velocity \(V_\rho\), the homothetic field \(H_{z,\rho}\),
and the translation parameter \(a\).
...
\subsection{The moving-ball endpoint trace}\label{sec:part-endpoint}\label{sec:endpoint}

The Lipschitz bound \(|q(\rho)-q(\sigma)|\le 2n\omega_n|\rho-\sigma|\)
just obtained (Lemma~\ref{lem:qLip}) lets us trade pointwise information
about \(q\) at the bad-density boundary \(\rdel\) for a Lebesgue-measure
average over a fixed-width slab.  The endpoint trace below converts the
budget bound \eqref{eq:budget} into a quadratic estimate on
\(q(\rdel)^2\), and is the geometric content that closes
...
\subsection{The positive kinetic estimate}\label{sec:part-kinetic}\label{sec:kinetic}

The affine shell estimate of \S\ref{sec:part-shell} provides an upper
bound on the one-sided derivative of \(q\) in terms of the level-set
defects \(D_H+D_I\) and an arbitrary choice of centre and translation.
The positive kinetic estimate below complements it: at a
distance-minimising centre it produces a \emph{lower} bound on the
homothetic-velocity defect of order \(q(\rho)\) itself, which is the
mechanism that converts the level-set budget \eqref{eq:budget} into a
quantitative estimate on \(q\).
...
\subsection{Closure of the bounded estimate and the Kohler--Jobin transfer}\label{sec:part-putting-together}

We now combine the pieces.  The endpoint trace
(Lemma~\ref{lem:endpoint}) bounds \(q(\rdel)^2\) by \(\dT(\Om)\); the
superlevel asymmetry transfer (Lemma~\ref{lem:superlevel-transfer})
passes from \(q(\rdel)\) back to \(\Fr(\Om)\); and the bound on
\(\rdel\) from Proposition~\ref{prop:levelset} guarantees that the
chosen radius is admissible whenever the deficit is small enough.
This yields the bounded Saint--Venant stability inequality, and the
proof of Theorem~\ref{thm:FK-main} is then completed by gluing the
global Saint--Venant inequality from \S\ref{sec:part-bddred} to the
Kohler--Jobin transfer of \S\ref{sec:kj}.
...
%---------------------------------------------------------------------
\subsubsection{From Saint--Venant to Faber--Krahn}
\label{ssec:transfer}
...
\bibitem{Faber1923}
G.~Faber,
\emph{Beweis dass unter allen homogenen Membranen von gleicher Fl\"ache und
gleicher Spannung die kreisf\"ormige den tiefsten Grundton gibt},
Sitzungsberichte der mathematisch-physikalischen Klasse der Bayerischen
Akademie der Wissenschaften zu M\"unchen (1923), 169--172.

\bibitem{Krahn1925}
E.~Krahn,
\emph{\"Uber eine von Rayleigh formulierte Minimaleigenschaft des Kreises},
Math.\ Ann.\ \textbf{94} (1925), 97--100.

\bibitem{Rayleigh1877}
J.~W.~Strutt (Lord Rayleigh),
\emph{The Theory of Sound, Vol.~I},
...
\bibitem{HansenNadirashvili1994}
...
\bibitem{BhattacharyaWeitsman1999}
T.~Bhattacharya and A.~Weitsman,
\emph{Estimates for Green's function in terms of asymmetry},
in \emph{Applied Analysis (Baton Rouge, LA, 1996)}, Contemp.\ Math.\
\textbf{221}, AMS, Providence, RI, 1999, pp.~31--58.

\bibitem{FuscoMaggiPratelli2009Eigenvalue}
N.~Fusco, F.~Maggi, and A.~Pratelli,
...
\bibitem{Tamanini1984}
I.~Tamanini,
\emph{Regularity results for almost minimal oriented hypersurfaces in
$\mathbf R^n$},
Quaderni Dipart.\ Mat.\ Univ.\ Lecce \textbf{1} (1984), 1--92.

\bibitem{Almgren1976}
F.~J.~Almgren,
\emph{Existence and regularity almost everywhere of solutions to elliptic
variational problems with constraints},
Mem.\ Amer.\ Math.\ Soc.\ \textbf{4} (1976), no.~165.
...
\emph{Sharp quantitative Faber--Krahn inequalities and the Alt--Caffarelli--Friedman
monotonicity formula},
preprint, arXiv:2107.03495 (2021).

\bibitem{Nitsch2014}
L.~Brasco, G.~De~Philippis, and B.~Ruffini,
\emph{Spectral optimization for the Stekloff--Laplacian: the stability
issue},
J.\ Funct.\ Anal.\ \textbf{262} (2012), 4675--4710.

\bibitem{DePhilippisVelichkov2020}
G.~De~Philippis and B.~Velichkov,
\emph{Existence and regularity of minimizers for some spectral functionals
with perimeter constraint},
Appl.\ Math.\ Optim.\ \textbf{69} (2014), 199--231.
