...
\cref{thm:torsion-reg,lem:regular-values,lem:containment}, in accordance
with the standing convention of \cref{ss:containment}: since $\hat v$ is a
regular value of the (real-analytic, in $\Om$) torsion function $u$ and
$\Etil$ is one connected component of $\{u>\hat v\}$, the set $\Etil$ is a
bounded open connected subset of $\Om$, on which $u-\hat v>0$ and
$\nabla u\neq0$ along $\set{u=\hat v}$. Its topological boundary
\[
   \partial\Etil=\set{u=\hat v}\cap\partial\Etil\subset\Om
\]
...
\begin{proof}
The set $\overline\Etil$ is bounded (\cref{lem:containment}\ref{it:cont-far}:
$\overline\Etil\cap\Om$ is compact in $\Om$, and $\Om$ is bounded, so
$\overline\Etil$ is a bounded subset of $\R^2$). Hence $U$ is the complement
of a bounded closed set and so has exactly one unbounded connected component
$U_\infty$ (the points of large modulus form a connected set lying in $U$).
Suppose $U$ had another (necessarily bounded) component $H$. By
\cref{lem:noholes}\,(ii) no bounded component of $\R^2\setminus\overline\Etil$
is compactly contained in $\Om$, so $\overline H$ is not a compact subset of
$\Om$, i.e.\ $\overline H$ meets $\partial\Om$; pick $q\in\overline H\cap
\partial\Om$. As $\overline\Etil$ is bounded, the point $q$ and the
...
to the end of the section because it reuses the trapping mechanism, but it is
logically independent of the conditional discussion and removes any
hypothesis on the location of $z$.) With
...
proved unconditionally in \cref{lem:zinterior}. The next lemma collects the
...
the scales \eqref{eq:ann-scales}\textup{)}, where the hypothesis
$z\in\Etil$ at the \emph{exact} $L^1$-optimal centre $z$ holds
unconditionally by \cref{lem:zinterior} below,
...
$\eta\le C\,\dT^{1/8}$. \emph{This rate is unconditional}: it is the ball
distance $a$ at the exact optimal centre that enters, with no recentring
penalty \textup{(}\cref{lem:zinterior}\textup{)}.
\end{theorem}
...
$\Etil$, with a definite inscribed ball. We prove this unconditionally and
\emph{at the sharp scale}, so that the rate \eqref{eq:annulus-rate} holds with
the ball distance $a$ measured at the exact optimum --- no recentring, hence
no square-root loss. The mechanism is the trapping mechanism itself, run
...
\label{lem:zinterior}
Let $\Etil$ be the inset \textup{(}$\dT\le\delta_\star$, connected,
hole-free, $\hat r\in[\tfrac12,1]$, scales \eqref{eq:ann-scales}\textup{)},
and let $z$ be an \emph{exact} $L^1$-optimal centre, $a=|\Etil\triangle
B_{\hat r}(z)|=\AF(\Etil)|\Etil|$. There is an absolute constant $C_0$ such
that, after possibly shrinking the absolute threshold $\delta_\star$,
...
hypothesis $z\in\Etil$ of \cref{thm:annulus} holds at the exact optimum, and
the rate \eqref{eq:annulus-rate} is unconditional.
\end{lemma}
...
unconditionally; the previously flagged degenerate boundary configuration of
the optimal centre does \emph{not} occur, and the conclusion
$\AF(\Om)^2\le C\dT$ carries no caveat on the location of the optimal centre.
The inputs of this section remain exactly connectedness, the no-hole property,
$\Exc$ and $a$ (the Serrin defect $D_H$ is not used).
\end{remark}
...
the inset's exact FMP centre is \emph{unconditional}; nothing in this section,
or downstream, is left contingent on the location of the optimal centre.
\end{remark}
