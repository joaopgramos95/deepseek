...
Let $\Omega\subset\R^2$ be bounded and let $u\in W^{1,1}(\Omega)\cap C^1(\Omega)$
with $u\ge 0$ \textup{(}no continuity up to $\partial\Omega$ is assumed; the
torsion function of \eqref{eq:torsion}, which is in $H^1_0(\Omega)\subset
W^{1,1}(\Omega)$ for bounded $\Omega$ and in $C^\infty(\Omega)$ by
\cref{thm:torsion-reg}, satisfies these hypotheses\textup{)}.  Define
$\mu(v):=|\{u>v\}|$ for $v\ge 0$.  Then $\mu$ is
...
\begin{proof}
The coarea formula \eqref{eq:coarea-formula} with $g = |\nabla u|^{-1}\mathbf{1}_{\{\nabla u\ne 0\}}$
gives, for $0\le s < t$,
\[
  |\{s < u \le t\}\cap\{\nabla u\ne 0\}|
