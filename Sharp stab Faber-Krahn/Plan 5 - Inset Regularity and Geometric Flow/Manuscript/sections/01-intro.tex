...
\Cref{sec:main} assembles all pieces and optimises the level parameter at
$\tau=\sqrt{\dT}$. The reduction is complete except for two precisely isolated
analytic points, on which the conclusion is conditional: the localisation of
the harmonic boundary-layer term in the cap-cutting estimate
\eqref{eq:flux-hyp} (the residual flux of the torsion function through the
interior cap floors, \cref{gap:torsion-stab}), and the quadratic mode-one
smallness \eqref{eq:mode1-scoped} of the graph profile at the optimal centre
(\cref{gap:barycentre}), which plays the role of a barycentre normalisation
without recentring the set. Both are stated explicitly as scoped hypotheses
where they are used; every other step --- including the trapping annulus, the
hole-filling, the sharp fold content, the measurable-graph realisation, and
the nearly-spherical coercivity --- is proved unconditionally.
