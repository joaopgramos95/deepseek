...
\end{equation}
Since $\mu_E=\abs\Etil=\pi-\abs{\Om\triangle\Etil}$ (as $\Etil\subset\Om$),
...
such that, with $G:=\{z+r\omega:r<\rho(\omega)\}$ and $G_0=\{r<\rho_0\}$ the
core--profile graph of \cref{cor:gd},
\begin{equation}\label{eq:s-real}
   \abs{\Etil\triangle G}\ \le\ \abs{\Etil\setminus G_0}+C\,\sigma,
   \qquad
...
\le C$, so the smoothing cost is $\le C\sigma=C\dT$). By \cref{cor:gd} the
core--profile defect obeys $\abs{\Etil\setminus G_0}\le C\,\eta(\Exc+a\eta)$,
the same bound as \eqref{eq:s-gd} for $\gd(\Etil)\le\abs{\Etil\setminus G_0}$;
hence
\begin{equation}\label{eq:s-EtilG}
   \abs{\Etil\triangle G}\ \le\ \abs{\Etil\setminus G_0}+C\dT
   \ \le\ C\,\eta(\Exc+a\eta)+C\dT\ \le\ C\,\dT^{1/2}.
\end{equation}
...
Here the excised set $\Etil\setminus G_0$ is the union of radial caps, all
contained in the collar $\{\rho_i<\abs{x-z}<\rho_e\}$ (\cref{cor:gd}), so the
collar hypothesis of \cref{lem:cutdeficit} is met; hence \cref{lem:cutdeficit}
...
\emph{Step 5: transfer of the deficit from $\Om$ to $\Etil$.} This is the
inset control \eqref{eq:dstar-inset} of \cref{prop:insetcontrols}, which
states
\begin{equation}\label{eq:s-transfer}
   \dstar(\Etil)\ \le\ \frac{\pi^2}{\mu_E^2}\,\dstar(\Om)\ \le\ C\,\dstar(\Om).
\end{equation}
We stress that this bound is established in \cref{sec:inset}
...
$\Etil$ is handled there by the exact additivity of torsion over the
...
both internal to \cref{sec:identity,sec:inset}. In particular the spillover
components need \emph{not} lie in the trapping collar of $\Etil$ --- so
\cref{lem:cutdeficit}, whose hypothesis requires the excised set to sit in a
thin concentric collar, is \emph{not} invoked here, and there is no circular
