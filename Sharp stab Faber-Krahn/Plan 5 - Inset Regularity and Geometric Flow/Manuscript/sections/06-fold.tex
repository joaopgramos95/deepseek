...
obeys $\hat r\ge\tfrac34$ (since $\mu_E=\pi-O(\sqrt\dT)$ by \cref{def:inset},
so $\hat r=\sqrt{\mu_E/\pi}\to1$), and $\eta\le C\dT^{1/8}\le\tfrac14$ by
\eqref{eq:fold-annulus}, whence $\rho_i\ge\hat r-\eta\ge\tfrac12$ (using
...
variation by the perimeter of $\Etil$ through the circle-crossing function
\[
   N(\rho):=\HH^0\bigl(\partial\Etil\cap\partial B_\rho(z)\bigr)
\]
of \cref{lem:angular-coarea}, which is the rigorous substitute for the
(false-in-the-$BV$-jump-case) identification of $\mathrm{TV}(\rho_0)$ with the
arclength of the polar graph point-set $\{z+\rho_0(\theta)e^{i\theta}\}$. The
key inequality is the layer-cake comparison
\begin{equation}\label{eq:TVlayer}
   \mathrm{TV}(\rho_0)
   \ =\ \int_{\rho_i}^{\rho_e}\HH^0\bigl(\partial^*\{\rho_0>\rho\}\bigr)\,d\rho
   \ \le\ \int_{\rho_i}^{\rho_e}N(\rho)\,d\rho .
\end{equation}
The first equality is the one-dimensional coarea (layer-cake) formula for the
$BV$ function $\rho_0$ on $S^1$ \cite[Theorem~3.40]{AFP2000}: for a.e.\
$\rho$ the superlevel set $\{\theta:\rho_0(\theta)>\rho\}\subset S^1$ is a
finite union of arcs, its essential boundary $\partial^*\{\rho_0>\rho\}$ is a
finite set of points, and the variation measure disintegrates as
$\abs{D\rho_0}(S^1)=\int_0^\infty\HH^0(\partial^*\{\rho_0>\rho\})\,d\rho$
(the integrand vanishes off $[\rho_i,\rho_e]$ since $\rho_0\in[\rho_i,
\rho_e]$). For the inequality, fix a regular $\rho\in(\rho_i,\rho_e)$ and let
$\theta_0\in\partial^*\{\rho_0>\rho\}$ be a point where $\rho_0=r_1$ crosses
the level $\rho$ (jumping or continuously). Since $r_1(\theta)$ is the
innermost crossing radius of $\partial\Etil$ on the ray $R_\theta$, the
crossing of the level $\rho$ at $\theta_0$ is realised by a point
$z+\rho\,e^{i\theta_0}$ of $\partial\Etil$ lying on the circle
$\partial B_\rho(z)$: indeed, on one side of $\theta_0$ the inner interval
$(0,r_1)$ of the slice \eqref{eq:fold-slice} reaches past radius $\rho$ and on
the other side it does not, so $\partial B_\rho(z)$ separates points of
$\Etil^{(1)}$ from points of $\Etil^{(0)}$ arbitrarily near
$z+\rho\,e^{i\theta_0}$, whence $z+\rho\,e^{i\theta_0}\in\partial\Etil$.
Distinct boundary directions $\theta_0$ give distinct points on
$\partial B_\rho(z)$, so $\HH^0(\partial^*\{\rho_0>\rho\})\le N(\rho)$,
proving \eqref{eq:TVlayer}. Now \cref{lem:angular-coarea} states
$\int_0^\infty N(\rho)\,d\rho=P_\theta:=\int_{\partial\Etil}\abs{\nu_\theta}
\,d\HH^1\le\Per(\Etil)$ (as $\abs{\nu_\theta}\le1$). Combining with
\eqref{eq:TVlayer},
\begin{equation}\label{eq:TVbound}
   \mathrm{TV}(\rho_0)\ \le\ \int_{\rho_i}^{\rho_e}N(\rho)\,d\rho
   \ \le\ P_\theta\ \le\ \Per(\Etil)\ \le\ \frac{\Per(\Etil)}{\rho_i}
   \ \le\ C,
\end{equation}
the penultimate inequality holding since $\rho_i\le1$ (indeed $\rho_i\ge
\tfrac12$ for $\dT\le\delta_\star$, so $1/\rho_i\le2$), and the last using
$\Per(\Etil)=2\pi\hat r+\Exc\le C$ for $\dT\le\delta_\star$. The form
