...
\title{The Selection--Level--Brandolini Hybrid Route\\
...
A bounded open set $U\subset\R^n$, $n\ge 2$, with the same volume as
the ball $B$ produces a smaller torsional rigidity unless it
\emph{is} a ball; the Saint--Venant deficit
\[
   \dT(U) \;:=\; T(B)-T(U) \;\ge\; 0
\]
quantifies this defect.  The Fraenkel asymmetry
\[
   \AF(U) \;:=\; \inf_{x_0\in\R^n}
   \frac{\bigl|U\triangle (x_0+B)\bigr|}{|U|}
\]
measures, in $L^1$, how far $U$ sits from being a translate of $B$.
A central conjecture in quantitative shape optimisation predicts that
these two scalar invariants are comparable in the sharp, dimension-free
fashion
\begin{equation}\label{eq:conjecture}
   \AF(U)^{2} \;\le\; C\,\dT(U),
\end{equation}
the exponent~$2$ on the left being optimal: a normal perturbation
$\partial B\mapsto\partial B+\eps\,\varphi(\theta)\,\nu$ of the unit
ball with zero-mean profile $\varphi\in C^\infty(S^{n-1})$ produces
$\AF\asymp\eps$ and, by a direct shape-derivative computation,
$\dT\asymp\eps^{2}$, so neither exponent in~\eqref{eq:conjecture} can
be improved.  The purpose of this article is to prove
\eqref{eq:conjecture} in every dimension $n\ge 2$ along a streamlined
hybrid route whose load-bearing geometric step is dimension-free.

\subsection*{A brief history}

The story begins with Fuglede's nearly-spherical-set
theorem~\cite{Fuglede1989}, which established the sharp quadratic
estimate $|E\triangle B|^{2}\lesssim P(E)-P(B)$ for $C^{1}$ normal
graphs over $\partial B$ with vanishing barycentre.  Three decades
later, the breakthrough of Brasco, De~Philippis and
Velichkov~\cite{BDV2015} settled the analogous problem for the
\emph{first Dirichlet eigenvalue} in any dimension: the inequality
$\AF(U)^{2}\le C\,(\lambda_1(U)-\lambda_1(B))$ holds with the sharp
exponent~$2$, proved via a selection principle that replaces a generic
competitor by a regular quasi-minimiser, followed by a nearly-spherical
bootstrap through Schauder all the way to $C^{2,\alpha}$ where Fuglede
applies.  In a parallel line of work, Brandolini, Nitsch, Salani and
Trombetti~\cite{BNST} produced a sharp \emph{Serrin}
overdetermined-problem stability theorem: a solution to $\Delta w=2$
in $D$ with $|Dw|$ close to a constant on $\partial D$ forces $D$ to
be quantitatively close to a ball, in the sense of a concentric
annular inclusion.  For the torsion (Saint--Venant) functional the
analogue of~\cite{BDV2015} was, however, only partially completed: in
two dimensions the sharp exponent had been reached by an isoperimetric
pocket-filling argument special to the plane, while in higher
dimensions either a planar hole-filling alternative or a genuinely
dimension-free replacement was missing.

\subsection*{What is proved here}

We close this gap by assembling four steps into a single chain that
is dimension-free at every stage except the BNST input itself.

\smallskip
\noindent\emph{Step~1 (Selection, \S\ref{sec:step1-selection}).}
A quantitative selection principle replaces $U$ by a member $\Etil$
of a regular class $\Sclass$ (Definition~\ref{def:Sclass}); we use
only the Alt--Caffarelli $C^{1,\gamma}$ free-boundary output for
selected quasi-minimisers (Theorem~\ref{thm:BDV-reg}), \emph{without}
the further Schauder bootstrap to $C^{2,\alpha}$ relied upon
in~\cite{BDV2015}.  This is possible because Step~2 is engineered so
that BNST is applied at the $C^{1,\gamma}$ regularity level.

\smallskip
\noindent\emph{Step~2 (Entering the $\sqrt{\dT}$ boundary layer,
\S\ref{sec:step2-bdry}).}  Using the volume-variable level identity
relating the heat-content and isoperimetric defects, we extract a
good level set $\Et$ of the torsion function $u_{\Etil}$ at height
$\hat t\asymp\sqrt{\dT}$ on which $\|\,|Du_{\Etil}|-1\|_{L^{\infty}
(\partial\Et)}$ is quantitatively small.  A $D_H\to L^\infty$ Serrin
upgrade by $\gamma$-Hölder interpolation then converts the $L^{2}$
Serrin defect into the $L^{\infty}$ input required by the BNST
theorem~\eqref{thm:BNST-Cgamma}, whose conclusion is an annular
inclusion $B_{r-\rho}(x_0)\subset \Et\subset B_{r+\rho}(x_0)$ with
$\rho$ an explicit power of $\dT$.

\smallskip
\noindent\emph{Step~3 (Realising the inset as a radial graph,
\S\ref{sec:step3-radialgraph}).}  The Brandolini annular inclusion is
\emph{geometric} but not yet of graph form.  The dimension-free
radial-graph promotion lemma (Lemma~\ref{lem:radialgraph}) combines
the inset inclusion with the $C^{1,\gamma}$ regularity inherited
from~$\Sclass$ to force $\partial\Et$ to be a $C^{1,\gamma}$ radial
graph over the sphere centred at~$x_0$, with no pocket-filling needed.
In two dimensions a slick polar-slicing alternative is available, and
is recorded as the self-contained subsection \S\ref{sec:hole-fill} for
the planar reader; it does not extend to $n\ge 3$ at the sharp
scaling, which is precisely why Lemma~\ref{lem:radialgraph} is the
dimension-free workhorse of the route.

\smallskip
\noindent\emph{Step~4 (Quantitative radial-graph theorem $(G_n)$,
\S\ref{sec:step4-Gn}).}  A direct small-amplitude radial-graph
stability theorem
(Theorems~\ref{thm:G12-explicit}--\ref{thm:Gn}) shows that, for
graphs over $S^{n-1}$ with barycentre at the centre and sufficiently
small amplitude, $\AF(\Et)^{2}\le C\,\dT(\Et)$.  A short
barycentre-recentring argument discharges the centring hypothesis.
Combined with the previous three steps, this proves
\eqref{eq:conjecture}.

\smallskip

The entire chain is dimension-free in the sense that each individual
step holds for $n\ge 2$ with constants depending only on $n$ and on
the selection-class parameters; the BNST Serrin theorem is the only
black-box input, and we re-prove it in Appendix~\ref{sec:BNST-proof}
at the $C^{1,\gamma}$ regularity level, strengthening the original
$C^{2,\alpha}$ statement of~\cite{BNST} (this is the
ingredient that lets us avoid the Schauder bootstrap in Step~1).

\subsection*{Main result}

\begin{theorem}[Sharp Saint--Venant stability]\label{thm:main}
Let $n\ge 2$.  There exist constants $C=C(\Sclass,n)>0$ and
$\delta_\star=\delta_\star(\Sclass,n)>0$ such that, for every
$U\in\Sclass$ with $\dT(U)\le\delta_\star$,
\[
   \AF(U)^{\,2} \;\le\; C\,\dT(U).
\]
Consequently, by the quantitative selection principle
(Theorem~\ref{thm:P-package}), there exist $C'=C'(n)>0$ and
$\delta'_\star=\delta'_\star(n)>0$ such that the same bound
$\AF(U)^{2}\le C'\,\dT(U)$ holds for every bounded open
$U\subset\R^n$ with $\dT(U)\le\delta'_\star$ and a uniform interior
cone.
\end{theorem}

\begin{remark}
The exponent $2$ in Theorem~\ref{thm:main} is sharp; the smallness
threshold $\delta_\star$ cannot be removed without changing the
exponent, since for $\dT$ of order one a global asymmetry estimate
must degenerate to the trivial $\AF\le 2$.
\end{remark}

\subsection*{Organisation}

Section~\ref{sec:prelim} collects preliminaries: the torsion
convention, the volume-variable $D_H/D_I$ level identity (with a
self-contained proof of Faber--Krahn / Saint--Venant), the
Alt--Caffarelli free-boundary input, the BNST theorem in its
$C^{1,\gamma}$ form, and the selection class~$\Sclass$.
Section~\ref{sec:step1-selection} executes Step~1 (selection
principle), opening with a gradient-flow heuristic that explains
\emph{why} the selected $\Etil$ inherits the regularity needed
downstream.  Section~\ref{sec:step2-bdry} carries out Step~2: the
descent into the $\sqrt{\dT}$ boundary layer, the $D_H\to L^\infty$
upgrade, and the BNST application.
Section~\ref{sec:step3-radialgraph} proves the dimension-free
radial-graph promotion lemma; the planar polar-slicing alternative is
laid out in the subsection~\S\ref{sec:hole-fill}.
Section~\ref{sec:step4-Gn} closes the chain with the quantitative
radial-graph theorem $(G_n)$ together with the
barycentre-recentring/transfer assembly that concludes the proof of
Theorem~\ref{thm:main}.  Two appendices collect the $C^{1,\gamma}$
re-proof of BNST (Appendix~\ref{sec:BNST-proof}) and the
Alt--Caffarelli free-boundary regularity package
(Appendix~\ref{sec:BDV-FB-detail}).
...
\subsection{The level-set deficit identity}\label{ss:level-id}

This subsection records, in self-contained form, the analytic
identity that drives the entire route.  Given the torsion function
\(u=u_U\), it splits the Saint--Venant deficit \(\dT(U)\) as a
non-negative integral, along the levels \(\{u=t\}\), of two pointwise
loss densities: a Cauchy--Schwarz loss \(D_H(t)\) measuring how
non-constant \(|\nabla u|\) is on the level, and an isoperimetric
loss \(D_I(t)\) measuring how non-circular the level is.  The
Faber--Krahn / Saint--Venant inequality \(\dT(U)\ge 0\) is an
immediate corollary (Corollary~\ref{cor:FK}); the quantitative use,
via Markov-type extraction of a ``good level'' at the
\(\sqrt{\dT}\) scale, is what drives Step~2.

\paragraph{Setup.}  Let \(U\subset\R^n\) be bounded open with smooth
boundary; the formulas extend to finite-perimeter sets by the standard
approximation.  Write \(u=u_U\), \(m(t):=|\{u>t\}|\),
\(\Sigma_t:=\{u=t\}\), \(P(t):=\HH^{n-1}(\Sigma_t)\),
\(f:=|\nabla u|\), and let \(v(s):=u^{*}(s)=m^{-1}(s)\) be the
decreasing rearrangement of \(u\).  For a regular level \(t\) the
coarea formula and the divergence theorem applied to \(E_t=\{u>t\}\)
give
\begin{equation}\label{eq:lev-coarea}
   a(t):=-m'(t)=\int_{\Sigma_t}\frac{1}{f}\,d\HH^{n-1},
   \qquad
   \int_{\Sigma_t}f\,d\HH^{n-1}=m(t).
\end{equation}
Set \(c_n:=n^{2}\omega_n^{2/n}\) (so \(c_2=4\pi\)).  The two
pointwise losses of the level-set proof of Faber--Krahn are
\begin{equation}\label{eq:DH-DI-def}
   D_H(t):=a(t)\,m(t)-P(t)^{2},
   \qquad
   D_I(t):=P(t)^{2}-c_n\,m(t)^{2-2/n}.
\end{equation}
Both are non-negative: \(D_H(t)\ge 0\) by Cauchy--Schwarz applied to
\(f^{1/2}\) and \(f^{-1/2}\), and \(D_I(t)\ge 0\) by the Euclidean
isoperimetric inequality applied to \(E_t\).  Their sum
\(D_{\rm lev}(t):=D_H(t)+D_I(t)=a(t)m(t)-c_n m(t)^{2-2/n}\) is the
total level-set loss.

A direct algebraic expansion gives the \emph{weighted variance
identity}
\begin{equation}\label{eq:lev-variance}
   \int_{\Sigma_t}\frac{(f-\bar f)^{2}}{f}\,d\HH^{n-1}
   \;=\;\frac{m(t)}{P(t)^{2}}\,D_H(t),
   \qquad
   \bar f:=\frac{1}{P(t)}\int_{\Sigma_t}f\,d\HH^{n-1}=\frac{m(t)}{P(t)},
\end{equation}
which exhibits \(D_H(t)\) as the squared weighted \(L^{2}\)-defect of
\(|\nabla u|\) from its mean on \(\Sigma_t\).  This identity is the
bridge to the quantitative Serrin estimate of
\S\ref{ss:pointwise-serrin}.

\begin{theorem}[Level-set deficit identity]\label{thm:level-identity}
For every bounded open \(U\subset\R^n\) with smooth torsion function
\(u_U\), and with \(L:=|U|\),
\begin{equation}\label{eq:level-identity-t}
   \dT(U)
   \;=\;\frac{1}{c_n}\int_0^{\|u\|_\infty}
        \frac{D_H(t)+D_I(t)}{m(t)^{1-2/n}}\,dt.
\end{equation}
Equivalently, after the change of variables \(s=m(t)\), \(t=v(s)\),
\(ds=-a(t)\,dt\),
\begin{equation}\label{eq:level-identity-s}
   \dT(U)
   \;=\;\frac{1}{c_n}\int_0^{L}
        \frac{D_H(v(s))+D_I(v(s))}{s^{\,1-2/n}\,a(v(s))}\,ds.
\end{equation}
In the planar case \(n=2\), \(c_n=4\pi\) and \(m^{1-2/n}=s^{1-2/n}=1\),
so
\begin{equation}\label{eq:level-identity-2d-t}
   \dT(U)
   \;=\;\frac{1}{4\pi}\int_0^{\|u\|_\infty}\!\bigl(D_H(t)+D_I(t)\bigr)\,dt,
\end{equation}
\begin{equation}\label{eq:level-identity-2d-s}
   \dT(U)
   \;=\;\frac{1}{4\pi}\int_0^{L}\frac{D_H(v(s))+D_I(v(s))}{a(v(s))}\,ds.
\end{equation}
\end{theorem}

\begin{proof}
Define the \emph{convexity variable}
\begin{equation}\label{eq:lev-Gdef}
   G(s):=I(s)+\frac{s^{1+2/n}}{(4+2n)\,\omega_n^{2/n}},
   \qquad
   I(s):=\int_{\{u>v(s)\}}u\,dx.
\end{equation}
By the rearrangement formula, \(I'(s)=v(s)\) and
\(I''(s)=v'(s)=1/m'(v(s))=-1/a(v(s))\) at regular \(s\).  The second
derivative of the added term is
\(\frac{d^{2}}{ds^{2}}\!\bigl(s^{1+2/n}/((4+2n)\omega_n^{2/n})\bigr)
=1/(c_n s^{1-2/n})\).  Combining, with \(s=m(t)\),
\begin{equation}\label{eq:lev-Gpp}
   G''(s)\;=\;\frac{1}{c_n\,s^{1-2/n}}-\frac{1}{a(t)}
        \;=\;\frac{a(t)\,m(t)-c_n\,m(t)^{2-2/n}}{s\,a(t)\,c_n\,s^{1-2/n}}
        \;=\;\frac{D_H(t)+D_I(t)}{s\,a(t)\,c_n\,s^{1-2/n}},
\end{equation}
using the decomposition \(D_H+D_I=a m-c_n m^{2-2/n}\) and \(s=m(t)\).

Set the endpoint convexity gap
\(\Gamma(U):=G'(L)-G(L)/L\).  Because \(G(0)=0\), integration by parts
on \([0,L]\) gives
\begin{equation}\label{eq:lev-IBP}
   \Gamma(U)\;=\;\frac{1}{L}\int_0^L\!s\,G''(s)\,ds.
\end{equation}
Substituting \eqref{eq:lev-Gpp},
\[
   \Gamma(U)\;=\;\frac{1}{L}\int_0^{L}\!
   \frac{D_H(v(s))+D_I(v(s))}{c_n\,a(v(s))\,s^{1-2/n}}\,ds,
\]
which is the bracketed integrand of \eqref{eq:level-identity-s}
divided by \(L\).

It remains to identify \(\Gamma(U)\) with \(\dT(U)/L\).  At \(s=L\)
we have \(v(L)=0\), so
\(G'(L)=v(L)+L^{2/n}/(2n\,\omega_n^{2/n})=L^{2/n}/(2n\,\omega_n^{2/n})\).
Also
\(G(L)/L=L^{-1}\int_U\!u\,dx+L^{2/n}/((4+2n)\omega_n^{2/n})\).
Subtracting and simplifying,
\[
   \Gamma(U)
   \;=\;\frac{L^{2/n}}{2n\,\omega_n^{2/n}}-\frac{L^{2/n}}{(4+2n)\omega_n^{2/n}}
        -\frac{1}{L}\int_U\!u\,dx
   \;=\;\frac{L^{2/n}}{n(n+2)\,\omega_n^{2/n}}-\frac{1}{L}\int_U\!u\,dx.
\]
The first term, multiplied by \(L\), is exactly the disk torsion
\(T(B_L)\) (where \(B_L\) is the ball of volume \(L\)): by direct
integration \(T(B_r)=\omega_n r^{n+2}/[n(n+2)]\), so
\(T(B_L)=L^{1+2/n}/[n(n+2)\omega_n^{2/n}]\).  The second term,
multiplied by \(L\), is \(\int_U u=T(U)\).  Hence
\[
   L\,\Gamma(U)\;=\;T(B_L)-T(U)\;=\;\dT(U),
\]
which combined with the previous display gives
\eqref{eq:level-identity-s}; the \(t\)-form
\eqref{eq:level-identity-t} follows from the change of variables
\(s=m(t)\), \(ds=-a(t)\,dt\) (the orientation reversal cancelling
with the sign of \(ds\)).  The planar specialisations
\eqref{eq:level-identity-2d-t}--\eqref{eq:level-identity-2d-s} are
the case \(n=2\), \(c_2=4\pi\), \(m^{1-2/n}=1\).
\end{proof}

\begin{corollary}[Faber--Krahn / Saint--Venant]\label{cor:FK}
For every bounded open \(U\subset\R^n\) with smooth torsion function,
\(\dT(U)\ge 0\), with equality if and only if \(U\) is a ball up to a
Lebesgue-null set.
\end{corollary}

\begin{proof}
The integrand on the right of \eqref{eq:level-identity-t} is
pointwise non-negative, so \(\dT(U)\ge 0\).  Equality forces
\(D_H(t)\equiv D_I(t)\equiv 0\) for a.e.\ \(t\), i.e.\ on a.e.\ level
\(\Sigma_t\), \(|\nabla u|\) is constant and \(E_t\) is a ball.
Standard rearrangement reconstruction (a.e.\ levels being balls and
\(u\) having constant gradient on each level) forces \(U\) to be a
ball up to a Lebesgue-null set.
\end{proof}

\paragraph{The form used downstream.}
The factor \(1/a(v(s))\) in the \(s\)-form
\eqref{eq:level-identity-2d-s} is bounded above and below on the
selected class \(\Sclass\).  Indeed, for \(U\in\Sclass\), the Hopf
gradient pinching (S5) gives \(q_-\le|\nabla u|\le q_+\) on
\(\partial U\), and by the boundary-layer height
Lemma~\ref{lem:height} below, on the layer-scale levels
\(t\in(0,r_0 c_0/2)\) the same bounds hold up to controllable
\(\gamma\)-Hölder error.  Combining with the \(\Sclass\)-uniform
perimeter bound \(P_-\le P_t\le P_+\),
\begin{equation}\label{eq:lev-a-bound}
   a(t)\;=\;\int_{\Sigma_t}\frac{1}{|\nabla u|}\,d\HH^{n-1}
   \;\in\;\Bigl[\frac{P_-}{q_+},\ \frac{2P_+}{q_-}\Bigr]
   \;=:\;[a_-(\Sclass),\,a_+(\Sclass)],
\end{equation}
with \(a_\pm\) depending only on \(\Sclass\).  Combining
\eqref{eq:level-identity-2d-s} with \eqref{eq:lev-a-bound} gives the
\(s\)-form integrated bound
\begin{equation}\label{eq:level-identity}
   \int_0^{|U|}\!\bigl(D_H(v(s))+D_I(v(s))\bigr)\,ds
   \;\le\;C_{\rm lev}(\Sclass)\,\dT(U),
   \qquad
   C_{\rm lev}(\Sclass):=4\pi\,a_+(\Sclass).
\end{equation}
In the body of \S\S\ref{sec:step1-selection}--\ref{sec:step4-Gn} we
write \(8\pi\) in place of \(C_{\rm lev}(\Sclass)\): this is the
value of \(C_{\rm lev}\) under the harmless normalisation
\(a_+(\Sclass)\le 2\) (achieved for sets with perimeter close to that
of the unit disk and gradient close to \(1/2\)).  The precise numeric
constant plays no further structural role; only the existence of a
\(\Sclass\)-uniform finite bound matters in what follows.
...
\subsection{Heuristic: a controlled deformation flow}
\label{ss:flow-heuristic}

Before stating the selection principle itself, we explain in informal
terms \emph{why} one expects such a principle to hold, and how its
proof secretly tracks a gradient flow.

Suppose we are handed a near-counterexample to Saint--Venant, i.e.\ a
bounded open set $U\subset\R^n$ with $|U|=\omega_n$, small deficit
$\dT(U)$, and small Fraenkel asymmetry $\AF(U)$.  Outside a controlled
class such a $U$ may be arbitrarily rough: cusps, fjords, vanishing
boundary gradient.  Our goal is to replace $U$ by a regular companion
$U'\in\Sclass$ which is close enough to $U$ that $\dT(U)$ and
$\AF(U)$ are controlled by those of $U'$.

The Plan-1 / BDV idea is to \emph{minimise a penalised torsional
energy} of the cartoon form
\begin{equation}\label{eq:J-cartoon}
   \mathcal{J}(V):=-T(V)+\eps\,\bigl(|V|-\omega_n\bigr)^{2}
      +\Lambda\,\bigl[\bigl(\alpha(V)-\alpha(U)\bigr)_{+}\bigr]^{2},
\end{equation}
where $\alpha(\cdot)$ is a smoothed surrogate for $\AF$ (the BDV
smooth asymmetry; see the precise functional $J_{\sigma,\Lambda}$ in
\S\ref{ss:penalised} below).  We use $\alpha$ rather than $\AF$
itself because $\alpha$ is Lipschitz under $L^{1}$-convergence of
indicators, whereas $\AF$ is only lower semicontinuous.  A minimising
sequence for $\mathcal{J}$ is morally a discrete \emph{gradient flow}
$V_{t}$: along the flow the torsional term $-T(V_{t})$ can only
decrease, while the two penalty terms remain coercive in $|V_{t}|$
and in $\alpha(V_{t})$, so $V_{t}$ cannot degenerate or escape to
infinity.

The crucial observation is that this flow preserves the order of
magnitude of the deficit and of the asymmetry:
\begin{itemize}[leftmargin=2em]
\item the energy term forces $\dT(V_{t})\lesssim\dT(V_{0})=\dT(U)$,
because the rate at which $-T$ decreases is bounded by the
penalisation; quantitatively, the comparison
$\dT(U')\le C_1\,\dT(U)$ in
Theorem~\ref{thm:P-package}\,(ii) is what encodes this;
\item the asymmetry penalty, written with the one-sided positive
part $(\alpha(V)-\alpha(U))_{+}$, only penalises drops in $\alpha$
below the initial value $\alpha(U)$, never the reverse; hence the
minimiser $U'$ has $\alpha(U')$ comparable to (in fact, not much
smaller than) $\alpha(U)$, and the BDV partner inequality
$\alpha\gtrsim\AF^{2}$ on the near-spherical class transfers this to
$\AF$.
\end{itemize}
The limit $U'$ of the flow is a quasi-minimiser of the
\emph{Bernoulli}-type energy $V\mapsto -T(V)+\Lambda_{+}|V|$ and, by
the Alt--Caffarelli / BDV free-boundary theory
(Theorem~\ref{thm:BDV-reg}), automatically has a $C^{1,\gamma}$
boundary with a class-uniform atlas norm.  This is exactly the
regularity packaged in $\Sclass$.

The rest of this section turns the cartoon into the statement and
proof of the selection principle (Theorem~\ref{thm:P-package}
below); the argument is a streamlined reorganisation of the
construction in~\cite[\S4]{BDV2015}, presented here so that
downstream sections (\S\S\ref{sec:step2-bdry}--\ref{sec:step4-Gn})
may work entirely inside~$\Sclass$.
...
\section{Step 2: Entering the $\sqrt{\dT}$ boundary layer}
\label{sec:step2-bdry}

This section executes the technical heart of the route.  Given
$U\in\Sclass$ delivered by Step~1, we extract a single level
$\hat t\asymp\sqrt{\dT(U)}$ at which the level-set deficit identity
of~\S\ref{sec:level-id} is sharpest, upgrade the resulting $L^2$
Serrin defect to a pointwise bound by $\gamma$-Hölder interpolation,
inherit a uniform $C^{1,\gamma}$ atlas for the rescaled inset, and
apply the Brandolini--Nitsch--Salani--Trombetti theorem (in its
$C^{1,\gamma}$ form, Appendix~\ref{sec:BNST-proof}) to obtain a
near-spherical inclusion.  A barycentre-recentring step concludes
Step~2 with a $C^{1,\gamma}$ inset of explicit Hausdorff distance to
the unit ball.

\subsection{Level-set identities and the stopped level}\label{sec:level-id}
...
\subsection{From \(D_H\) to pointwise Serrin}\label{ss:pointwise-serrin}
...
\subsection{Inset regularity, recentring, and Brandolini}
...
\section{Step 3: Realising the inset as a radial graph}\label{sec:step3-radialgraph}
...
\subsection{Hole-filling: an alternative route for $n=2$}
\label{sec:hole-fill}
...
\section{Step 4: Quantitative stability for $L^\infty$--$L^2$ small graphs}\label{sec:step4-Gn}
...
\subsection{The selected-domain theorem}\label{sec:selected-domain}
...
\subsection{Transfer from arbitrary planar sets}\label{sec:transfer}
...
\bibitem{Fuglede1989} B.~Fuglede, \emph{Stability in the isoperimetric problem for convex or nearly spherical domains in $\R^n$}, Trans. Amer. Math. Soc. \textbf{314} (1989), 619--638.
