...
\title{Weighted Serrin Collar Reduction}
\author{}
\date{}
...
\texttt{selected-boundary-layer-theorem.md} and identifies the actual remaining
sharpness obstruction.

The conclusion is:
\begin{enumerate}
\item the weighted \(D_H\)-to-Serrin upgrade is available in selected collars,
at least in the standard oscillation form, by interpolation;
\item the specific Input S used in
\texttt{selected-boundary-layer-theorem.md} is even simpler: it follows from
quantitative isoperimetry through \(D_I(t)\);
\item therefore the missing sharp piece is not Serrin stability. It is a
boundary deficit propagation theorem that avoids the \(1/\eta\) loss in the
...
\section{Selected-Collar Hypotheses}
...
\section{Weighted Variance Gives Ordinary Variance}
...
\section{Interpolation to Oscillation}
...
\section{Input S Is Already Implied by the Isoperimetric Deficit}

The Input S used in \texttt{selected-boundary-layer-theorem.md} was
\[
\mathcal A(E_t)^2
\le
...
\section{Boundary Traces of the Level Deficits}
...
\section{The Tempting Boundary-Deficit Theorem Is Too Strong}
...
\paragraph{Boundary deficit propagation.}
For selected minimizers in the graph regime,
\[
D_I(0)+D_H(0)
\le
...
\section{Relation to Plan 1}

The plausible replacement is the selected Bernoulli law. The Plan 1 notes on
Bernoulli spectral closure linearize the Bernoulli condition and find a
spectral multiplier \(k-1\) on spherical harmonics. This would suppress the
non-neutral modes if the required tame \(C^{2,\gamma}\times L^2\) Bernoulli
remainder estimate were proved. That estimate is not currently established.

In boundary-deficit language:
\begin{itemize}
\item \(D_H(0)\) measures failure of the Bernoulli datum \(q=|\nabla u|\) to be
constant.
\item \(D_I(0)\) measures the perimeter deficit of the selected free boundary.
\item These defects are too strong to be bounded by torsion energy for
arbitrary graphs. The Bernoulli spectral route is an attempted mechanism for
excluding the high-frequency modes on the selected class, but it remains
conditional on the missing tame expansion.
\end{itemize}
So the remaining work is either the BDV graph-entry plus nearly spherical second
variation, or a fully justified Plan 1 Bernoulli spectral expansion. It is not a
stronger Serrin theorem for rough interior levels.
