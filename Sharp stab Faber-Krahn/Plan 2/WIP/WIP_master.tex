...
Brasco--De~Philippis--Velichkov~\cite{BDV} proof and from the
...
concentrated in the first five blocks.
\end{abstract}
...
sharp quantitative isoperimetric inequality~\cite{FMP}, extended
this to all dimensions with exponent~$4$ on $\Fr$.

The sharp conjecture with exponent~$2$ was settled in 2015 by
Brasco--De~Philippis--Velichkov~\cite{BDV}: there exists
$\sigma(n)>0$ with
...
regularity~\cite{AC1981} and a Fuglede-type
expansion~\cite{Fuglede} to produce a nearly spherical
contradiction. Subsequent work of Brasco--De~Philippis~\cite{BraDeP2017}
and Allen--Kriventsov--Neumayer~\cite{AKN} addresses related
...
imports the bounded reduction of \cite[\S5]{BDV} (equivalently
Plan~1's \textsc{BoundedReduction}) and the Kohler--Jobin transfer
of Brasco~\cite{Brasco2014} (Plan~1's \textsc{KohlerJobinTransfer}).
...
Theorem~\ref{thm:L} ($\Fr(\Om)\le\Fr(E_{\rhad})+2|\Om\setminus
E_{\rhad}|/\omega_n$) and the squaring identity
$(a+b)^2\le 2(a^2+b^2)$; sharp exponent~$2$.
\end{theorem}

\subsection{Block VII: \textsc{GlobalAssembly}}\label{ssec:FK}
...
\begin{proof}
Fix $\rstar=1/2$. The chain
Theorems~\ref{thm:L}--\ref{thm:BL} delivers the bounded
...
\textbf{Verdict.} All $R$- and $\rstar$-dependence is absorbed at
...
Sobolev--Sard (Figalli--Maggi App.~A) & not used & uses \\\hline
BDV bounded reduction \cite[\S5]{BDV} & uses & uses \\
...
\textbf{Wave~1} (\texttt{agent6-adversarial-audit.md}) flagged five
issues: centre-choice ambiguity (Fraenkel vs.\ centroid); reliance
...
\textbf{Wave~2} (\texttt{wave2-*}) addressed: the centroid choice
...
\textbf{Wave~3} (\texttt{wave3-F,G,H,J}) introduced two new
structural inputs: (F) the $H^1$-in-$\rho$ centroid bound
...
\bibitem{Plan1Master}
Plan~1 master document, \texttt{Final/master.tex}.
...
in \emph{Shape Optimization and Spectral Theory}, De Gruyter Open, 2017.
