...
\item $\rho_\delta:=1-\kappa\sqrt{\delta_T(\Om)}$ the deficit endpoint;
$\rhad:=\rho_\delta$ the metric-trace radius (upper endpoint of the
order-one window $J^*=[\rho_\delta-L^*,\rho_\delta]$ where the
uniform pointwise bound $d_\X(F_\rho,B_1)^2\le C_*\delta_T$ holds).
...
\begin{theorem}[Weighted metric trace at $\rho_\delta$;
\cite{WeightedMetricTrace}]\label{thm:K}
Conditional on \eqref{eq:Fboxed},
$\int(d_\X(F_\rho,B_1)^2+|\dot F_\rho|_\X^2)d\mu\le C\,\delta_T$
(integrated action, encoding $\int D_I\,d\mu$ via Fusco--Julin and
$\int D_H\,d\mu$ via the metric first variation),
$\int|\dot F_\rho|_\X^2\,d\rho\le C'\delta_T$ (kinetic bound).
Combining the two on the order-one window
$J^*=[\rho_\delta-L^*,\rho_\delta]$ with $L^*=(\rho_\delta-\rstar)/4$
via Markov on the distance term plus kinetic Cauchy--Schwarz
transport gives the uniform pointwise bound
...
$d_\X(F_{\rho_\delta},B_1)^2\le C_*\,\delta_T(\Om)$ at the upper
endpoint of the deficit window. Either input alone forces a
$\sqrt{\delta_T}$ exponent loss; the $D_H/D_I$ pairing yields the
sharp bound at $\rho_\delta$.
\end{theorem}
...
\;}
\end{equation}
The proof combines the endpoint case $\rhad=\rho_\delta$ of
\eqref{eq:Kboxed} with Lemma~8.3 of \cite{BoundaryLayerTransfer}
($\Fr(\Om)\le\Fr(E_{\rho_\delta})+2|\Om\setminus E_{\rho_\delta}|/\omega_n$,
where $|\Om\setminus E_{\rho_\delta}|\le \omega_n n\kappa\sqrt{\delta_T}$
since $\rho_\delta=1-\kappa\sqrt{\delta_T}$) and the squaring identity
