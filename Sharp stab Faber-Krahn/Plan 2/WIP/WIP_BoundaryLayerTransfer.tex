...
trace input} by transferring the metric-trace estimate at the inner
level $\rhad$ back to the original set $\Om$.
The upstream weighted metric trace (\cite{WeightedMetricTrace}) supplies a radius
...
and the constants of \cite{WeightedMetricTrace}. The exponent $2$ is sharp.
...
The admissible class of $\Om$ here is the output of the bounded
reduction \cite{BDV-BoundedReduction-Plan1}, which produces
$\Om\subset B_{R_*(n)}$ with $|\Om|=\omega_n$.

\begin{theorem}[Metric trace at $\rhad$; upstream input from \cite{WeightedMetricTrace}]
...
\paragraph{Reference.}
This is the conclusion of \cite[Thm.~5.6]{WeightedMetricTrace},
...
$C_n:=\omega_n n(\kappa+C_*)$. Equivalently, write $C'_n:=n(\kappa+C_*)$
so that $C_n=\omega_n C'_n$ and the prefactor in
\eqref{eq:transfer-rhat} simplifies to $2C'_n$ without
double-cancellation of $\omega_n$. Choosing $\delta_2:=\min\{1,\delta_1\}$
...
\end{equation}
where $C'_n=n(\kappa+C_*)$ (equivalently $C_n=\omega_n C'_n$) is the
constant of Lemma~\ref{lem:layer-vol}.
\end{corollary}
...
explicit constants of the repaired metric trace. Specialising to
$R=R_*(n)$ (the bounded reduction radius) and $\rstar=1/2$ as in
...
isoperimetric/Saint--Venant inequality \cite{FMP2008,Maggi2012} applied
