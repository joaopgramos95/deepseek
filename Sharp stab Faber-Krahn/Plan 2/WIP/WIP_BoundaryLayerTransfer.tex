...
\begin{abstract}
We close the route-$\delta$ chain by transferring the metric-trace
estimate at the inner level $\rhad$ back to the original set $\Om$.
The upstream weighted metric trace
(\cite{WeightedMetricTrace}) supplies a radius
$\rhad\in[\rho_\delta-C_*\delta_T,\rho_\delta]$, with
$\rho_\delta=1-\kappa\sqrt{\delta_T}$, such that
$d_{\X}(F_{\rhad},B_1)^2\le C_*\,\delta_T(\Om)$. The boundary-layer
volume $|\Om\setminus E_{\rhad}|=\omega_n(1-\rhad^{\,n})$ is
$O(\sqrt{\delta_T})$; the elementary superlevel-transfer estimate
of \cite{LevelSetIdentity} (Lemma~8.3) gives
$\Fr(\Om)\le \Fr(E_{\rhad})+C_n\sqrt{\delta_T}$. Squaring with
$(a+b)^2\le 2a^2+2b^2$ doubles the constant on $\delta_T$ but
...
the inequality is trivial after enlarging $C$ via the FMP
quantitative isoperimetric inequality applied to $\Om$. Every
constant is polynomial in $1/\rstar$, $R$, $n$ and the constants of
the upstream building blocks.
\end{abstract}
...
\end{equation}
The constant $C(n,R,\rstar)$ is polynomial in $1/\rstar$, $R$, $n$
and the constants of \cite{CentroidBound},
\cite{SpaceTimeIdentity}, \cite{WeightedMetricTrace}. The exponent
$2$ is sharp.
\end{theorem}
...
metric trace lemma assembled in \cite{WeightedMetricTrace} from
\cite{SpaceTimeIdentity} and \cite{CentroidBound}. Set
...
in \cite[\S5]{SpaceTimeIdentity}; equivalently
...
\begin{theorem}[Metric trace at $\rhad$; \cite{WeightedMetricTrace}]
...
$\delta_T(\Om)\le\delta_1$, there exists
\begin{equation}\label{eq:rhad-window}
\rhad=\rhad(\Om)\in[\rho_\delta-C_*\delta_T(\Om),\;\rho_\delta]
\end{equation}
such that
...
This is the conclusion of \cite[Theorem on weighted metric trace]{WeightedMetricTrace},
combining the integrated metric-derivative bound
\cite[(4.2)]{WeightedMetricTrace} with the profile-gap measure
\cite[(5.1)]{LevelSetIdentity} and the
metric Sobolev trace lemma \cite[Lemma~3.1]{WeightedMetricTrace}.
...
Let $\rhad$ be as in Theorem~\ref{thm:metric-trace}. There is
$C_n>0$ depending only on $n$ such that, for
$\delta_T(\Om)\le\delta_2(n,\rstar)$ small,
\begin{equation}\label{eq:layer-vol}
|\Om\setminus E_{\rhad}|=\omega_n\bigl(1-\rhad^{\,n}\bigr)
\le C_n\sqrt{\delta_T(\Om)}.
\end{equation}
\end{lemma}

\begin{proof}
From \eqref{eq:rho-delta}--\eqref{eq:rhad-window},
\begin{equation}\label{eq:rhad-lower}
\rhad\ge \rho_\delta-C_*\delta_T(\Om)=1-\kappa\sqrt{\delta_T(\Om)}-C_*\delta_T(\Om).
\end{equation}
The map $\rho\mapsto 1-\rho^n$ is $C^1$ with
$\frac{d}{d\rho}(1-\rho^n)=-n\rho^{n-1}$, so by the mean-value theorem
applied on $[\rhad,1]\subset[\rstar,1]$,
\[
1-\rhad^{\,n}\le n(1-\rhad)\le n\bigl(\kappa\sqrt{\delta_T(\Om)}+C_*\delta_T(\Om)\bigr).
\]
For $\delta_T(\Om)\le 1$, the second summand is bounded by
$C_*\sqrt{\delta_T(\Om)}$, giving \eqref{eq:layer-vol} with
$C_n:=\omega_n n(\kappa+C_*)$. Choosing $\delta_2:=\min\{1,\delta_1\}$
ensures both \eqref{eq:rhad-window} and $\delta_T(\Om)\le 1$.
\end{proof}
...
inequality for symmetric differences gives, since
$z+B_{r_s}\subset z+B_1$ (because $r_s\le 1$),
...
Cicalese--Leonardi $L^1$-rearrangement bookkeeping \cite{Maggi2012}.
\end{remark}
...
\Fr(\Om)\le \Fr(E_{\rhad})+\frac{2C_n}{\omega_n}\sqrt{\delta_T(\Om)},
\end{equation}
where $C_n$ is the constant of Lemma~\ref{lem:layer-vol}.
\end{corollary}

\begin{proof}
$E_{\rhad}\subset\Om$ by definition, and
$|\Om\setminus E_{\rhad}|\le C_n\sqrt{\delta_T(\Om)}$ by
...
$a:=\Fr(E_{\rhad})$, $b:=\frac{2C_n}{\omega_n}\sqrt{\delta_T(\Om)}$.
Then \eqref{eq:transfer-rhat} reads $\Fr(\Om)\le a+b$. Squaring,
\begin{equation}\label{eq:square}
\Fr(\Om)^2\le 2a^2+2b^2
=2\Fr(E_{\rhad})^2+\frac{8C_n^2}{\omega_n^2}\delta_T(\Om).
\end{equation}
By \eqref{eq:Aerho},
$\Fr(E_{\rhad})^2\le \omega_n^{-2}C_*\delta_T(\Om)$. Substituting,
\begin{equation}\label{eq:final-const}
\Fr(\Om)^2\le \frac{2C_*+8C_n^2}{\omega_n^2}\,\delta_T(\Om)
=:C_3\,\delta_T(\Om).
\end{equation}
This proves \eqref{eq:small-delta} with $\delta_0:=\delta_2$. The
...
\cite[Theorem on weighted metric trace]{WeightedMetricTrace}. By the
analysis in \cite[\S7]{SpaceTimeIdentity}, $C_*$ is polynomial in
$(1/\rstar, R, n)$ and linear in the centroid-bound constant
$C_F=C_F(n,R,\rstar)$ of \cite{CentroidBound}.

\item $C_n=\omega_n n(\kappa+C_*)$ from Lemma~\ref{lem:layer-vol},
where $\kappa=\kappa(n,\rstar)$ is the profile-gap constant of
\cite[\S5]{SpaceTimeIdentity}.

\item $\omega_n=|B_1|$ is purely dimensional.
\end{itemize}

In particular, $C_3$ is polynomial in
$(1/\rstar, R, n, C_F)$ and \emph{tracks linearly} in the new structural
input $C_F$ from the centroid bound. Specialising to $R=R_*(n)$ (the
bounded reduction radius) and $\rstar=1/2$ as in
\cite{WeightedMetricTrace}, $C_3$ depends only on $n$.
...
oscillation-index input to the homothetic velocity defect
\cite{SpaceTimeIdentity}. None of these can be improved beyond
...
\begin{remark}[Alternative via FMP]\label{rem:FMP}
A second proof of \eqref{eq:large-delta} uses the FMP quantitative
isoperimetric/Saint--Venant inequality \cite{FMP2010,Maggi2012} applied
to $\Om$ itself: $\Fr(\Om)^4\le C_{\rm FMP}\delta_T(\Om)$, which on
$\delta_T(\Om)\ge\delta_0$ implies a quadratic bound with constant
$C'_{\rm FMP}/\delta_0^{1/2}$. The bound via $\Fr<2$ is sharper for
our purposes.
\end{remark}
...
of \cite[Introduction]{BDV}: along the family of
...
Theorem~\ref{thm:main} is precisely the input to the bounded
reduction step of \cite[Theorem~5.1]{BDV-BoundedReduction-Plan1},
specialised to $R=R_*(n)$. Together with the Kohler--Jobin transfer
of \cite[Theorem~6.1]{KohlerJobinTransfer-Plan1}, it yields the
...
variation, \cite{CentroidBound} the H$^1$-in-$\rho$ centroid bound,
\cite{SpaceTimeIdentity} the integrated profile-gap (B$^2$-int)
inequality, and \cite{WeightedMetricTrace} the metric trace at
...
H$^1$-in-$\rho$ Fraenkel-centroid bound (the new structural input);
constant $C_F=C_F(n,R,\rstar)$.

\bibitem{SpaceTimeIdentity}
Plan~2 building block \textsc{SpaceTimeIdentity}.
Space-time $\Pi$ identity; integrated (B$^2$-int) inequality via
slicing-then-squaring; profile-gap measure constant $\kappa$.

\bibitem{WeightedMetricTrace}
Plan~2 building block \textsc{WeightedMetricTrace}.
Integrated action $\to$ pointwise endpoint at $\rhad$; metric trace
estimate $d_{\X}(F_{\rhad},B_1)^2\le C_*\delta_T$ with
$\rhad\in[\rho_\delta-C_*\delta_T,\rho_\delta]$.
