...
gives $\Fr(\Om)\le \Fr(E_{\rho_\delta})+C_n\sqrt{\delta_T}$. Squaring
...
\begin{remark}[Conditionality through Hyp-G]\label{rem:hypG}
The constant $C_*=C_*(n,R,\rstar)$ inherited from
\cite[Thm.~1.3]{WeightedMetricTrace} depends polynomially
on the ratio $(M_0/m_0)^2$ of the standing gradient hypothesis
Hyp-G of \cite[\S 7]{LevelSetIdentity}.  This dependence is purely
through the metric-derivative bound; the boundary-layer transfer
arguments of the present block do not require Hyp-G directly.  In
particular, the residual conditionality of the sharp Faber--Krahn
chain is concentrated at Hyp-G in \cite{LevelSetIdentity} and does
not enlarge in this block.
\end{remark}
...
\]
This proves \eqref{eq:layer-vol} with $C_n:=\omega_n n\kappa$.
Equivalently, $C'_n:=n\kappa$ so that $C_n=\omega_n C'_n$ and the
prefactor in \eqref{eq:transfer-rhat} simplifies to $2C'_n$ without
double-cancellation of $\omega_n$.  Choosing $\delta_2:=\delta_1$
ensures \eqref{eq:rhad-window} applies.
\end{proof}
...
$|\Om\setminus E_{\rho_\delta}|\le C_n\sqrt{\delta_T(\Om)}$ by
...
\item $C_n=\omega_n n\kappa$ from Lemma~\ref{lem:layer-vol},
