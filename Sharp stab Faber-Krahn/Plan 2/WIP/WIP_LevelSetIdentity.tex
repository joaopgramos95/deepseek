...
\]
defined for a.e.\ $t$ (see Lemma~\ref{lem:coarea}--\ref{lem:flux}), and
...
$u^{*}$ is bounded and Borel, hence in $L^{1}(0,|\Om|)$. By
...
The companion note `level-set-deficit-identity.md' wrote the second
...
\]
The same computation for $u_{B}$ on $\Om=B$ (where equality holds
throughout because $D_{H},D_{I}\equiv 0$ on $B$) gives
$\Gamma(B)=0$, i.e.\
...
the integrated $D_{H}+D_{I}$. The centroid bound
\cite{CentroidBound} pins the natural centre to the centroid of
$u$. The space--time identity \cite{SpaceTimeIdentity} squares the
slicing $T_{1}=T_{1}^{\rm rad,slic}+T_{1}^{\rm tan}$ to convert the
integrated defect into the (B$^{2}$-int) bound. The weighted metric
trace \cite{WeightedMetricTrace} promotes integrated action to a
...
Kohler--Jobin transfer to the Faber--Krahn deficit.
...
\textbf{27} (1975) (BV chain rule for monotone AC inverses).
...
block \textsc{BoundaryLayerTransfer}.
