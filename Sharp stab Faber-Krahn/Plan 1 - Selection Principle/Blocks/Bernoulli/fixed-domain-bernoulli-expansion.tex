...
\item Route A graph entry now uses uniform Schauder bounds plus interpolation
from the small \(L^\infty\) graph norm to enter the BDV nearly spherical
\(C^{2,\gamma_0}\) regime. For Route B, the remaining work is the explicit
constant bookkeeping in the bilinear source enumeration.
\end{itemize}
