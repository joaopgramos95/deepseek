...
\item[(3)] Graph entry into the small $C^{2,\gamma_0}$ regime:
\textbf{written out} via uniform Schauder bounds and interpolation in
...
into the implicit constants of the spectral closure theorem in
...
\section{Uniform Schauder bootstrap and small $C^{2,\gamma_0}$ entry}\label{sec:bootstrap}
...
Theorem~6.2) and BDV's nearly spherical theorem need a small
$C^{2,\gamma_0}$ graph norm. The Schauder step gives uniform high regularity,
not small high regularity by itself; smallness comes from interpolating that
uniform high norm with the small $L^\infty$ graph size produced by Hausdorff
closeness.

\begin{proposition}[Uniform Schauder bootstrap]\label{prop:schauder}
For a BDV selected minimizer with $\norm{g}_{C^{1,\gamma}}\le\mu$ small,
and for every integer $m\ge3$,
\[
\norm{g}_{C^{m,\gamma}(\partial B_1)}
\le M_m(N,R),
\]
where $q=\abs{\nabla u}\big|_{\partial U}$ is the smooth Bernoulli coefficient
from the selected Euler--Lagrange law, with BDV Lemma~4.16 supplying the
starting uniform bounds.
\end{proposition}
...
\]
Reading off the boundary trace gives a uniform $C^{2,\gamma}$ bound for
$g$. Since the selected Bernoulli law has an explicit smooth right-hand side
on the fixed collar, and BDV Lemma~4.16 supplies the starting uniform
coefficient bounds in the flat regime, iterating the same Schauder estimate
gives $\norm{g}_{C^{m,\gamma}}\le M_m(N,R)$ for every fixed $m$.
\end{proof}

\begin{lemma}[Interpolation to small $C^{2,\gamma_0}$]\label{lem:interp}
Fix $0<\gamma_0<\gamma$ and $m\ge3$. If
\[
\norm{g}_{L^\infty(\partial B_1)}\le h,
\qquad
...
\le C h^{1-\theta}M_m(N,R)^\theta
\]
for some $\theta=\theta(N,m,\gamma_0)\in(0,1)$. Hence the required
$C^{2,\gamma_0}$ smallness follows by strengthening the graph-entry threshold
so that $h$ is sufficiently small.
\end{lemma}

\begin{proof}
This is the standard interpolation inequality between $C^0$ and
$C^{m,\gamma}$ on the compact manifold $\partial B_1$. In the graph-entry
setting, $h\le C d_H(\partial U,\partial B_1)\le
C\alpha(U)^{1/(2N)}$, so the smallness is enforced by the explicit
asymmetry threshold.
\end{proof}

\begin{remark}
The previous wording incorrectly suggested that one can make
$\norm{g}_{C^{2,\gamma}}$ small merely by choosing $\mu$ small, despite the
fixed $L_q(N,R)$ term in the Schauder bound. The corrected mechanism is:
uniform Schauder bounds plus $L^\infty$ smallness plus interpolation.
References:
