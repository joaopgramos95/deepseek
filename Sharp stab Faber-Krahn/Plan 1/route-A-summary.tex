...
\medskip
\noindent
\textbf{Scope:} this note documents Route A (using BDV Theorem~5.1 as the
final closure). Route B (replacing BDV Theorem~5.1 by the Bernoulli
...
\[
\alpha(\Omega):=\inf_{x_0\in\R^N}\int_\Omega\bigl(\abs{x-x_0}-r_\Omega\bigr)\d x,
\quad r_\Omega:=(\abs{\Omega}/\omega_N)^{1/N}.
\]
(BDV Lemma 2.5.) BDV Lemma~4.2 gives $\mathcal A(\Omega)^2\le C_{\rm Fr}(N)\alpha(\Omega)$.
...
Lipschitz, nondegeneracy, and smooth Bernoulli coefficient bounds, with
constants depending only on $(N,R)$. Specifically:
...
The closing step is:
\begin{quote}
\textbf{BDV Theorem 5.1} (nearly spherical). There exist $c_{\rm sph}(N)>0$
and $\delta_{\rm sph}(N)>0$ such that for every set
$U=\{(1+g(\theta))\theta:\theta\in\partial B_1\}$ with
$\abs{U}=\abs{B_1}$, $\hbox{barycenter}(U)=0$, and
$\norm{g}_{C^{2,\gamma}(\partial B_1)}\le\delta_{\rm sph}$,
\[
...
:=E(U)+f_{\hat\eta}(\abs U-\abs{B_1})
+\sqrt{\varepsilon^2+\tau^2(\alpha(U)-\varepsilon)^2},
\]
$U\subset B_R$, where $f_{\hat\eta}(s)$ is the BDV symmetric volume
penalization
\[
f_{\hat\eta}(s):=\begin{cases}-\hat\eta s & s\ge0,\\ -\hat\eta_0 s & s<0,\end{cases}
\]
with $\hat\eta,\hat\eta_0$ chosen large enough that minimizers of
$J_{\tau,\hat\eta,\Omega_0}$ have unit volume.
\end{definition}
...
Existence: $J_{\tau,\hat\eta,\Omega_0}$ is lower semicontinuous in
Hausdorff convergence on $B_R$ (compactness of $\{U\subset B_R\}$ + BDV
density estimates) and bounded below, so admits a minimizer.
...
BDV Theorem~5.1 (nearly spherical) needs a $C^{2,\gamma}$ graph; graph entry
supplies only $C^{1,\gamma}$. The bootstrap is via the hodograph transform
applied to the torsion equation with the smooth Bernoulli condition.

\begin{proposition}[Bootstrap, \texttt{corrections-response.tex} Prop.~2.1]\label{prop:bootstrap}
Let $\partial\widetilde\Omega=\{(1+g)\theta\}$ be a $C^{1,\gamma}$ graph with
$\norm{g}_{C^{1,\gamma}}\le C\mu$, and let $\widetilde u$ be its torsion
potential. Assume BDV Lemma~4.16: the Bernoulli coefficient
$q(y)=\abs{\nabla\widetilde u(y)}$ on $\partial\widetilde\Omega$ satisfies
$q\in C^{1,\gamma}(\partial\widetilde\Omega)$ with
$\norm{q}_{C^{1,\gamma}}\le L_q(N,R)$.
Then $g\in C^{2,\gamma}(\partial B_1)$ with
\[
\norm{g}_{C^{2,\gamma}(\partial B_1)}\le C_{\rm boot}(N,R)\,
(\norm{g}_{C^{1,\gamma}}+L_q(N,R))
\le C'_{\rm boot}(N,R).
\]
\end{proposition}

\begin{proof}
Apply the hodograph transform $V(\rho,\theta)=\widetilde u(\rho\theta)$ in
the collar $\{(1-\eta)\le\rho\le 1+g(\theta)\}$, with $\eta=O(\mu)$ small.
In $(\rho,\theta)$ coordinates, $\widetilde u$ satisfies a uniformly
elliptic equation
\[
-a^{ij}(\rho,\theta;\nabla V)\partial_{ij}V=b(\rho,\theta;\nabla V)
\]
with $C^\gamma$ coefficients, and the free boundary $\partial\widetilde\Omega$
becomes the fixed hyperplane $\rho=1+g(\theta)$ in the parameter space.

The Bernoulli condition transforms to an oblique condition
$\Phi(\nabla V,\rho,\theta)=q(\theta)$ on $\rho=1+g(\theta)$, with $\Phi$
smooth.

By Kinderlehrer--Nirenberg (\emph{Ann.\ Sc.\ Norm.\ Sup.\ Pisa Cl.\ Sci.}\ 1977,
\emph{Regularity in free boundary problems}, Thm.~2.1) and Lieberman
(\emph{Adv.\ Math.}\ 1985, \emph{The Perron process for oblique
derivative problems}, Thm.~3.2):
\[
\norm{V}_{C^{2,\gamma}(\hbox{collar})}\le C(\norm{V}_{C^{1,\gamma}}+\norm{q}_{C^{1,\gamma}}).
\]
...
\begin{remark}
For $\mu$ small, $C'_{\rm boot}(N,R)\le\delta_{\rm sph}(N)$, the smallness
threshold of BDV Theorem~5.1. This is achieved by decreasing $\mu(N,R)$, with
the consequent change to $\alpha_{\rm graph}$ tracked through
Theorem~\ref{thm:graphentry}.
\end{remark}
...
\textbf{(BDV Theorem 5.1.)} There exist $c_{\rm sph}(N)>0$ and
$\delta_{\rm sph}(N)>0$ such that for every set $U$ of the form
...
\item $\norm{g}_{C^{2,\gamma}(\partial B_1)}\le\delta_{\rm sph}(N)$,
\end{itemize}
we have
\[
E(U)-E(B_1)\ge c_{\rm sph}(N)\norm{g}_{H^{1/2}(\partial B_1)}^2.
...
Proposition~\ref{prop:bootstrap}, and Corollary~\ref{cor:Qlower} into a
direct lower bound on $Q_\alpha(\Omega_0)$.
...
$\alpha(\Omega_0)\le 2\alpha_{\rm graph}(N,R)$, and
$Q_\alpha(\Omega_0)<q_{\rm sel}(N,R)$,
\[
Q_\alpha(\Omega_0)\ge q_*(N,R)=\frac{c_*(N)}{C_{\rm sel}(N,R)},
\]
or equivalently
\[
...
\emph{Graph entry.} By hypothesis $\alpha(\Omega_0)\le 2\alpha_{\rm graph}$,
so~\eqref{eq:sel} gives $\alpha(\widetilde\Omega)\ge\alpha(\Omega_0)/2$ but
also $\alpha(\widetilde\Omega)\le\alpha(\Omega_0)\le 2\alpha_{\rm graph}$.
Hmm: we need $\alpha(\widetilde\Omega)\le\alpha_{\rm graph}$.
Strengthen the hypothesis to $\alpha(\Omega_0)\le\alpha_{\rm graph}$
(redefine $\alpha_*:=\alpha_{\rm graph}$ for the theorem's regime); then
$\alpha(\widetilde\Omega)\le\alpha(\Omega_0)\le\alpha_{\rm graph}$.

By Theorem~\ref{thm:graphentry},
$\partial\widetilde\Omega=\{(1+g)\theta\}$ is a $C^{1,\gamma}$ graph with
$\norm{g}_{C^{1,\gamma}}\le C\mu$. By Proposition~\ref{prop:bootstrap},
$\norm{g}_{C^{2,\gamma}}\le\delta_{\rm sph}(N)$ for $\mu(N,R)$ small.
...
\begin{proof}
Small asymmetry, $\mathcal A(\Omega)^2\le C_{\rm Fr}\alpha_{\rm graph}$:
combine Theorem~\ref{thm:sv-sharp} with $\mathcal A^2\le C_{\rm Fr}\alpha$.

Large asymmetry, $\mathcal A(\Omega)^2>C_{\rm Fr}\alpha_{\rm graph}$:
direct compactness, see \S\ref{sec:far}.

Take $c_{\rm SV}:=\min$ of the two constants.
\end{proof}

\subsection{The far-from-ball compactness constant}\label{sec:far}
...
$\mathcal A(\Omega)\ge\mathcal A_{**}:=(C_{\rm Fr}(N)\alpha_{\rm graph}(N,R))^{1/2}$,
\[
...
\begin{proof}
By Saint--Venant, $\Omega\mapsto E(\Omega)-E(B_1)$ is strictly positive away
from the ball, continuous in Hausdorff/$L^1$ convergence on
$\{\Omega\subset B_R:\abs{\Omega}=\abs{B_1}\}$, and the constraint set is
compact in Hausdorff topology. The infimum over
$\{\mathcal A(\Omega)\ge\mathcal A_{**}\}$ is attained at some $\Omega_\infty$
with $\mathcal A(\Omega_\infty)\ge\mathcal A_{**}>0$, hence not a ball.
Therefore the infimum is strictly positive.
\end{proof}
...
$\delta_{\rm SV}=-E(\Omega)+E(B_1)\ge c_{\rm SV}(N,R)\mathcal A(\Omega)^2/2$
(where $-2E=T$). By Lemma~\ref{lem:transferlin}, for $\delta_{\rm SV}$ small,
\[
\lambda_1(\Omega)-\lambda_1(B^*)\ge\frac{2L_N}{(N+2)T_N}\,\delta_{\rm SV}
\ge\frac{L_N\,c_{\rm SV}(N,R)}{(N+2)T_N}\,\mathcal A(\Omega)^2.
\]

\emph{Large asymmetry.} For $\mathcal A(\Omega)>\mathcal A_*$, by the
analogue of Lemma~\ref{lem:far} for $\lambda_1$:
$\lambda_1(\Omega)-\lambda_1(B^*)\ge c_{\rm far,FK}(N,R)\ge c_{\rm far,FK}\mathcal A(\Omega)^2/4$
(using $\mathcal A(\Omega)\le 2$).

Take
\[
c_{\rm FK}(N,R):=\min\Bigl(\frac{L_N\,c_{\rm SV}(N,R)}{(N+2)T_N},\;\frac{c_{\rm far,FK}(N,R)}{4}\Bigr).
\]
\end{proof}
...
\item Nearly spherical: $c_{\rm sph}(N), \delta_{\rm sph}(N)$ from BDV
Theorem~5.1.
...
\item Faber--Krahn: $c_{\rm FK}(N,R)=\min(L_Nc_{\rm SV}/((N+2)T_N),c_{\rm far,FK}/4)$.
\end{enumerate}
...
compactness-derived constants are $c_{\rm far,SV}$ and $c_{\rm far,FK}$, which
are far from the ball and do not enter the small-asymmetry regime.
\end{remark}
...
``Given any $\Omega_0$ with $\alpha(\Omega_0)\le\alpha_{\rm graph}$:
\begin{itemize}
\item Construct $\widetilde\Omega$ in one step (Thm.~\ref{thm:selection}).
\item Note $\widetilde\Omega$ is a $C^{2,\gamma}$ graph
(Thms.~\ref{thm:graphentry}, Prop.~\ref{prop:bootstrap}).
\item Apply BDV Theorem 5.1 pointwise to read off
...
\item Prop.~\ref{prop:bootstrap} ($C^{1,\gamma}\to C^{2,\gamma}$ bootstrap):
proof via Kinderlehrer--Nirenberg in \texttt{corrections-response.tex} \S 2.
\item Theorem~\ref{thm:nearlysph}: BDV Theorem 5.1, used as black box.
...
coefficient), 4.18 (Alt--Caffarelli flatness improvement), 5.1 (nearly
spherical), 5.2 ($\alpha$ vs $L^2$). All cited by theorem number and used
...
\ref{lem:transferpt}, \ref{lem:transferlin}, far-from-ball compactness
Lemma~\ref{lem:far}), or
\item quoted from BDV by theorem number (Thms.~4.9, 4.12, 4.16, 4.18,
5.1, 5.2 and Lemma 4.2).
\end{itemize}
