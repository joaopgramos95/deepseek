...
\title{Bernoulli Spectral Closure for Selected Minimizers}
\author{}
\date{}
...
\section{Why Boundary-Deficit Propagation Is Too Strong}
...
\section{Linearized Torsion Data at the Ball}
...
:=\sum_{k\ge0}(k-1)g_k.
\]
The kernel consists exactly of the \(k=1\) translation modes, while the volume
constraint removes the \(k=0\) mode.

\section{Linearization of the Selected Bernoulli Law}
...
\section{Nonlinear Estimates Needed for a Proof}
...
\paragraph{Lemma 5.1: Bernoulli map expansion.}
...
\paragraph{Lemma 5.2: \(\alpha\)-variation bracket expansion.}
Let
\[
\Psi_g(\theta)
=
...
\section{Consequence for the Selection Contradiction}
...
\begin{enumerate}
\item the Bernoulli coefficient is not arbitrary; it is an \(O(\sigma)\)
perturbation of a constant plus the controlled \(\alpha\)-variation;
\item the linearized Bernoulli map has a spectral gap on all modes except
volume and translations.
\end{enumerate}

\section{What Remains to Make This Fully Publication-Ready}

The argument above is now reduced to routine, but still nontrivial, estimates:
\begin{enumerate}
\item upgrade graph entry to a uniform \(C^{2,\gamma}\) graph. BDV Lemma 4.16
gives smooth \(q_u\), and Theorem 4.18 plus Schauder bootstrapping supplies
this once the \(C^{1,\gamma}\) graph is obtained;
\item write Lemmas 5.1 and 5.2 in full detail with a fixed pullback map and
explicit dependence on the \(C^{2,\gamma}\) radius;
\item track the small volume-normalization error from \(U_*\) to the
