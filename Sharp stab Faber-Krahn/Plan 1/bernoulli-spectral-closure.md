...
This gives a direct spectral closure of the selected-minimizer contradiction.
...
So, in the small-graph regime and for \(\sigma\le\sigma_*\), the only selected
minimizer satisfying the Euler-Lagrange equation is the ball.
...
The selection principle produces a selected minimizer \(U\) whose asymmetry
...
once \(\alpha(U)\le\alpha_{\rm graph}\). The spectral closure above then says
that if the selection parameter \(\sigma\) is small enough, \(U\) must be the
ball. This contradicts \(\alpha(U)>0\).
...
## 7. What remains to make this fully publication-ready

The argument above is now reduced to routine, but still nontrivial, estimates:
...
No external Serrin stability theorem is needed. The gap is closed by the
selected Bernoulli law plus the Steklov spectral gap.
