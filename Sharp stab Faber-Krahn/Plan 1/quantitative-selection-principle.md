...
This note implements the concrete selection-principle step from `plan1.md`.
The purpose is not to replace the whole Brasco--De Philippis--Velichkov proof,
but to isolate the part that can be made quantitative without changing their
free-boundary machinery.

The main output is a single-set version of BDV Proposition 4.4: from one
near-counterexample, construct one selected quasi-open minimizer whose
...
BDV Lemma 4.5 says that, for \(\hat\eta=\hat\eta(N,R)\), \(B_1\) is a minimizer
of \(F_{\hat\eta}\) among sets in \(B_R\).
...
- \(C_2=C_2(R)\), the Lipschitz constant of \(\alpha\) on subsets of \(B_R\)
  from Lemma 4.2(ii).
- \(\sigma_2(N,R)\) from Lemma 4.9, used for nondegeneracy and density.
- \(\sigma_3(N,R)\) from Lemma 4.16, used to obtain smooth, uniformly
  bounded Bernoulli coefficients.

It is useful to separate two thresholds:
...
is enough for existence of the minimizer and the uniform perimeter bound in
the proof of BDV Lemma 4.6, while
...
is the threshold used when the Alt--Caffarelli regularity package is needed.
All constants below are allowed to depend on \(N,R\), and once
\(\tau\le\tau_{\rm reg}\) the free-boundary constants are uniform.
...
### Proposition 4. Single-set selection map preserving \(Q_\alpha\)
...
This is the precise single-set replacement for the limiting estimate
BDV obtain in Proposition 4.4(iv). The smallness of
\(\varepsilon_{\rm sel}\) is only used after selection, to keep the dilation
inside a fixed bounded class and to make the later Hausdorff/flatness
threshold available; the quotient preservation itself is governed by
\(\rho(q,\tau)\).
...
fixed fraction of \(B_d(y)\) lies in \(U\Delta B_1\), so again \(d^N\lesssim M\).
...
\(N\). Fix \(0<\mu\le\bar\mu/16\), and choose
\(\rho_\mu>0\) so that
