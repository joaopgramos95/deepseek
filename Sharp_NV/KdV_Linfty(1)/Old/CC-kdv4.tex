...
\item For the Frequency restricted estimates, should agree on notation in \eqref{eq:multilinear_geral}
 and need to introduce all the relevant parameters $b, b', \wt b , \wt b '$. This is because estimate for $2<p<\infty$, need to be done for fixed time object, without Duhamel integral
...
\andreia{Should we still have a short section on notation before going into normal form ?}
...
\andreia{Add general description of what happens in the other subsections}
	...
	
	{
		...
		\begin{itemize}
			...
			\item $L^{s,p}$, if we decide to use this space
...
\begin{split}
\Ical (\TT,m; v)
= \dt \Ical \Big(\TT,\frac{m}{\Psi(\TT)} ; v \Big)
...
& = \dt\bigg[ \int_{\G(\TT)} e^{it \Psi(\TT)} \frac{m(\vec\xi)}{\Psi(\TT)}  \prod_{\bul\in \TT^\infty} \ft v_\bul (t) d\vec\xi  \bigg]
\\
...
\begin{align}
\Ical(\TT, m)(t)
& = \dt \Ical\Big( \TT, \frac{m}{\Psi(\TT)} ;v \Big)(t)\\
...
=
 \Ft_x[ \NN ( \<1>; v )] 
+ 
\dt\Ical \big( \<1>,  \ind_{A^c} K; v \big) 
\\
&
\quad 
+
...
&
= 
\dt \Ical(\<21>, - \ind_{A^c \cap A_1^c} KK_1 ; v) 
+
\dt \Ical(\<22>, - \ind_{A^c \cap A_2^c} KK_2 ; v) 
\\
&
\quad 
...
\andreia{Already changed $N$ to $J$ in statement and proof (27/11)}
\begin{proposition}
\label{PRO:NFE}
...
\label{NFE1a}
\begin{split}
\Ft_x [\BB(\TT;v)]  & = \dt\Ical(\TT, \mulB(\TT); v),\\
...
The following result shows the convergence to zero of the error terms  $\MM(\TT; v)(t)$ in \eqref{NFE1}  as $J\to\infty$. 
...
\dt\Ical (\TT,\mulB(\TT) ; v_1,\dots,v_{j+1}).
\end{align*}
...
\andreia{Changed the $u$'s to $w$'s in statement and proof (27/11)}
...
\andreia{Continue this remark based on making changes to the proof}
...
\dt z
&
= \NN_1(\<1>;e^{t\partial_x^3}z) + \sum_{j=2}^\infty \sum_{\TT \in \Tr_j} \sum_{\l=2}^4  \NN_\l(\TT;e^{t\partial_x^3}z)
\\
&
=: 
...
\andreia{corrigir isto!!} To recover solutions for \eqref{kdv}, {\color{red} \bf given that the relevant maps are well-defined (see Lemma~\ref{})}, we are interested in solving \eqref{z1} with initial data satisfying
\begin{align}
\label{z2}
...
\begin{align*}
\wt{\BB}(\TT) [w ] & =
\sum_{ P\subseteq \TT^{0,f}  } \BB(\TT\setminus P ) \big[ f_{\bul} = w, \ \bul
...
\andreia{Changed proof to match notation in the statement (27/11)}

We note that there is a unique element on LHS of \eqref{wint0a} with homogeneity 2 in $w$, namely $\NN(\<1>; w,w)$.
Therefore, $\NN_1(\<1>;w) = \NN(\<1>;w)$ and $\mul_1(\<1>) = \mul(\<1>)$ as in \eqref{NFE1a}-\eqref{NFE0b}. 

Next, we consider the contributions on the LHS of \eqref{wint0a} with underlying tree structure of $\<21>$, $\<22>$, and $\<33>$.  
...
\subsection{Properties of the gauge transform}

\andreia{Move properties of the gauge to here }
...
$$\| u(t)\|_{\FL^{s,\infty}_x} = \|\wt{u}(t)\|_{\FL^{s,\infty}_x} \lesssim \|\jap{\xi}^s\ft {u}\|_{L^\infty_\xi L^1_\tau} \les \|u\|_{X^{s,b}_{\infty,1}}, 
			...
			\begin{align*}
                ...
                + \|\jb{\xi}^s (1-e^{-i(t-t')\xi^3})\ft{u}(t')\|_{L^{\infty}_\xi(K) } \\&\lesssim \|\wt{u}(t)-\wt{u}(t')\|_{L^{s,\infty}_\xi} + \sup_{\xi\in K}\big|1 -e^{-i (t-t')\xi^3}\big| \| u \|_{X^{s,b}_{\infty,1}} \to 0\quad \mbox{as }t\to t', 
			\end{align*}
     ...
     %                \les \bigg\| \jap{\xi}^s \int_{|\eta|T >1} \jap{\eta}^{\wt{b}} |\eta|^{-1} |\ft{G}(\eta, \xi) \ft{\psi}_T(\tau-\eta) | d\eta \bigg\|_{L^\infty_\xi L^1_\tau} \\
            ...
            Combining the estimates above, we obtain \eqref{lin2a}-\eqref{lin2b}. 
            \qedhere 
	...
	
	{
		...
		\begin{equation}\label{eq:multilinear_geral}
			\Ft_x (\mathcal{N}[u_1,\dots, u_k])(\xi)=\int_{\xi=\xi_1+\dots+\xi_k} m(\xi_1,\dots,\xi_k)\prod_{j=1}^k\ft{u}_j(\xi_j) d\xi_1\cdots d\xi_{k-1}.
		\end{equation}
...
$$
        ...
        $$
        \left\|\psi_\delta(t)\int_0^t e^{-(t-t')\partial_x^3}\Ncal[u_1,u_2](t')dt' \right\|_{X^{s,b}_{\infty, \infty} \cap X^{s,\tilde{b}}_{\infty,1}}  \lesssim (\delta^{1+b'-b} + \delta^{2+\tilde{b}'-b})\left\|\Ncal[u_1,u_2] \right\|_{X^{s,b'}_{\infty, \infty} \cap X^{s,\tilde{b}'}_{\infty,1}}.
        $$
...
Using \eqref{fre},
		...
		\begin{lemma}[{\cite[Lemma 3.1]{CLS}}]\label{lem:fre_bi}
			...
			\begin{equation}
				\left\|\psi_\delta(t)\int_0^te^{-(t-t')\partial_x^3} \Ncal[u_1,u_2](t')dt'\right\|_{X^{s,b}_{2}} \lesssim \delta^{0^+}\|u_1\|_{X^{s,b}_{2}} \|u_1\|_{X^{s,b}_{2}}.
			\end{equation}
		\end{lemma}
...
Then 
			...
			% Then
			\begin{align}
				\left\|\psi_\delta(t)\int_0^te^{-(t-t')\partial_x^3}\Ncal[u_1,\dots, u_k](s)ds \right\|_{X^{s,b}_{\infty, \infty}\cap X^{s,\tilde{b}}_{\infty, 1}} & \les \delta^{0^+}\prod_{j=1}^k\|u_j\|_{X^{s,b}_{\infty, \infty} \cap X^{s,\tilde{b}}_{\infty,1}}.
			\end{align}
...
$$
        ...
        \begin{align}
				\left\|\psi_\delta(t)\int_0^te^{-(t-t')\partial_x^3}\Ncal[u_1,\dots, u_k](s)ds \right\|_{X^{s,b}_{\infty, \infty}} & \les \delta^{0^+}\left\|\Ncal[u_1,\dots, u_k] \right\|_{X^{s,b'}_{\infty, \infty}\cap X^{s,\tilde{b}'}_{\infty, 1}}, \\
				\left\|\psi_\delta(t)\int_0^te^{-(t-t')\partial_x^3}\Ncal[u_1,\dots, u_k](s)ds \right\|_{X^{s,\wt{b}}_{\infty, 1}} & \les \delta^{0^+}\left\|\Ncal[u_1,\dots, u_k] \right\|_{X^{s,\tilde{b}'}_{\infty, 1}}.
			\end{align}
            As such, it suffices to prove that
\begin{align}
				\left\|\Ncal[u_1,\dots, u_k] \right\|_{X^{s,b'}_{\infty, \infty}} & \les \prod_{j=1}^k\|u_j\|_{X^{s,b}_{\infty, \infty}},\label{eq:infty_por_infty} \\
				\left\|\Ncal[u_1,\dots, u_k] \right\|_{X^{s,\wt{b}'}_{\infty, 1}}  & \les \prod_{j=1}^k\|u_j\|_{X^{s,b}_{\infty, \infty} \cap X^{s,\tilde{b}}_{\infty,1}}.\label{eq:1_por_infty}
			\end{align}
			We begin with \eqref{eq:infty_por_infty}. Observe that, since $b'<0$ and $\tilde{b}>0$,
			\begin{align*}
				&\|\Ncal[u_1,\dots, u_k] \|_{X^{s,b'}_{\infty, \infty}} \lesssim \|\Ncal[u_1,\dots, u_k] \|_{X^{s,0}_{\infty, \infty}} \\\lesssim \ &\sup_{\tau, \xi} \int \frac{|m|\jap{\xi}^s}{\prod_{j=1}^k\jap{\xi_j}^s} \frac{1}{\prod_{j=1}^\ell \jap{\tau_j-\xi_j^3}^b}\prod_{j=\ell+1}^k \jap{\xi_j}^s|\hat{u}_j| d\tau_2\dots d\tau_k d\xi_2 \dots d\xi_k\cdot \prod_{j=1}^\ell \|u_j\|_{X_{\infty,\infty}^{s,b}}
		...
		\begin{lemma}[{\cite[Lemma 2]{COS}}]\label{lem:fre_tri}
            ...
            Then, for $b=(1/2)^+$ and $0<\delta\ll 1$,
            \begin{align}
				\left\|\psi_\delta(t)\int_0^te^{-(t-t')\partial_x^3}\Ncal[u_1,\dots, u_k](s)ds \right\|_{X^{s,b}_{2}} & \les \delta^{0^+}\prod_{j=1}^k\|u_j\|_{X^{s,b}_{2}}.
			\end{align}
		\end{lemma}
...
\smallskip
\noi
\underline{\textbf{Case 1:} $|\xi_{11}| \gg |\xi_{21}|$}
...
\smallskip
\noi
\underline{\textbf{Case 3:} $|\xi_{11}| \sim |\xi_{21}|$ and $\big| |\xi_{12}| - |\xi_{22}| \big| \lor \big| |\xi_{11}| - |\xi_{21}| \big| \lor \big| |\xi_{11}| - |\xi_{22}| \big| \lor \big| |\xi_{12}| - |\xi_{21}| \big|\ll |\xi| $}
...
\begin{align}
    \Gc_t^{-1}[z] = z(t) + \DD[z,z](t), 
\end{align}
for $\DD$ as in \eqref{balba}.
% 
% 
We start by showing that $\Gc$ is a small perturbation of the identity on $B_\dl \subset L^\infty_T \FL^{s, p}(\R)$, for $\dl>0$ sufficiently small. In particular, this follows once we show that $\DD$ is a contraction on this set. 
...
\begin{align}
\|\DD[z_{1},z_{2}]\|_{L^\infty_T \mathcal{F}L^{s,\infty}}   
&
\les
...
\begin{align}
\|\DD[z_{1},z_{2}]\|_{L^\infty_T \mathcal{F}L^{s,2}}   
&
\les 
\sup_{\|z\|_{L^2} \le 1 }
...
\begin{equation}\label{eq:est_D}
		\|\DD[z_{1},z_{2}]\|_{L^\infty_T \mathcal{F}L^{s,p}}
        \les
C\|z_{1}\|_{L^\infty_T \mathcal{F}L^{s,p}}\|z_{2}\|_{L^\infty_T \mathcal{F}L^{s,p}}. 
\end{equation}
...
\end{align}
\andreia{The proof is ignoring the time dependence on the operator $\DD$, which adds and oscillatory factor which we need to take into account . Could just do $t=0$ which would remove this issue?
