...
Then, in Subsection~\ref{SUBSEC:lwp}, we prove well-posedness of \eqref{rkdv} (Theorem~\ref{thm:lwp_rkdv}) via a contraction mapping argument on the gauged normal form equation \eqref{z1}, which together with the equivalence between \eqref{z1} and \eqref{kdv} for smooth solutions (Lemma~\ref{lem:equivalencia}), gives well-posedness of the original equation \eqref{kdv} (Theorem~\ref{thm:wp_kdv_intro}).
...
\smallskip 
\noi {\rm(ii)}
Given $0<T\le 1$, {$s\gg1$ sufficiently large},
and $b,\wt{b}\in\R$ as in Theorem~\ref{thm:lwp_rkdv}, and $\dl>0$ sufficiently small,
the map  $\Gc^{-1}$ is a continuous bijection on open balls $B_\dl$ where 
\begin{itemize}
\item $B_\dl \subset  X^{s,b}_{p}(0,T)$ when $2 \le p < \infty$;

\item $B_\dl \subset  X^{s,b}_{\infty, \infty}(0,T) \cap X^{s, \wt{b}}_{\infty, 1}(0,T)$ when $p=\infty$.
\end{itemize}
Consequently, the gauge transform $\Gc$ is a continuous map on $B_\dl$. 


\smallskip
\noi {\rm(iii)} The map $\Gc^{-1}$ is unbounded on $\FL^{s,p}(\R)$ for $s\le - \frac12 - \frac1p$. 
...
\begin{proof}
To show (i)-(ii), considering $\Gc^{-1}$ in \eqref{gauge1}, we start by showing that $\Gc^{-1}$ is a small perturbation of the identity on open balls $B_\dl$ in the relevant space, with $\dl>0$ sufficiently small.
In particular, this follows once we show that $\DD$ is a contraction on this set. 


We first consider (i).
When $p=\infty$, recalling $A^c$ in \eqref{A}, we have
\begin{align}
\|\DD[z,z]\|_{\mathcal{F}L^{s,\infty}}   
&
\les
\sup_\xi \jb{\xi}^s \int_{A^c} 
\frac{|\widehat{z}(\xi_{1})||\widehat{z}(\xi_{2})|}{\jap{\xi_{1}}\jap{\xi_{2}}} d\xi_{1} 
\notag
\\
&\les
\bigg( \sup_{\xi} \int_{|\xi_{1}|>1} \frac{1}{\jap{\xi_{1}}^{2s+2 - s\lor 0 }} d\xi_{1} 
\bigg) 
\|z\|_{\mathcal{F}L^{s,\infty}}^2
\notag
\\
& \les
\|z\|_{ \mathcal{F}L^{s,\infty}}^2
...
\les 
\sup_{\|f\|_{L^2} \le 1 }
	 \int_{\xi, (\xi_1,\xi_2)\in A^c} \frac{\jap{\xi}^s|\widehat{z}(\xi_{1})||\widehat{z}(\xi_{2})||\widehat{f}(\xi)|}{\jap{\xi_{1}}\jap{\xi_{2}}} d\xi_{1}d\xi 
     \notag
    \\
    & \les \sup_{\|f\|_{L^2 } \le 1}  \bigg( \sup_{\xi_{1}}  \int \ind_{A^c} \frac{\jap{\xi}^{2s}}{\jap{\xi_{1}}^{4s+4}}d\xi \bigg)^{\frac{1}{2}}
\|z\|_{H^s}^2\|f\|_{L^2}
\notag
\\
&
...
\begin{equation}\label{eq:est_D}
		\|\DD[z,z]\|_{ \mathcal{F}L^{s,p}}
        \les
\|z\|_{ \mathcal{F}L^{s,p}}^2, 
\end{equation}
...
Next, we show (ii), proceeding as for (i). Let $u \in B_\dl$. 
For finite $p$ and $u\in B_\dl \subset X^{s, b}_{p}(0,T)$,
from \eqref{gauge1} and H\"older's inequality,
we have
\begin{align}
\| \DD[z,z] \|_{X^{s, b}_p(0,T)} 
&
\les
\sup_{\xi, \tau} \bigg( \int_{\substack{\tau=\tau_1 + \tau_2 \\ \xi = \xi_1 + \xi_2, A^c}} 
\frac{\jb{\tau-\xi^3}^{bp'}}{\jb{\tau_1-\xi_1^3}^{bp'} \jb{\tau_2 - \xi_2^3}^{bp'} } \frac{\jb{\xi}^{sp'}}{ \jb{\xi_1}^{(s+1)p'} \jb{\xi_2}^{(s+1)p'}}
\bigg)^{\frac{1}{p'}}
\| z\|^2_{X^{s, b}_{p}}
\\
&
=: \Big( \sup_{ \tau, \xi} G(\tau, \xi) \Big)^{\frac{1}{p'}}
\| z\|^2_{X^{s, b}_{p}}
,
\end{align}
where it remains to bound the first factor. 
Note that we either have (a) $|\tau- \xi^3| \les \max_{j=1,2} |\tau_j - \xi_j^3|$ or (b) $|\tau - \xi^3| \les |\xi^3 - \xi_1^3 - \xi_2^3| \sim |\xi \xi_1\xi_2|$.
If (a) holds, with maximum attained for $j=2$, we have
\begin{align}
    G(\tau, \xi) 
    &
    \les 
    \int \frac{1}{\jb{\tau_1 - \xi_1^3}^{bp'}} \frac{1}{\jb{\xi_1}^{(s+2)p'}} d\tau_1 d\xi_1 \les 1, 
\end{align}
given that $b> \frac{1}{p'}$ and $s>-2 + \frac{1}{p'}$. If (b) holds, we obtain
\begin{align}
    G(\tau, \xi) 
    &
    \les 
    \int \frac{1}{\jb{\tau_1 - \xi_1^3}^{2bp'}} \frac{1}{\jb{\xi_1}^{(s+2-3b)p'}} d\tau_1 d\xi_1 \les 1, 
\end{align}
for $s>3b - 2 + \frac{1}{p'}$.

If $p=\infty$, we consider the same cases (a)-(b) as above.
If (a) holds, with $\jb{\tau-\xi^3} \les \jb{\tau_1 - \xi_1^3}$, by H\"older's inequality, we have 
\begin{align}
    \| \DD[z,z] \|_{X^{s, b}_{\infty, \infty}(0,T) \cap X^{s, \wt b}_{\infty, 1}(0,T) }
    &
    \les 
    \| z\|_{X^{s, b}_{\infty, \infty}(0,T) \cap X^{s, \wt b}_{\infty, 1}(0,T)}^2 
    \sup_{\tau, \tau_2, \xi} \int_{\xi=\xi_1+\xi_2,A^c
    } \frac{1}{\jb{\tau_2-\xi_2^3}^{\wt b}} \frac{\jb{\xi}^s}{\jb{\xi_1}^{2(s+1)}} d\xi_1 
    \\
    &
    \les \| z\|_{X^{s, b}_{\infty, \infty}(0,T) \cap X^{s, \wt b}_{\infty, 1}(0,T)}^2 
\end{align}
given that $\wt b\ge0$ and $s>-\frac12$.
...
\\
    ...
    \les 
    \| z\|_{X^{s, b}_{\infty, \infty}(0,T) \cap X^{s, \wt b}_{\infty, 1}(0,T)}^2 
    \sup_{\tau, \tau_1, \tau_2 , \xi} \int_{\xi=\xi_1+\xi_2,A^c
    } \frac{1}{\jb{\tau_1-\xi_1^3}^{b\land \wt b}\jb{\tau_2-\xi_2^3}^{ \wt b}} \frac{\jb{\xi}^s \jb{\xi \xi_1 \xi_2}^{b \lor \wt b}}{\jb{\xi_1}^{2(s+1)}} d\xi_1 
    \\
    &
    \les \| z\|_{X^{s, b}_{\infty, \infty}(0,T) \cap X^{s, \wt b}_{\infty, 1}(0,T)}^2 
    ,
\end{align}
for $b,\wt b \ge0$ and $s> \max(0, -1 + 3 (b \lor \wt b))$. 
...
Lastly, we prove the unboundedness of the map $\Gc^{-1}$ on $\FL^{s,p}(\R)$ for small $s$ in (iii). Let $f \in \FL^{s, p}(\R)$ with 
\begin{align}
...
We can now prove that, under the gauge transform $\Gc$ \eqref{gauge}, \eqref{kdv} and \eqref{rkdv} (more specifically, \eqref{z1}) are equivalent for smooth solutions. 
 Combined with Theorem~\ref{thm:lwp_rkdv}, this will allow us to show well-posedness of the original equation \eqref{kdv}.




\begin{lemma}\label{lem:equivalencia}
Fix $s\gg 1$, $2\le p < \infty$,  $b>\frac{1}{p'}$, and $0<T\ll 1$. There exists $\delta>0$ sufficiently small such that, given $u_0\in \FL^{s,p}(\R)$ with $\|u_0\|_{\FL^{s,p}(\R)}<\delta$, the following are equivalent:

\smallskip
\noi{\rm(i)} $u\in X^{s,b}_p(0,T)$ is a strong integral solution to  \eqref{kdv} with initial data $u_0$.

\smallskip
\noi{\rm(ii)} $v=e^{t\dx^3}u$, with $u\in X^{s,b}_p(0,T)$, is a strong integral solution of \eqref{NFE} (with absolute summability of the nonlinear terms) with initial data $u_0$.

\smallskip
\noi{\rm(iii)} $z=\Gc^{-1}[u]\in X^{s,b}_p(0,T)$ is a strong integral solution of \eqref{z1} (with absolute summability of the nonlinear terms) with initial data  $z_0=\Gc^{-1}[u_0]$.


\smallskip \noi In all three items above, the corresponding strong integral equation is verified in the $L^\infty_T\FL^{s,p}(\R)$ topology.

\end{lemma}

\begin{remark}
    In the case $p=\infty$, an analogous statement holds, where one must replace the Fourier restriction spaces $X^{s,b}_p(0,T)$ with $X^{s,b}_{\infty,\infty}(0,T) \cap X^{s,\tilde{b}}_{\infty, 1}(0,T)$ (as in the statement of Theorem \ref{thm:lwp_rkdv}).
\end{remark}

\begin{proof}
    \textit{Step 1. \rm(i)$\Rightarrow$\rm(ii).} The proof of this implication is tantamount to the formalization of the infinite normal form reduction, which relies on four technical assumptions (see also the discussion in \cite[Section 4]{KOY20}): the differentiation-by-parts in time; the dominated convergence theorem to exchange time derivatives and frequency integrals; the summability of the terms $\NN(\TT;v)$ and $\BB(\TT;v)$; and the convergence of the remainders $\MM(\TT;v)$ to zero (in some topology).
	
	Concerning the two first assumptions, this is guaranteed by the decay of both $v$ and $\dt v$ (as $s\gg 1$) and the fact that, for $\xi$ fixed, {$\ft{v}(\xi)$} is $C^1$ in the time variable.
	To verify the third condition, we observe that, if $\TT\in \Tr_{j}$, $j\ge 2$, then from \eqref{NFE0b},  we have
    \begin{equation}\label{eq:estimativa_mul}
        |\mulB(\TT)| + |\mul(\TT)|\le C^j,
    \end{equation}
    for some $C>0$.  
    As such, from \eqref{NFE1a} together with \eqref{eq:estimativa_mul}, it follows that 
	\begin{align*}
	&\left\|\int_0^t(\NN(\TT;v)+\BB(\TT;v))(t')dt'\right\|_{{L^\infty_T }\mathcal{F}L^{s,p}(\R)}\\\lesssim\ & (1+T)C^j\sup_\xi \left( \int_{\xi=\sum_{\bul\in\TT^\infty}\xi_\bul} \left(\jap{\xi}^{s}\prod_{{\bul\in \TT^\infty}} \frac{1}{\jap{\xi_\bul}^{s}}\right)^{p'} d\vec{\xi} \right)^{\frac{1}{p'}}\|u\|_{L
    ^\infty_T \mathcal{F}L^{s,p}(\R)}^{j+1}\\\lesssim\ & (C(T,s,p)\delta)^j,
	\end{align*}
    where we used the fact that $|\xi|\le (j+1)\max_{\bul\in\TT^\infty}|\xi_\bul|$ and $\|u\|_{L
    ^\infty_T \mathcal{F}L^{s,p}(\R)}\lesssim \delta$ (from the local well-posedness theory \cite{Grunrock_ldv_hier}).
  
    
    If $\TT\in \Tr_1$, that is, $\TT=\<1>$, then $|\mulB(\<1>)|\le 1$ from \eqref{NFE0b}, and the corresponding boundary term $\BB(\<1>;v)$ can be controlled as above. For the term $\NN(\<1>;v)$, we take advantage of the extra information $u\in X^{s,b}_p(0,T)$ and use 
    { Lemma~\ref{lem:linear_estimates_qfinite}, \eqref{NFE1a}-\eqref{NFE0b}, \eqref{wint0b}-\eqref{wint0c}, 
    \eqref{z1}, and} 
    Proposition \ref{prop:bourgLp}:
    {
    \begin{equation}
\left\|\int_0^t \NN(\<1>;v)(t')dt'\right\|_{L^\infty_T\FL^{s,p}(\R)} \lesssim \left\|\int_0^t e^{-(t-t')\dx^3} \Ncal_1(\<1>;u)(t')dt'\right\|_{X^{s,b}_p(0,T)} \lesssim \|u\|_{X^{s,b}_p(0,T)}^2
.
    \end{equation}
}

    \noi
In particular, from the estimates above and the bound on the number of trees with $j$ parents in \eqref{eq:catalan}, we get
\begin{equation}
    \sum_{j\ge 1}\sum_{\TT\in\Tr_j}\left\|\int_0^t(\NN(\TT;v)+\BB(\TT;v))(t')dt\right\|_{L^\infty_T \mathcal{F}L^{s,p}(\R)} \lesssim \sum_{j\ge 1}\frac{(2j)!}{(j+1)! j!}(C(T,s,p)\delta)^j <\infty,
\end{equation}
for $\delta=\delta(T,s,p)$ small, thus proving the summability of $\NN$ and $\BB$. 

It remains to prove the decay of the remainder terms $\MM(\TT)$, $\TT\in \Tr_j$, as $j\to \infty$. Let $j\in\N$ and $\TT\in\Tr_j$. Since from \eqref{psi-tree}
$$
|\Psi(\TT)|\le C^j\max_{\bul\in \TT^\infty} |\xi_\bul|^3,
$$
it follows from \eqref{NFE1a} and \eqref{eq:estimativa_mul} that
\begin{align}
    &\bigg\|\int_0^t\MM(\TT;v)(t')dt'\bigg\|_{{L^\infty_T }\FL^{s,p}(\R)} \\\lesssim &\ (1+T)C^j\sup_\xi \bigg( 
    \int_{\xi=\sum_{\bul\in\TT^\infty}\xi_\bul} \bigg\{\jap{\xi}^{s}\big(\max_{\bul\in\TT^\infty}|\xi_\bul|^3\big)
    \ind_{\bigcap_{\star\in\TT^0} A^c_\star}
    \prod_{{\bul\in \TT^\infty}} \frac{1}{\jap{\xi_\bul}^{s}}\bigg\}^{p'} d\vec{\xi} \bigg)^{\frac{1}{p'}}\delta^{j+1}\\\lesssim\ &(C(T,s,p)\delta)^j,
\end{align}



\noi
where we used the fact that $|\xi|\le C^j |\xi_{\bul}|$ for all $\bul\in\TT^\infty$, due to the restrictions to $A^c_\star$, $\star\in \TT^0$.
Therefore, together with \eqref{eq:catalan}, we have
$$
\sum_{\TT\in \Tr_j}\left\|\int_0^t\MM(\TT;v)(t')dt'\right\|_{{L^\infty_T}\FL^{s,p}(\R)} \lesssim \frac{(2j)!}{j!(j+1)!}(C(T,s,p)\delta)^j \to 0\quad \mbox{as }j\to\infty, 
$$
for $\dl$ sufficiently small.


\smallskip
\noi\textit{Step 2.} (ii)$\Leftrightarrow$(iii). The fact that $u\in X^{s,b}_p(0,T)$ if and only if $z\in X^{s,b}_p(0,T)$ follows directly from Lemma \ref{LEM:D}. As for the integral equations, the equivalence is a direct consequence of Subsection \ref{SUBSEC:gauge}, the invertibility properties of $\Gc$ over $\FL^{s,p}(\R)$ in {Lemma~\ref{LEM:D}} and the fact that the cancellations observed in Lemmas \ref{LEM:wbd} and \ref{LEM:wint} are term-by-term cancellations.


\smallskip
\noi\textit{Step 3.} (ii)$\Rightarrow$(i). Given $v$ as in (ii), by Step 2, $z=\mathcal{G}^{-1}[e^{-t\partial_x^3}v]\in X^{s,b}_p(0,T)$
is a strong integral solution of \eqref{z1}.


Let $u\in X^{s,b}_p(0,T)$ be the solution to \eqref{kdv} with initial data $u_0$ (whose existence is ensured by Grünrock's result \cite{Grunrock_ldv_hier}). By Steps 1 and 2, $z'=\Gc^{-1}[u]\in X^{s,b}_p(0,T)$ is also a strong integral solution of \eqref{z1}. By uniqueness over $X^{s,b}_p(0,T)$ (Theorem \ref{thm:lwp_rkdv}), we must have $z=z'$ and therefore $u=e^{-t\dx^3}v$, concluding the proof.
\qedhere
...
{Lastly, we show well-posedness of \eqref{kdv}.}


\begin{proof}[Proof of Theorem \ref{thm:wp_kdv_intro}] We consider only the case $2\le p<\infty$. For smooth initial data $\FL^{s',p}(\R)$, $s'\gg 1$,  Lemma~\ref{lem:equivalencia} ensures that $\wt{\Theta}$ is exactly the \eqref{kdv} data-to-solution map. By Theorem \ref{thm:lwp_rkdv} and the mapping properties of the gauge transform $\Gc$ (Lemma \ref{LEM:D}), $\wt{\Theta}$ (and therefore the \eqref{kdv} flow) has a unique continuous extension to $\FL^{s,p}(\R)$.
\end{proof}
