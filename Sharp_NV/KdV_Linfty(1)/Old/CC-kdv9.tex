...
\andreia{I believe the data-to-solution map below is Lipschitz continuous, so we could emphasize this in the statement. KPV 2001 in Theorem 1.4 show that for $s<-\frac34$, there is failure of uniform continuity \cite{KPV_ill}. This means that our result is also optimal at $p=2$, because there is failure of uniform continuity below our regularity. Maybe there is a point in proving a similar failure result for $\FL^{s,p}$ and $2<p<5$}
