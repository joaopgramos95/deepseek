...
\DeclareMathOperator{\mulB}{mul_{\mathcal{B}}}
...
\item Want $\D$ or $\DD$ for the bilinear operator coming from boundary piece? Former in later sections and latter in introduction
...
\end{itemize}
    ...
    \item Do we use the notion of 4-grandparent?

    \item Missing some hats for F.T., so check all equations (decided to not omit when defining new variables for NFE)


\item In Subsection~\ref{SUB:tree}, started using $\NN$ for the multilinear operators. Can we use $\mathcal{N}$ since never used ? This may be more noticeable and easier to read
\\
Can similarly use $\mathcal{N}^{\gfrak}, \mathcal{N}^{\bfrak}$. This should be easier to change by changing the macro for $\NN$
\\
In the derivation of NFE, we also use $\mathrm{M}$ which then matches the original style of $\NN$ instead of the caligraphic one, but $\mathcal{M}$ is used later on for the decompositions of the multiplier. 
Should make choice here for consistency. Could also use $\mathcal{E}$ for "error" or $\mathcal{R}$ for "remainder"?
\\
The only issue with the above is that $\B$ is already in use for the boundary piece operator, instead of only to denote generic boundary terms



\item Simplify statement of Lemma~\ref{LEM:wint} with immediate clarification of the types and the multipliers for each type
\\
Instead of having $\NN^\gfrak $ can have $\NN^{\I}, \NN^{\rm II},$..., for example, or $\NN^1, \NN_1$ ?

\item Uniformize notation for Cases, with or without underline, with or without $\bul$, with or without textbf
...
\item In
Section~\ref{sec:fourier_restr}, change $\dl$ appearing as small parameter for cutoffs, by another letter, since $\dl$ shows up in well-posedness as small data parameter

\item For the Frequency restricted estimates, should agree on notation in \eqref{eq:multilinear_geral}
 and need to introduce all the relevant paramters $b, b', \wt b , \wt b '$. This is because estimate for $2<p<\infty$, need to be done for fixed time object, without Duhamel integral

 \item write remark on dgBO in $\FL^{s, \infty}$

 \item change $\Phi$ to $\phi$ in integrals in FREs
...
\end{corollary}
	...
	\begin{equation}\label{rkdv}\tag{KdV*}
		\partial_t z + \partial_x^3 z = 
        \sum_{j=2}^\infty N_j[z]
	...
	\begin{figure}[h]
		\centering
		\includegraphics[height=5cm]{rekdv_lwp.pdf}
	...
	\begin{figure}[h]
		\centering
		\includegraphics[height=5cm]{kdv_lwp.pdf}
	...
	\begin{remark}
		...
		$$
		\left|\frac{\xi}{\Phi}\right|=\frac{1}{3|\xi_1\xi_2|}\lesssim 1\qquad \mbox{over }A.
		$$
	...
	\textbf{Notations.} Throughout this work, $\mathcal{F}_\ast$ respresents the Fourier transform in the variable $\ast$. When taken over the spatial variable, we may also use the notation $\hat{\cdot}$ for the Fourier transform and $\check{\cdot}$ for its inverse. Given real numbers $a,b$, we say that $|a|\lesssim |b|$ if there exists a universal constant $C>0$ such that $a<Cb$. If $C$ can be chosen to be smaller than $10^{-10}$, we write $|a|\ll |b|$. If both $a\lesssim b$ and $b\lesssim a$, we write $a\sim b$. Finally, if $|b-a|\ll |a|$, we say that $a\simeq b$.
	
	
	We start by introducing relevant notations and function spaces.
	...
	
	{
		...
		\begin{itemize}
			...
			\item Add notion of $\ll$, $\les$, $\land$, $\lor$, ...
			
			\item Fourier transform
			
			\item Sobolev spaces and norms
...
\section{Infinite normal form equation and change of variables}
...
\color{red}
Add comment along the lines of: the procedure is based on the Duhamel formulation in \eqref{duhamel} and performing integration by parts in a region where the current nonlinear term cannot be estimated at sufficiently low regularity (namely in the region that imposes $s>-\frac12$ in the bilinear estimate in Lemma~\ref{LEM:bil0}.)
}
...
\begin{align}
\label{duhamel2}
\ft v(t) = \ft v(0) + i \int_0^t \int_{\xi = \xi_1 + \xi_2} e^{it' \Psi(\<1>)}  K \Psi(\<1>)   v_{1} (t') v_{2} (t')
d\xi_{1} dt'.
\end{align}
Then, splitting the region of integration $\{\xi=\xi_1+\xi_2\}$ into $A\cup A^c$ as in \eqref{A}, and performing a differentiation-by-parts to the contribution restricted to the bad region $A^c$, we obtain
\begin{align}
\label{NFE01}
\begin{split}
\ft v(t)  
= \ft v(0)  
&
+ i \int_0^t \int_{\xi = \xi_1 + \xi_2} e^{it' \Psi(\<1>)} 
[ \ind_{A} K \Psi(\<1>)   ]
(\ft v_{1} \ft v_{2} )(t')
d\xi_{1}  dt'
\\
&
+ \bigg[ \int_{\xi = \xi_1 + \xi_2} e^{it' \Psi(\<1>)} 
[\ind_{A^c}
 K ] 
(\ft v_{1} \ft v_{2} )(t')
d\xi_{1} 
...
&
-
  \int_0^t \int_{\xi = \xi_1 + \xi_2} e^{it' \Psi(\<1>)} 
[ \ind_{A^c} K ]
...
&
- i
\int_0^t \int_{\substack{\xi = \xi_1 + \xi_2 \\ \xi_1 = \xi_{11}+ \xi_{12}  }} \exp\Big(it' \Psi(\<21>) \Big)
...
&
\quad 
-i \int_0^t \int_{\substack{\xi = \xi_1 + \xi_2 \\ \xi_1 = \xi_{21}+ \xi_{22}  }} \exp\Big(it' \Psi(\<22>) \Big)
...
%\Ical(\TT, m)(t):= 
\Ical(\TT, m; v)(t) := i \int_0^t \int_{\G(\TT)}
e^{it'\Psi(\TT)}m(\vec{\xi})
\prod_{\bul\in \TT^\infty}  \ft v_\bul (t') d\vec{\xi}dt'
\end{equation}
...
\begin{lemma}[Integration by parts on trees]
\label{LEM:IBP}
...
\Ical (\TT,m; v)(t) &
= \Big[
\B\Big(\TT,\frac{m}{\Psi(\TT)} ; v \Big)(t')
\Big]_{t'=0}^{t'=t} 
+ \sum_{\star \in \TT^\infty} \Ical 
\Big(\TT \overset{\star}{\oplus} \<1>
, - \xi_\star \frac{m}{\Psi(\TT)} ; v \Big)(t)
\\
&
= \Big[
\B\Big(\TT,\frac{m}{\Psi(\TT)} ; v \Big)(t')
...
+ \sum_{\star \in \TT^\infty} \Ical 
\Big(\TT \overset{\star}{\oplus} \<1>
, - K_\star \Psi(\star) \frac{m}{\Psi(\TT)} ; v \Big)(t)
,
\end{split}
\end{align}
where $\Ical$ and $\B$ are the integral and boundary functionals in \eqref{eq:defiI} and \eqref{eq:defiB}, respectively, $\TT \overset{\star}{\oplus} \<1> $ denotes tree concatenation at the final node $\star\in \TT^\infty$ as in Definition~\ref{DEF:tree3}(ii), and $\Psi(\star), \Psi(\TT), K_\star$ are as in \eqref{psi-tree}, \eqref{psi-circ}, and \eqref{Kmul}.
...
\Ical(\TT, m)(t)
&
= i \int_0^t \int_{\G(\TT)} e^{it'\Psi(\TT)} m(\vec \xi) \prod_{\bul\in \TT^\infty} \ft v_\bul (t') d\vec\xi dt' 
\\
& =  i \int_0^t \int_{\G(\TT)} \frac{d}{dt'} \bigg( \frac{e^{it'\Psi(\TT)}}{ i \Psi(\TT)} \bigg) m(\vec \xi) \prod_{\bul\in \TT^\infty} \ft v_\bul (t') d\vec\xi dt'  \\
& = \bigg[ \int_{\G(\TT)} e^{it' \Psi(\TT)} \frac{m(\vec\xi)}{\Psi(\TT)}  \prod_{\bul\in \TT^\infty} \ft v_\bul (t') d\vec\xi  \bigg]_{t'=0}^{t' = t} 
\\
& \qquad - \int_0^t \int_{\G(\TT)} e^{it' \Psi(\TT)} \frac{m(\vec\xi)}{\Psi(\TT)} \partial_{t'}  \bigg(\prod_{\bul\in \TT^\infty} \ft v_\bul \bigg)(t') d\vec\xi dt'.
\end{align}
Combining \eqref{eq:defiB} and \eqref{dtv}, we have
\begin{align}
\Ical(\TT, m)(t)
& = \Big[ \B\Big( \TT, \frac{m}{\Psi(\TT)} ;v \Big) (t') \Big]_{t'=0}^{t'=t} \\
& \quad - \sum_{\star \in \TT^\infty} i \int_0^t \int_{\G ({\TT \overset{\star}{\oplus} \cherryS})} e^{it'[\Psi(\TT) + \Psi(\star) ]} K_\star \Psi(\star) \frac{m(\vec\xi) }{\Psi(\TT)} \bigg( \ft v_{\star1} \ft v_{\star2} \prod_{\bul\in\TT^\infty\setminus\{a\}} \ft v_\bul \bigg) (t') d\vec\xi dt'
\\
& =
\Big[ \B\Big( \TT, \frac{m}{\Psi(\TT)} ;v \Big) (t') \Big]_{t'=0}^{t'=t} 
+
...
\\
&
=: \Ft_x[ \NN ( \<1>; v )] (t, \xi) + \Ft_x[ \MM(\<1>; v) ] (t,\xi)
\label{NFE000}
\end{align}
where $\Ical$ and $\B$ are as in \eqref{eq:defiI} and \eqref{eq:defiB}. 

We now derive the second iteration of the normal form equation. 
First, we perform an integration by parts as in Lemma~\ref{LEM:IBP}:
\begin{align}
\label{NFE00}
\begin{split}
\ft v(t) 
...
=
 \Ft_x[ \NN ( \<1>; v )] (t, \xi) 
+ 
\big[ \B \big( \<1>,  \ind_{A^c} K; v \big)(t') \big]_{t'=0}^{t'=t}
\\
&
+
\Ical \Big( \<21>, - \ind_{A^c} KK_1\Psi(1) ; v  \Big) (t) 
+
\Ical \Big( \<22>, - \ind_{A^c} KK_2\Psi(2) ; v  \Big) (t)
. 
\end{split}
\end{align}

\noi
As for the first normal form reduction in \eqref{NFE01}, we split each region of frequency integration between a good and a bad set, and only perform an integration by parts when restricted to the latter. Generalizing the definition of $A, A^c$ in \eqref{A}, given a tree $\TT$ and a parent node $\bul \in \TT^0$, we define the analogous sets $A_\bul$ and $A_\bul^c$ as
\begin{align}
\label{defA}
\begin{split}
A_\bul& := A_\bul^{\le 1} \cup A_\bul^{> 1}
\\
&
:=
 \big\{ (\xi_{\bul1}, \xi_{\bul2}) \in \R^2: \ \xi_\bul =
\xi_{\bul1}
+ \xi_{\bul2}, \ |\xi_{\bul1}| \lor  |\xi_{\bul2}|  \le 1 \big \} \\
 &
\quad
\cup
\big\{ (\xi_{\bul1}, \xi_{\bul2}) \in \R^2: \ \xi_\bul =
...
+ \xi_{\bul2}, 
\
 \big| | \xi_{\bul1} |  - | \xi_{\bul2}| \big|  \ges (|\xi_{\bul1}| \lor |\xi_{\bul2}|) \ge 1
 \big\} , \\
A_\bul^c & := \big\{ (\xi_{\bul1}, \xi_{\bul2}) \in \R^2: \ \xi_\bul =
\xi_{\bul1} +
\xi_{\bul2},\
|\xi_{\bul1}| \lor |\xi_{\bul2}| \ge1, \
\big| | \xi_{\bul1}| - |\xi_{\bul2}| \big| \ll (|\xi_{\bul1}| \lor |\xi_{\bul2}|)\big\} 
.
\end{split}
\end{align}
...
&
+
\Ical \Big( \<21>, - \ind_{A^c \cap A_1} KK_1\Psi(1) ; v  \Big) (t) 
+
\Ical \Big( \<21>, - \ind_{A^c \cap A_1^c} KK_1\Psi(1) ; v  \Big) (t) 
\\
&
+
\Ical \Big( \<22>, - \ind_{A^c \cap A_2} KK_2\Psi(2) ; v  \Big) (t)
+
\Ical \Big( \<22>, - \ind_{A^c \cap A_2^c} KK_2\Psi(2) ; v  \Big) (t).
\end{align}
As before, we keep the integral contributions localized to a ``good set" $A_\bul$. Regarding the contributions localized to $A_\bul^c$ for all $\bul\in \TT^0$, despite the restriction to the ``bad regions'', we will keep a suitable part of these terms before performing another integration by parts. In particular, by using the additivity of the phase $\Psi$ in \eqref{eq:additivity}, for $(\star, \TT)\in \{ (1, \<21>) , (2, \<22>) \}$,  we write
\begin{align}
\Psi(\star) 
= \Psi(\cherry{\star}{}{}) 
= \big[ \Psi(\cherry{\star}{}{}) + \Psi(\<1>) \big]- \Psi(\<1>) 
=
\Psi(\TT) - \Psi(\<1>),
\end{align}
...
&
+
\Ical \Big( \<21>, -[ \ind_{A^c \cap A_1} \Psi(1) - \ind_{A^c \cap A_1^c} \Psi(\<1>)  ]KK_1 ; v  \Big) (t) 
+
\Ical \Big( \<21>, - \ind_{A^c \cap A_1^c} KK_1\Psi(\<21>) ; v  \Big) (t)
\\
&
+
\Ical \Big( \<22>, - [\ind_{A^c \cap A_2} \Psi(2) - \ind_{A^c \cap A_2^c} \Psi(\<1>)  ] KK_2; v  \Big) (t) 
+
\Ical \Big( \<22>, - \ind_{A^c \cap A_2^c} KK_2\Psi(\<22>) ; v  \Big) (t)
\\
&
= : 
 \Ft_x[ \NN ( \<1>; v )] (t, \xi) 
...
\label{NFE002}
\end{align}
Note that $\NN$ are time-integral terms, $\BB$ is a boundary term, and $\MM$ are remainders on which we apply \eqref{IBP} at the next step. 
...
&
= 
\big[ \B(\<21>, - \ind_{A^c \cap A_1^c} KK_1 ; v) \big]_{t'=0}^{t'=t}
+
\big[ \B(\<22>, - \ind_{A^c \cap A_2^c} KK_2 ; v) \big]_{t'=0}^{t'=t}
\\
&
\quad 
+ \Ical\big( \<31> ,  \ind_{A^c\cap A_1^c} KK_1 K_{11} \Psi(11)   ; v \big)(t)
+ \Ical\big( \<32> ,  \ind_{A^c\cap A_1^c} KK_1 K_{12}  \Psi(12) ; v \big)(t)
\\
&
\quad 
+ \Ical\big( \<33> ,  [ \ind_{A^c\cap A_1^c} \Psi(2)+ \ind_{A^c \cap A_2^c} \Psi(1) ] KK_1 K_2    ; v \big)(t)
\\
&
\quad 
+ \Ical\big( \<34> ,  \ind_{A^c \cap A_2^c } K K_2 K_{21} \Psi(21) ; v \big)(t)
+ \Ical\big( \<35> , \ind_{A^c \cap A_2^c } K K_2 K_{22}  \Psi(22)   ; v \big)(t)
, 
\end{align}
...
&
\quad 
+ \Ical\big( \<31> ,  [\ind_{A^c\cap A_1^c \cap A_{11}} \Psi(11) - \ind_{A^c\cap A_1^c \cap A_{11}^c} \Psi(\<21>) ] KK_1 K_{11}  ; v \big)(t)
\\
&
\quad 
+
 \Ical\big( \<32> , [ \ind_{A^c\cap A_1^c \cap A_{12}} \Psi(12) - \ind_{A^c \cap A_1^c \cap A_{12}^c} \Psi(\<21>)  ]KK_1 K_{12}  ; v \big)(t)
\\
&
\quad 
+ \Ical\big( \<33> ,  [ \ind_{A^c\cap A_1^c \cap A_{2}} \Psi(2)+ \ind_{A^c \cap A_1 \cap A_2^c} \Psi(1)  - \ind_{A^c \cap A_1^c \cap A^2_c} \Psi(\<1>)] KK_1 K_2    ; v \big)(t)
\\
&
\quad 
+ \Ical\big( \<34> , [ \ind_{A^c \cap A_2^c \cap A_{21}} \Psi(21) - \ind_{A^c \cap A_2^c \cap A_{21}^c} \Psi(\<22>) ]K K_2 K_{21}  ; v \big)(t)
\\
&
\quad 
+ \Ical\big( \<35> ,[ \ind_{A^c \cap A_2^c \cap A_{22} } \Psi(22) - \ind_{A^c \cap A_2^c \cap A_{22}^c} \Psi(\<22>) ] K K_2 K_{22}    ; v \big)(t)
\\
&
\quad 
+ \sum_{\TT\in\Tr_3} 
...
v(t) - v(0)
&
= \sum_{j=1}^2 \sum_{\TT \in \Tr_j} \BB(\TT,v)(t) 
+ 
\sum_{j=1}^3 \sum_{\TT\in \Tr_j} \NN(\TT; v) (t)
+
\sum_{j=1}^3 \sum_{\TT\in \Tr_j} \MM(\TT; v) (t), 
\end{align}
...
\begin{proposition}
\label{PRO:NFE}
Let $N\in\N$, $u$ be a smooth solution to \eqref{kdv}, and $v(t) = \Ft_x(e^{t\dx^3} u(t) )$ solve \eqref{duhamel}.
Then, 
the $N$-th normal form equation for $v$ is given by
\begin{align}
\label{NFE1}
v(t)=v(0) 
+
\sum_{j=1}^{N-1} \sum_{\TT\in \Tr_j} \BB(\TT; v) (t)
+ \sum_{j=1}^{N} \sum_{\TT\in \Tr_j} \NN(\TT; v) (t)
+ \sum_{\TT\in \Tr_{N}}
\MM(\TT; v)(t),
\end{align}
...
\label{NFE1a}
\begin{split}
\Ft_x [\BB(\TT;v)] (t,\xi) & = \left[\B(\TT, \mulB(\TT); v)(t')\right]_{t'=0}^{t'=t},\\
\Ft_x [\NN(\TT;v)] (t,\xi) & = \Ical(\TT, \mul(\TT) ; v  )(t), \\
\Ft_x [\MM(\TT;v)] (t,\xi) & = \Ical(\TT, \Psi(\TT)\mulB(\TT); v) (t),
\end{split}
\end{align}

\noi
with $\Ical(\TT, m; v)$ and $\B(\TT, m;v )$ as in \eqref{eq:defiI} and \eqref{eq:defiB}, respectively.
Moreover,
...
The claim for $N=1,2$ holds due to \eqref{NFE000} and \eqref{NFE002}.
To show \eqref{NFE1} for $N\ge3$, it suffices to show the following identity for $N\ge 2$:
\begin{equation}\label{eq:teseinducao}
\sum_{\TT\in\Tr_{N}} \MM(\TT; v) 
= 
\sum_{\TT\in \Tr_{N}} \BB(\TT; v) 
+
\sum_{\TT\in \Tr_{N+1}} \NN(\TT; v) 
+ \sum_{\TT\in \Tr_{N+1}} \MM(\TT; v),
\end{equation}
along with the definitions in \eqref{NFE1a}-\eqref{NFE0b}. Note that we know it to hold already for $N=2$. 


Let $N\in \N$ with $N\ge2$, and assume that \eqref{eq:teseinducao} holds for $1\le M \le N$, 
with $\BB(\TT; v)$ and $\MM(\TT; v)$ given in \eqref{NFE1a}, and multipliers as in \eqref{NFE0b}, for any $\TT\in \Tr_M$, $1\le M \le N$, and similarly for $\NN(\TT; v)$ for $\TT\in \Tr_M$ for $1\le M \le N+1$.
For a fixed $\TT\in\Tr_{N}$, using the integration-by-parts formula \eqref{IBP}, we obtain
\begin{align*}
\Ft_x [\MM(\TT; v) ](t,\xi) 
&
=\Ical(\TT, \Psi(\TT)\mulB(\TT); v)(t)
\\
& =
\left[\B(\TT,\mulB(\TT); v)(t')\right]_{t'=0}^{t'=t} +
\sum_{\star\in\TT^\infty}
\Ical (\TT \overset{\star}{\oplus} \<1>, -\mulB(\TT) K_\star \Psi(\star) ; v )(t)
\\
&
= \Ft_x[\BB(\TT; v) ](t, \xi)  
+
\sum_{\star\in\TT^\infty} \Ical (\TT \overset{\star}{\oplus} \<1>, -\mulB(\TT) K_\star \Psi(\star) ; v) (t) .
\end{align*}



\noi
Thus, summing over the choices of $\TT \in \Tr_{N}$, we have
\begin{align}
\label{mul2a}
\begin{split}
&
\sum_{\TT\in \Tr_{N}} \Ft_x [\MM(\TT; v) ](t,\xi) 
\\
&
= \sum_{\TT\in \Tr_{N}} \Ft_x[\BB(\TT; v) ](t, \xi)  
+
\sum_{\TT\in \Tr_{N}}
\sum_{\star\in\TT^\infty}
\Ical 
...
\end{split}
\end{align}
since that map taking $(\TT, \star) \in \Tr_N \times \TT^\infty \mapsto (\TT', \star) \in \Tr_{N+1} \times (\TT')^{0,\ff}$ with $\TT' = \TT \overset{\star}{\oplus} \<1>$ is a bijection, and  
where the last equality follows from the linearity of $\Ical$ with respect to the multiplier, as seen in \eqref{eq:defiI}. Since for $\TT\in \Tr_{N+1}$, the tree $\TT \overset{\star}{\ominus} \<1>\in \Tr_{N}$, by the induction hypothesis, using \eqref{NFE0b}, we can write the multiplier of the last term as follows
...
&
=
\sum_{\TT\in \Tr_{N+1}}  \Ical \big(\TT, \mul(\TT) + \mulB(\TT) \Psi(\TT) ; v \big) (t)
 \\
& 
= 
 \sum_{\TT\in \Tr_{N+1}}  \Ft_x [\NN(\TT ; v ) ](t, \xi) 
+  \sum_{\TT\in \Tr_{N+1}} \Ft_x [\MM(\TT ; v ) ] (t, \xi),
\end{align}
...
\andreia{I am convinced the convergence only holds here for $s>-\frac12$ because of Lemma~\ref{LEM:bil0} and the frequency restriction we have. So we cannot claim that the INF makes sense below this regularity. Unless I am missing something in the proof. }

The following result shows the convergence to zero of the error terms  $\MM(\TT; v)(t)$ in \eqref{NFE1}  as $N\to\infty$. 
\begin{lemma}
\color{red}

\begin{align}
\bigg\| \sum_{\TT\in \Tr_N} \MM(\TT; v ) (t) \bigg\|_{ L^{\infty}_\xi (\R) } \to 0 , 
\end{align}
as $N\to\infty$. 
...
\begin{lemma}
	Given $s\gg 1$, $T>0$ and $u_0\in \mathcal{F}L^{s,p}(\R)$ with $\|u_0\|_{\mathcal{F}L^{s,p}}\ll 1$, the corresponding global solution to \eqref{kdv} satisfies \eqref{NFE} for each $t\in [0,T]$ (in $\mathcal{F}L^{s-3,p}(\R)$). 
\end{lemma}
\begin{proof}
...
\begin{align}
\label{NFE}
v_1(t)=v_1(0) + \sum_{j=1}^{\infty} \sum_{\TT\in \Tr_j} \left(\NN(\TT) +
\B(\TT) \right)
,
\end{align}
with $\NN(\TT)$ and $\B(\TT)$ as in \eqref{NFE1a}.
...
Before defining our change of variables, we introduce multilinear versions of the functionals $\Ical$ and $\B$ in \eqref{eq:defiI} and \eqref{eq:defiB}.
...
\Ical(\TT, m; v_1,\dots,v_{j+1}) (t)=i\int_0^t \int_{\Gamma_\TT}
e^{it'\Psi(\TT)}m(\vec{\xi}) \prod_{\ell
=1}^{j+1} \ft v_j(t',\xi_{\bul_\ell}) d\vec{\xi}dt'
\end{equation}
and the boundary term
...
\end{equation}
Note that
when $v_1 = \cdots = v_{j+1} = v$, the definitions above agree with those of $\Ical(\TT, m; v )$ and $\B(\TT, m; v)$ in \eqref{eq:defiI} and \eqref{eq:defiB}, respectively.  Moreover, we define the multilinear analogues of $\NN(\TT)$ and $\BB(\TT)$ in \eqref{NFE1a} in Proposition~\ref{PRO:NFE}, as follows:


\noi
\begin{align*}
\Ft_x [\NN(\TT; v_1, \ldots, v_{j+1} ) ] (t, \xi )
&= 
\Ical(\TT,\mul(\TT) ; v_1, \ldots,
v_{j+1} ) (t)
, \\
\Ft_x [\BB(\TT; v_1, \ldots, v_{j+1}) ] (t, \xi) &=
\big[ \B (\TT,\mulB(\TT) ; v_1,\dots,v_{j+1}) (t')\big]_{t'=0}^{t'=t}.
\end{align*}
...
,
\end{align}
where $\B$ is as in \eqref{eq:defiB}. Then, we have
\begin{align}
\label{wa}
v(t) - v(0) 
=
...
\begin{align}
w(t) & = w(0) -  \BB(\<1>;w)(t) + \BB(\<1>;v) (t) + \NN(\<1>;v)(t) 
\nonumber\\
& \qquad + \sum_{j=2}^\infty \sum_{\TT\in\Tr_j} \Big( \NN(\TT; v) (t) +
\B(\TT; v) (t)
 \Big) \nonumber\\
& = w(0) 
+ \BB\big(\<1>; w, \D[w, w] \big) (t)
+ \BB \big(\<1>; \D[w, w],w\big) (t) 
+
\BB\big(\<1>; \D[w,w], \D[w, w]\big) (t)
  \nonumber\\
& \qquad 
+ \NN(\<1>; w ,w ) (t) + \NN\big(\<1>; w, \D[w, w]\big) (t)
+
\NN\big(\<1>; \D[w, w], w\big) (t)
+ \NN\big(\<1>; \D[w, w],\D[w, w]\big) (t)
 \nonumber\\
& \qquad 
...
$\star \in \TT^{0,\ff}$, and $u_1, \ldots, u_{j+1}: [0,T]\times \R \to \R$ be any smooth functions. Moreover, consider the lexigraphical ordering of the leafs of $\TT$ as in \eqref{lexic}, 
and let $1 \le k \le j$ be such that $(\bul_k, \bul_{k+1})$ are the children of $\star$ in $\TT$. 
Then, we have 
\begin{align}
\label{wbd0}
\BB\big(\TT ; u_1, \ldots, u_{j+1} \big) 
+ \BB\big(\TT\overset{\star}{\ominus} \<1>; u_1,  \ldots, u_{k-1} , \D[u_k,u_{k+1}] , u_{k+2}, \ldots,  u_{j+1} \big) 
\equiv 0
,
\end{align}
%Consequently, 
...
We can then further rewrite the equation \eqref{w1} by grouping time-integral terms in a similar fashion. Similarly to Lemma~\ref{LEM:wbd} which establishes the cancellation of \textit{all} boundary terms in the $w$ equation \eqref{w1}, the next lemma also shows some cancellation for time-integral terms. In 
...
\label{wint0b}
\begin{split}
\Ft_x \big[ \NN_\ell (\TT; w) \big] (t, \xi)& =
\Ical \big(\TT, {\mul}_\l(\TT); w \big) (t),
% , \\
...
\begin{remark} \rm
(i)
     From \eqref{wint0c}, we see that the only trees with nonzero contributions are, up to permutations, are as depicted in Figure \ref{FIG:types}.
...
\noi (ii) From the definition of the multipliers in \eqref{wint0c}, note that the terms of Type $\III$ and $\IV$ do not have a restriction on the frequencies in the generation with parent $\pb(\star)$ and $\star$, respectively. 
...
\begin{align}
\label{w2}
w(t) & = w(0 ) + \NN_1(\<1>;w)(t) + \sum_{j=2}^\infty \sum_{\TT \in \Tr_j} \sum_{\l=2}^4  \NN_\l(\TT;w) ( t) ,
\end{align}
...
&
= e^{-t\dx^3}z(0)
+ \NN_1(\<1>;e^{t\partial_x^3}z)(t) + \sum_{j=2}^\infty \sum_{\TT \in \Tr_j} \sum_{\l=2}^4  \NN_\l(\TT;e^{t\partial_x^3}z) ( t) 
\end{align}
In Section \ref{sec:multi}, we will use the Fourier restriction norm method to construct solutions for \eqref{z1}. 
To recover solutions for \eqref{kdv}, {\color{red} \bf given that the relevant maps are well-defined (see Lemma~\ref{})}, we are interested in solving \eqref{z1} with initial data satisfying
\begin{align}
\label{z2}
z(0) + \D[e^{t\dx^3}z,e^{t\dx^3}z](0)
= 
u_0, 
\end{align}
where $\D$ is the bilinear operator in \eqref{w}. 
{
\color{blue}
Moreover, given a solution $z$ to \eqref{z1} sufficiently smooth, with initial data as in \eqref{z2}, we can recover a solution to \eqref{kdv} as follows
\begin{align}
z(t) 
\overset{e^{t\dx^3}}{
\mapsto }
w(t) 
\overset{\mathcal{G}^{-1}}{\mapsto }
v(t)
\overset{e^{-t\dx^3}}{
\mapsto }
u(t)
,
\end{align}
where $\mathcal{G}[w] = w + \D[w,w]$ denotes the change of variables in \eqref{w}, and $\mathcal{G}^{-1}$ its inverse, when well-defined. See Lemma~\ref{LEM:D} for details on the invertibility of this map. We can then write
\begin{align}
u(t) 
=
e^{-t\dx^3} \Gc^{-1} [ e^{t\dx^3} v ] (t).
\end{align}
}

\andreia{I think if we are going to write this sort of inversion formula and since we are introducing the change of variables, the result in Lemma~\ref{LEM:D} should also be moved to this section}

We conclude this subsection with the proofs of Lemma~\ref{LEM:wbd} and Lemma~\ref{LEM:wint}.
...
\begin{proof}[Proof of Lemma~\ref{LEM:wbd}]
...
=
\bigg[
\int_{\G(\TT)} e^{it' \Psi(\TT)} \mulB(\TT) \prod_{\l=1}^{j+1} \ft{u}_{\l} (t', \xi_{\bul_j}) d\vec\xi
\\
&
\quad 
+
\int_{\G(\TT\overset{\star}{\ominus}\<1>)} \int_{\xi_{\star} = \xi_k + \xi_{k+1}} e^{it' \Psi(\TT\overset{\star}{\ominus}\<1>) + it' \Psi(\star)} \mulB(\TT\overset{\star}{\ominus}\<1>) \mulB(\cherry{\star}{}{}) \prod_{\l=1}^{j+1} \ft{u}_{\l} (t', \xi_{\bul_j})  \, d\xi_k d\vec\xi
\bigg]_{t'=0}^{t'=t}
\\
&
= 
...
\big\}
\prod_{\l=1}^{j+1} \ft{u}_{\l} (t', \xi_{\bul_j}) d\vec\xi
\bigg]_{t'=0}^{t'=t}
\end{align}
...
To show \eqref{wbd1}, given $j\in \N$ with $j\ge 2$, and $\TT \in \Tr_j$, we gather all the contributions on the right-hand side of \eqref{wbd1} which are represented by $\TT$ ignoring colourings:
\begin{align}
\label{wbd2a}
...
\andreia{Kept the old proof below. It is shorter, but it does not directly use the pairwise cancellation in \eqref{wbd0}. 
We can choose which version to keep later. }

Let $j\in \N$ and $\TT \in \Tr_j$. Then, gathering all the boundary terms in \eqref{w1}
associated with the tree $\TT$ (i.e., all the terms with homogeneity $|\TT^\infty|$ in $w$), we have
\begin{align*}
\wt{\B}(\TT) [w ] & =
\sum_{ P\subseteq \TT^{0,f}  } \B(\TT\setminus P ) \big[ f_{\bul} = w, \ \bul
\in (\TT\setminus P)^\infty \setminus P ; f_{\star} = \D[w] , \star \in P
\big].
\end{align*}


Using \eqref{NFE1a}, \eqref{eq:defiB}, and \eqref{NFE0b}, we see that
\begin{align}
\wt{\B}(\TT) [w ]  = \bigg[ B(\TT, \wt{\mul}_\B (\TT) )[w] \bigg]_{t'=0}^{t'=t},
\end{align}
where
\begin{align}
\label{wbd1a}
...
\wt{\mul}_\B(\TT)
 &
= \sum_{P\subseteq \TT^{0,\ff}} \mul_\B(\TT\setminus P)
...
\andreia{Change proof to match notation in the statement}

We note that there is a unique element on LHS of \eqref{wint0a} with homogeneity 2 in $w$, namely $\NN(\<1>; w,w)$, thus the claim easily follows with the multipliers in \eqref{wint0b} for $\<1>$ from \eqref{NFE1a}-\eqref{NFE0b}. 

Next, we consider the contributions on LHS of \eqref{wint0a} associated with the trees $\<21>$, $\<22>$, and $\<33>$ ignoring colourings. In particular, we treat the contributions $\NN(\<1>)$ due to \eqref{NFE1a} have no term localized to $A^c$, thus the resulting formulas will not fit under the general case. 
...
where we note the agreement with \eqref{wint0b}.
	...
	\begin{align}
		...
		\end{split}
	\end{align}
	where we used \eqref{NFE0b}, and the fact that $(\TT\overset{C}{\ominus}\<1>)^0 = \TT^0 \setminus C$. It remains to show that the multipliers defined above agree with \eqref{wint0c}.




	We first consider ${\mul}^{\bfrak}(\TT)$, which we write as 
	\begin{align}
		{\mul}_{\bfrak} (\TT) 
%& =
...
\noi
$\bul$ \underline{\textbf{Case 1}: (a) $\star\in \TT^{0,\ff}$ or (b) $\star \in \TT^{0}\setminus\TT^{0,\ff}$ and $\{\star1, \star2\}\cap \TT^{0}\setminus\TT^{0,\ff} \neq \emptyset$ }
...
\noi
$\bul$ \underline{\textbf{Case 2}: $\star\in \TT^0\setminus\TT^{0,\ff}$, $(\star1, \star2) \in \TT^{0,\ff} \times \TT^\infty$ or $(\star1, \star2) \in \TT^\infty \times \TT^{0,\ff}$}
...
\noi
$\bul$ \underline{\textbf{Case 3}: $\star \in \TT^{0} \setminus \TT^{0,\ff}$ and $\star1,\star2\in \TT^{0,\ff}$}
...
\begin{align}
\mul_{\bfrak}(\TT) = (-1)^{j+1} \bigg( \prod_{\bul \in \TT} \ind_{A_\bul^c} K_\bul \bigg) \sum_{\star\in\TT^0\setminus\TT^{0,\ff}}\Psi(\star)
\big( 
- \ind_{\TT^{0,\ff} = \{\star1\}} - \ind_{\TT^{0,\ff} = \{\star2\}} + \ind_{\TT^{0,\ff} = \{\star1, \star2\}}
\big), 
\end{align}
which can be rewritten as \eqref{wint0c}.
...
\noi
$\bul$ \underline{\textbf{Case 1}: $\star \in \TT^{0}\setminus\TT^{0,\ff}$ and $\{\star1, \star2\}\cap \TT^{0}\setminus\TT^{0,\ff} \neq \emptyset$ }
...
\noi
$\bul$ \underline{\textbf{Case 2}: $\star\in \TT^{0,\ff}$  }
...
,
\end{align}
which we can rewrite as in  \eqref{wint0c}.
\qedhere
...
\end{proof}
	...
	In this section, we introduce the function spaces over which the solution to \eqref{rkdv} will be built and prove linear estimates associated with the Duhamel integral formulation (Section \ref{sec:fourier_restr}). The crux of the  construction of the solution is then the derivation of specific multilinear estimates, which we reduce to some frequency-restricted estimates (Section \ref{sec:fre}).
	
	
	\subsection{Fourier restriction spaces}\label{sec:fourier_restr}
	
	We first introduce the restriction norm spaces $X^{s,b}_{p,q}$, first introduced by Bourgain \cite{bourg1, bourg2} and Klainerman-Machedon in \cite{KlainermanMachedon} and adapted to the Fourier-Lebesgue setting in \cite{Grunrock_mkdv, HerrGrunrock_FL}.
...
When $p=q$, we may also abbreviate $X^{s,b}_{p,q}$ as $X^{s,b}_p$.
		...
		$$
		\|u\|_{X^{s,b}_{p,q}} = \|\jap{\xi}^s\jap{\tau}^b\mathcal{F}_t\tilde{u}\|_{L^p_\xi L^q_\tau}.
		$$
		
		The case $p=q=2$ corresponds to the classic Bourgain spaces (\cite{bourg1, bourg2}, see also \cite[Chapter 2.6]{tao_book} and \cite[Chapter 7.4]{linares_ponce_book}), while the case $2<p=q<\infty$ was studied by Herr and Grünrock \cite{herrgrunrock_fl}. For convenience, we recall the relevant linear properties. In what follows, $\psi\in C_c^\infty(\R)$ is a smooth cut-off function with $\psi\equiv 1$ over $[-1,1]$ and $\psi\equiv 0$ on $\R\setminus [-2,2]$. Given $\delta>0$, we set $\psi_\delta(t)=\psi(t/\delta)$.
		
		\begin{lemma}\label{lem:linear_estimates_qfinite}
			
			
			{\rm(i)} For $s\in\R$ and $b>1/p'$,  given $u\in X^{s,b}_{p,p}$, we have $u\in C(\R;L^{s,p}(\R))$ with
			\begin{equation}\label{lin_gh}
				\|\psi u\|_{L^\infty_tL_x^{s,p}(\R)}\lesssim \|u\|_{X^{s,b}_{p,p}}.
			\end{equation}
			In particular, $X^{s,b}_{p,p} \hookrightarrow C(\R;L^{s,p}(\R))$.
			
			\smallskip
			
			
			\noi {\rm(ii)} Let $s\in\R$, $b>1/p'$, and $2<p<\infty$. Then, for all $f \in \FL^{s,p}(\R)$, we have
			\begin{align}
				\label{lin1_gh}
			...
			\noi{\rm(iii)} Let $s\in\R$ and $-1/p<b' \le 0 \le b \le 1+b'$. Then, for $F\in X^{s,b'}_{p}$, we have
			
			\noi
			\begin{align}
				\label{lin2a_gh}
				\bigg\| \psi_\dl (t) \int_0^t e^{-(t-t')\dx^3 } F(t') \, dt' \bigg\|_{X^{s,b}_{p} } & \les \dl^{1+b'-b} \| F \|_{X^{s,b'}_{p}}.
			\end{align}
		\end{lemma}
		
		It is important to stress that, in the above result, the case $p=\infty$ is out-of-reach. It turns out that this delicate case can still be addressed by exploiting a different integrability exponent for the time-frequency variable, as the next lemma shows. For a similar result, check \cite[Section 7]{CC24}. 
		
		\begin{lemma}\label{lem:linear_estimates}
			
			
			{\rm(i)} For $s\in\R$ and $b>0$,  given $u\in X^{s,b}_{\infty,1}$, we have $\tilde{u}\in C(\R;L^{s,\infty}(\R))$ with
			\begin{equation}\label{lin}
				\|\jap{\xi}^s\tilde{u}\|_{L^\infty_tL^{\infty}_\xi}\lesssim \|u\|_{X^{s,b}_{\infty,1}}.
			\end{equation}
			In particular,
			\noi
			\begin{align}
	...
	\subsection{Frequency restricted estimates}\label{sec:fre}
	...
	
	{
		...
		\begin{equation}\label{eq:multilinear_geral}
			(\mathcal{N}[u_1,\dots, u_k])^\wedge(t,\xi)=\int_{\xi=\xi_1+\dots+\xi_k} m(\xi_1,\dots,\xi_k)\prod_{j=1}^k\hat{u}_j(\xi_j) d\xi_1\dots d\xi_{k-1}.
		\end{equation}
        
		In this subsection, we reduce the derivation of multilinear estimates for \eqref{eq:multilinear_geral} in Bourgain spaces to some frequency-restricted estimates (\cite{COS}) on the associated resonance function
        $$
        \Phi=-\xi^3+\sum_{j=1}^k\xi_j^3.
        $$
		We first consider the particular case $k=2$. 
		...
		\begin{lemma}\label{lem:reduction_fre_k2}
			...
			\begin{equation}\label{fre}
				\sup_{\xi} \int \frac{|m|\jap{\xi}^s}{\jap{\xi_1}^s\jap{\xi_2}^s} \mathbbm{1}_{|\Phi-\alpha|<M} d\xi_1 \lesssim \jap{\alpha}^\lambda M^\beta.
			\end{equation}
            Take
            $$
            \beta<b<1-\lambda,\quad 0<\tilde{b}<b-\beta.
            $$
Then, for $0<\delta\ll 1$,
\begin{equation}\label{eq:bilin_infty}
    \left\|\psi_\delta(t)\int_0^t e^{-(t-t')\partial_x^3}\mathcal{N}[u_1,u_2](t')dt' \right\|_{X^{s,b}_{\infty, \infty} \cap X^{s,\tilde{b}}_{\infty,1}}  \les \delta^{0^+}\|u_1\|_{X^{s,b}_{\infty, \infty} \cap X^{s,\tilde{b}}_{\infty,1}}\|u_2\|_{X^{s,b}_{\infty, \infty} \cap X^{s,\tilde{b}}_{\infty,1}}.
\end{equation}
			...
			% 	b> \frac{1+\beta}{2},\quad-\wt{b}'>\beta,\quad b-1-\wt{b}'>\beta,\quad -b'>\lambda.
			% \end{equation}
			% Then
			% \begin{align}
			% 	\|N[u_1,u_2] \|_{X^{s,b'}_{\infty, \infty}} & \les \|u_1\|_{X^{s,b}_{\infty, \infty}}\|u_2\|_{X^{s,b}_{\infty, \infty}}, \\
			% 	\|N[u_1,u_2] \|_{X^{s,\wt{b}'}_{\infty, 1}} & \les \|u_1\|_{X^{s,b}_{\infty, \infty} \cap X^{s,\tilde{b}}_{\infty,1}}\|u_2\|_{X^{s,b}_{\infty, \infty} \cap X^{s,\tilde{b}}_{\infty,1}}.
			% \end{align}
		\end{lemma}
		% \begin{remark}
		% 	The conditions \eqref{condb} and \eqref{condb2} can be satisfied whenever $\beta+2\lambda<1$. First, take
		% 	$$
		% 	\frac{1+\beta}{2} < b < 1+b'< 1-\lambda.
		% 	$$
		% 	Next, choose
		% 	$$
		...
		% \end{remark}
		\begin{proof}
Take $b', \tilde{b}'\in \R$ satisfying
			$$
			 b < 1+b'< 1-\lambda,\quad -1+\wt{b}<\wt{b}'<b-\beta-1.
$$
        Applying \eqref{lin2a} and \eqref{lin2b},
        $$
        \left\|\psi_\delta(t)\int_0^t e^{-(t-t')\partial_x^3}N[u_1,u_2](t')dt' \right\|_{X^{s,b}_{\infty, \infty} \cap X^{s,\tilde{b}}_{\infty,1}}  \lesssim (\delta^{1+b'-b} + \delta^{2+\tilde{b}'-b})\left\|N[u_1,u_2] \right\|_{X^{s,b'}_{\infty, \infty} \cap X^{s,\tilde{b}'}_{\infty,1}}.
        $$
        We now control each norm on the r.h.s. using the frequency-restricted estimate \eqref{fre}.
			Set
			$$
			\mathcal{M}=\frac{|m|\jap{\xi}^s}{\jap{\xi_1}^s\jap{\xi_2}^s}.
			$$
            Since 
            $$(\tau-\xi^3) - \Phi =  (\tau_1-\xi_1^3) + (\tau_2-\xi_2^3),
            $$
            we may suppose that $|\tau_1-\xi_1^3|\gtrsim |\Phi-(\tau-\xi^3)|$.
			A direct estimate yields
			\begin{align}
				\|N[u_1,u_2] \|_{X^{s,b'}_{\infty, \infty}} &\lesssim \sup_{\tau,\xi} \int \mathcal{M}\jap{\tau-\xi^3}^{b'}|\hat{u}_1|(\tau_1,\xi_1) |\hat{u}_2|(\tau_2,\xi_2)d\tau_1 d\xi_1 \\& \lesssim \sup_{\tau,\xi} \int \mathcal{M}\frac{\jap{\tau-\xi^3}^{b'}}{\jap{\tau_1-\xi_1^3}^b  } |\hat{u}_2|(\tau_2,\xi_2)d\tau_1d\xi_1 \cdot  \|u_1\|_{X^{s,b}_{\infty,\infty}}\\& \lesssim \sup_{\tau,\xi} \int \mathcal{M}\frac{\jap{\tau-\xi^3}^{b'}}{\jap{\Phi-(\tau-\xi^3)}^b  } |\hat{u}_2|(\tau_2,\xi_2)d\tau_1d\xi_1 \cdot  \|u_1\|_{X^{s,b}_{\infty,\infty}}
                \\& \lesssim \sup_{\alpha,\xi} \int \mathcal{M}\frac{\jap{\alpha}^{b'}}{\jap{\Phi-\alpha}^b  } d\xi_1 \cdot  \|u_1\|_{X^{s,b}_{\infty,\infty}}\|u_2\|_{X^{s,\tilde{b}}_{\infty,1}}
			\end{align}
Using \eqref{fre},
			\begin{align}
				\|N[u_1,u_2] \|_{X^{s,b'}_{\infty, \infty}}\lesssim &\sup_{\alpha,\xi}\sum_{M dyadic} \frac{\jap{\alpha}^{b'}}{M^{b}}\int \mathcal{M}\mathbbm{1}_{|\Phi-\alpha|<M} d\xi_1 \lesssim \sup_{\alpha,\xi}\sum_{M dyadic} \frac{\jap{\alpha}^{b'+\lambda}}{M^{b-\beta}} <\infty,
			\end{align}
			since $b'\le -\lambda$ and $b>\beta$.
            
            For the $X^{s,\tilde{b}}_{\infty, 1}$ estimate, we have
			\begin{equation}
				\|N[u_1,u_2]\|_{X^{s,\tilde{b}'}_{\infty,1}} \lesssim \sup_\xi \int \mathcal{M} \jap{\tau-\xi^3}^{\tilde{b}'}  \hat{u}_1 \hat{u}_2 d\tau_1 d\xi_1.
			\end{equation}
			We split into two cases: if $|\tau-\xi^3|\gtrsim |\Phi|$, then
			\begin{align}
				\|N[u_1,u_2]\|_{X^{s,\tilde{b}'}_{\infty,1}} &\lesssim \sup_\xi \int \mathcal{M} \jap{\Phi}^{\tilde{b}'} \hat{u}_1\hat{u}_2 d\tau_1 d\xi_1 \\&\lesssim \sup_\xi \int \mathcal{M} \jap{\Phi}^{\tilde{b}'} d\xi_1 \cdot  \prod_{j=1}^2\|u_j\|_{X^{s,\tilde{b}}_{\infty,1}} \\&\lesssim \sum_{M dyadic} M^{\tilde{b}'} \sup_\xi \int \mathcal{M} \mathbbm{1}_{|\Phi|<M} d\xi_1 \cdot \prod_{j=1}^2\|u_j\|_{X^{s,\tilde{b}}_{\infty,1}}  \lesssim \prod_{j=1}^2\|u_j\|_{X^{s,\tilde{b}}_{\infty,1}} ,
			\end{align}
			by \eqref{fre} and $\tilde{b}'+\beta<b-1<0$. On the other hand, if $|\tau-\xi^3|\ll |\Phi|$, we may assume that $|\tau_1-\xi_1^3|\gtrsim |\Phi|$. Then
			\begin{align}
				\|N[u_1,u_2]\|_{X^{s,\tilde{b}'}_{\infty,1}} &\lesssim \sup_\xi \int \mathcal{M} \frac{\jap{\tau-\xi^3}^{\tilde{b}'}}{\jap{\tau_1-\xi_1^3}^b} \jap{\tau_1-\xi_1^3}^b\hat{u}_1  \hat{u}_2 d\tau d\tau_2 d\xi_1 \\&\lesssim \sup_\xi \int \frac{\mathcal{M}}{\jap{\Phi}^{b-\tilde{b}'-1^+}\jap{\tau-\xi^3}^{1+} } d\tau d\xi_1 \cdot \|u_1\|_{X^{s,b}_{\infty,\infty}} \|u_2\|_{X^{s,\tilde{b}}_{\infty,1}} \\&\lesssim \sum_{M dyadic} M^{1^+-b+\tilde{b}'} \sup_\xi \int \mathcal{M} \mathbbm{1}_{|\Phi|<M} d\xi_1\cdot \|u_1\|_{X^{s,b}_{\infty,\infty}}  \|u_2\|_{X^{s,\tilde{b}}_{\infty,1}}\\& \lesssim\|u_1\|_{X^{s,b}_{\infty,\infty}}  \|u_2\|_{X^{s,\tilde{b}}_{\infty,1}}
			\end{align}
			again by \eqref{fre} and the fact that $b-\tilde{b}'-1-\beta>0$.
		\end{proof}
		
		The nest lemma focuses on the derivation of bilinear estimates in the standard Bourgain spaces $X^{s,b}_{2,2}$.
		
		\begin{lemma}[{\cite[Lemma 3.1]{CLS}}]\label{lem:fre_bi}
			Suppose that, for some $0<\beta<1$, either:
			\begin{itemize}
				\item for all $j\in\{\emptyset,1,2\}$\footnote{The empty set symbol $\emptyset$ here corresponds to the frequency component $\xi$, without indices.}, $l\neq j$, $\alpha\in \R$ and $1<M\lesssim |\alpha|$,
				\begin{equation}\label{eq:frequad}
					\sup_{\xi_j}\int_\Gamma \frac{|m|^2\jap{\xi}^{2s}}{\jap{\xi_{1}}^{2s_1}\jap{\xi_{2}}^{2s_2}}\mathbbm{1}_{|\Phi-\alpha|<M} d\xi_{l} \lesssim \jap{\alpha}^\beta M^\beta.
				\end{equation}
				\item for all $\alpha\in \R$ and $M>1$, there exists $j\in\{1,2\}$ such that, with $l\neq j$
				\begin{equation}\label{eq:frequad2}
					\sup_{\xi_j}\int_\Gamma \frac{|m|^2\jap{\xi}^{2s}}{\jap{\xi_{1}}^{2s_1}\jap{\xi_{2}}^{2s_2}}\mathbbm{1}_{|\Phi-\alpha|<M} d\xi_{l}  \lesssim \jap{\alpha} M^\beta.
				\end{equation}
				\item for all $\alpha\in \R$ and $M>1$,
				\begin{equation}\label{eq:frequad3}
					\sup_{\xi}\int_\Gamma \frac{|m|^2\jap{\xi}^{2s}}{\jap{\xi_{1}}^{2s_1}\jap{\xi_{2}}^{2s_2}}\mathbbm{1}_{|\Phi-\alpha|<M} d\xi_{1}  \lesssim \jap{\alpha}^\beta M.
				\end{equation}
			\end{itemize}
			%This remains true if one replaces $(\xi,\beta)$ with $(\xi_1,\beta_1)$.
			Then, for $b=(1/2)^+$ and $0<\delta\ll 1$,
			\begin{equation}
				\left\|\psi_\delta(t)\int_0^te^{-(t-t')\partial_x^3} N[u_1,u_2](t')dt'\right\|_{X^{s,b}_{2}} \lesssim \delta^{0^+}\|u_1\|_{X^{s,b}_{2}} \|u_1\|_{X^{s,b}_{2}}.
			\end{equation}
		\end{lemma}
		
		We now consider the case of trilinear (or higher-order) operators of the form \eqref{eq:multilinear_geral}.
		
		\begin{lemma}\label{lem:reduction_fre_kge3}
			Fix $2\le \ell\le k$. Suppose that there exist $\beta<1$ and multipliers $\mathcal{M}:\R^k\to \R^+$ and $\mathcal{M}':\R^{k-\ell}\to \R^+$ such that
			\begin{equation}
				\frac{|m|\jap{\xi}^s}{\prod_{j=1}^k\jap{\xi_k}^s} \lesssim \mathcal{M}(\xi_1,\dots, \xi_{k})\mathcal{M}'(\xi_{\ell+1},\dots, \xi_{{k}}),
			\end{equation}
			\begin{equation}
				\int \mathcal{M}'(\xi_{\ell+1},\dots, \xi_{{k}}) d\xi_{\ell+1}\dots d\xi_k <\infty
			\end{equation}
			and
			\begin{equation}\label{fre_kge3}
				\sup_{\xi, \xi_{\ell+1},\dots \xi_{k},\alpha} \int \mathcal{M}(\xi_1,\dots,\xi_k) \mathbbm{1}_{|\Phi-\alpha|<M} d\xi_1\dots d\xi_{\ell-1} \lesssim  M^\beta.
			\end{equation}
			Take
            $$
            1-\frac{1-\beta}{\ell}<b<1,\quad 0<\tilde{b}<b-\beta.
            $$
Then 
			%Suppose that there exist $\beta,\lambda\ge 0$ with $\beta+\lambda<1$ such that, for any $\alpha\in \R$ and $M>1$,
			%\begin{equation}\label{fre}
			...
			%\end{equation}
			% Take $b,b',\tilde{b},\tilde{b}'\in \R$ satisfying
			% \begin{equation}\label{condb_a}
			% 	1-b+b'\ge 0,\quad -1\le \wt{b}'\le 0\quad 1-\wt{b}+\wt{b}'\ge 0,\quad 2-b+\wt{b}'\ge 0,\quad \wt{b}> 0,
			% \end{equation}
			% \begin{equation}\label{condb2_a}
			% 	\ell b-\ell + 1> \beta,\quad-\wt{b}'>\beta,\quad b-1-\wt{b}'>\beta,\quad b'<0.
			% \end{equation}
			% Then
			\begin{align}
				\left\|\psi_\delta(t)\int_0^te^{-(t-t')\partial_x^3}N[u_1,\dots, u_k](s)ds \right\|_{X^{s,b}_{\infty, \infty}\cap X^{s,\tilde{b}}_{\infty, 1}} & \les \delta^{0^+}\prod_{j=1}^k\|u_j\|_{X^{s,b}_{\infty, \infty} \cap X^{s,\tilde{b}}_{\infty,1}}.
			\end{align}
			
		\end{lemma}
		% \begin{remark}
		% 	Once again, the conditions \eqref{condb_a} and \eqref{condb2_a} can be satisfied whenever $\beta<1$: it suffices to consider
		% 	$$
		% 	1-\frac{1-\beta}{\ell} < b < 1+b'< 1.
		% 	$$
		% 	and then
		% 	$$
		% 	-1<-1+\wt{b}<\wt{b}'<b-\beta-1.
		...
		\begin{proof}
Choose $b', \tilde{b}'\in \R$ such that
$$
			 b < 1+b'< 1,\quad -1+\wt{b}<\wt{b}'<b-\beta-1.
			$$
        By Lemma \ref{lem:linear_estimates}, 
        \begin{align}
				\left\|\psi_\delta(t)\int_0^te^{-(t-t')\partial_x^3}N[u_1,\dots, u_k](s)ds \right\|_{X^{s,b}_{\infty, \infty}} & \les \delta^{0^+}\left\|N[u_1,\dots, u_k] \right\|_{X^{s,b'}_{\infty, \infty}\cap X^{s,\tilde{b}'}_{\infty, 1}}, \\
				\left\|\psi_\delta(t)\int_0^te^{-(t-t')\partial_x^3}N[u_1,\dots, u_k](s)ds \right\|_{X^{s,\wt{b}}_{\infty, 1}} & \les \delta^{0^+}\left\|N[u_1,\dots, u_k] \right\|_{X^{s,\tilde{b}'}_{\infty, 1}}.
			\end{align}
            As such, it suffices to prove that
\begin{align}
				\left\|N[u_1,\dots, u_k] \right\|_{X^{s,b'}_{\infty, \infty}} & \les \prod_{j=1}^k\|u_j\|_{X^{s,b}_{\infty, \infty}},\label{eq:infty_por_infty} \\
				\left\|N[u_1,\dots, u_k] \right\|_{X^{s,\wt{b}'}_{\infty, 1}}  & \les \prod_{j=1}^k\|u_j\|_{X^{s,b}_{\infty, \infty} \cap X^{s,\tilde{b}}_{\infty,1}}.\label{eq:1_por_infty}
			\end{align}
			We begin with \eqref{eq:infty_por_infty}. Observe that, since $b'<0$ and $\tilde{b}>0$,
			\begin{align*}
				&\|N[u_1,\dots, u_k] \|_{X^{s,b'}_{\infty, \infty}} \lesssim \|N[u_1,\dots, u_k] \|_{X^{s,0}_{\infty, \infty}} \\\lesssim \ &\sup_{\tau, \xi} \int \frac{|m|\jap{\xi}^s}{\prod_{j=1}^k\jap{\xi_j}^s} \frac{1}{\prod_{j=1}^\ell \jap{\tau_j-\xi_j^3}^b}\prod_{j=\ell+1}^k \jap{\xi_j}^s|\hat{u}_j| d\tau_2\dots d\tau_k d\xi_2 \dots d\xi_k\cdot \prod_{j=1}^\ell \|u_j\|_{X_{\infty,\infty}^{s,b}}
				\\\lesssim \ &\sup_{\substack{\tau, \tau_{\ell+1},\dots,\tau_k\\ \xi, \xi_{\ell+1}, \xi_k}} \left(\int \mathcal{M} \frac{1}{\prod_{j=1}^\ell \jap{\tau_j-\xi_j^3}^b}d\tau_2\dots d\tau_\ell d\xi_2\dots d\xi_\ell\right) \times\\ & \qquad \qquad \qquad \times \int \mathcal{M}'\prod_{j=\ell+1}^k \jap{\xi_j}^s|\hat{u}_j| d\tau_{\ell+1}\dots d\tau_k d\xi_{\ell+1} \dots d\xi_k\cdot \prod_{j=1}^\ell \|u_j\|_{X_{\infty,\infty}^{s,b}}
				\\\lesssim \ &\sup_{\substack{\tau, \tau_{\ell+1},\dots,\tau_k\\ \xi, \xi_{\ell+1}, \xi_k}} \left(\int \mathcal{M} \frac{1}{ \jap{\Phi + \tau-\xi^3 - \sum_{j=\ell+1}^k (\tau_j-\xi_j^3)}^{\ell b - \ell +1}} d\xi_2\dots d\xi_\ell\right) \times\\ & \qquad \qquad \qquad \times \int  \mathcal{M}' d\tau_{\ell+1}\dots  d\xi_{\ell+1}\cdot \prod_{j=\ell+1}^k \|u_j\|_{X_{\infty,1}^{s,0}}\cdot \prod_{j=1}^\ell \|u_j\|_{X_{\infty,\infty}^{s,b}}
				\\\lesssim \ &\sup_{{\alpha,  \xi, \xi_{\ell+1}, \xi_k}} \left(\int \mathcal{M} \frac{1}{ \jap{\Phi -\alpha}^{\ell b - \ell +1}} d\xi_2\dots d\xi_\ell\right) \cdot \prod_{j=\ell+1}^k \|u_j\|_{X_{\infty,1}^{s,\tilde{b}}}\cdot \prod_{j=1}^\ell \|u_j\|_{X_{\infty,\infty}^{s,b}}
			\end{align*}
			To control the last integral, decompose dyadically in $\Phi-\alpha$ and use \eqref{fre_kge3},
			\begin{align*}
				&\sup_{{\alpha,  \xi, \xi_{\ell+1}, \xi_k}} \int \mathcal{M} \frac{1}{ \jap{\Phi -\alpha}^{\ell b - \ell +1}} d\xi_2\dots d\xi_\ell \\\lesssim\ & \sum_{M dyadic} M^{-\ell b +\ell-1} \sup_{{\alpha,  \xi, \xi_{\ell+1}, \xi_k}} \int \mathcal{M} \fia d\xi_2\dots d\xi_\ell \\\lesssim\ & \sum_{M dyadic} M^{-\ell b +\ell-1+\beta} <\infty. 
			\end{align*}
			This concludes the proof of the first estimate. For \eqref{eq:1_por_infty}, one proceeds as in the case $k=2$, we omit the details.
			%Set
			%$$
		...
		In the context of $X^{s,b}_{2,2}$ spaces, the reduction of multilinear bounds to frequency-restricted estimates was shown by the second author, together with Oliveira and Silva.
		
		
		\begin{lemma}[{\cite[Lemma 2]{COS}}]\label{lem:fre_tri}
			Suppose that, for some $0<\beta<1$, there exist a proper nonempty subset $A\subset \{\emptyset,1,\dots, m\}$ and multipliers $\mathcal{M}_1,\mathcal{M}_2\ge 0$ such that
			$$
			\sup_{\xi_{j\in A}} \int_{\Gamma} \mathcal{M}_1 \mathbbm{1}_{|\Psi-\alpha|<M} d\xi_{j\notin A} + \sup_{\xi_ j\notin A} \int_{\Gamma} \mathcal{M}_2 \mathbbm{1}_{|\Psi-\alpha|<M} d\xi_{j\in A} \lesssim M^\beta
			$$
			and
			$$
			\mathcal{M}_1\mathcal{M}_2=\frac{|m|^2\jap{\xi}^{2s}}{\prod_{j=1}^k\jap{\xi_{j}}^{2s_j}}.
			$$
            Then, for $b=(1/2)^+$ and $0<\delta\ll 1$,
            \begin{align}
				\left\|\psi_\delta(t)\int_0^te^{-(t-t')\partial_x^3}N[u_1,\dots, u_k](s)ds \right\|_{X^{s,b}_{2}} & \les \delta^{0^+}\prod_{j=1}^k\|u_j\|_{X^{s,b}_{2}}.
			\end{align}
		\end{lemma}
		
		We conclude this subsection with a couple of technical lemmas which are useful in the derivation of frequency-restricted estimates. The first claims that, as long as there is some frequency weights to spare, one may consider $\beta=1$ in Lemmas \ref{lem:reduction_fre_k2} to \ref{lem:fre_tri}.
		...
		\begin{lemma}\label{lem:quadraticas}
		...
		\andreia{Start by adding relevant auxiliary results that may be needed, such as Morse's Lemma} \simao{Yes, at the end of this subsection.}
...
\section{Estimating time-integral terms}\label{sec:multi}

This section is dedicated to 
estimating the nonlinear terms $\NN_\l(\TT;w)$  appearing in the gauged equation \eqref{z1} for $z$. 
...
$b=1-,\ \tilde{b}=0+$, and $\ta>0$ such that
	\begin{align}
		\| \NN_\l(\TT; u_1,\dots, u_{j+1})\|_{X^{s,b}_{\infty, \infty}\cap X^{s,\wt b}_{\infty, 1}} \le T^\ta C^j\prod_{k=1}^{j+1}\|u_k\|_{X^{s,b}_{\infty, \infty}\cap X^{s,\tilde{b}}_{\infty,1}}, \label{eq:Linftybourg1} 
%         \\
...
Let $s>-\frac34$, $j \in\N$, $\TT \in \Tr_j$, and $\NN_\l(\TT)$, $\l=1,\dots,4$, as in \eqref{wint0b}.
	...
	\begin{align}\label{eq:bourg_bi_l2}
		\| \NN_\l(\TT; u_1,\dots, u_{j+1})\|_{X^{s,b}_{2}}  \le T^\ta C^j\prod_{k=1}^{j+1}\|u_k\|_{X^{s,b}_{2}}
	\end{align}
    for some constant $C>0$ independent of $j$ and $\TT$.
\end{proposition}
...
there exist $b=\frac{1}{p'}+$ and $\ta>0$ such that
	\begin{align}\label{eq:bourg_bi_lp}
		\| \NN_\l(\TT; u_1,\dots, u_{j+1})\|_{X^{s,b}_{p}}\le T^\ta C^j\prod_{k=1}^{j+1}\|u_k\|_{X^{s,b}_{p}}
	\end{align}
    for some constant $C>0$ independent of $j$ and $\TT$.
\end{proposition}
...
To show \eqref{eq:multi_mais_forte}, we then employ the frequency-restricted machinery developed in Section \ref{sec:fre}.
%
...
Given $j\in\N$ and $\TT\in \Tr_j$, from the definition of $\NN_\l(\TT)$  in \eqref{wint0b}, we note that these contributions can be written in the form $\NN(u_1, \ldots, u_{j+1})$ as 
in \eqref{eq:multilinear_geral}
with multipliers $m = \mul_\ll(\TT)$ (see \eqref{wint0c}). Thus, the estimates in \eqref{eq:Linftybourg1} and \eqref{eq:Linftybourg2} follow from Lemma~\ref{lem:reduction_fre_k2} and Lemma~\ref{lem:reduction_fre_kge3}, 
once we show the following frequency-restricted estimates:
\begin{align}
\label{eq:fre_geral}
	\sup_{\alpha, \xi} \int_{\G(\TT)} \jap{\xi}^s  {\mul}_\gfrak(\TT)  \ind_{|\Psi(\TT) - \al| < M} \prod_{\bul\in \TT^\infty} \frac{1}{\jap{\xi_\bul}^s} d\vec\xi 
&
\le 
C^j M^{1-}, 
\\
\label{eq:fre_geral2}
\sup_{\alpha, \xi} \int_{\G(\TT)} \jap{\xi}^s  {\mul}_\bfrak(\TT) 
 \ind_{|\Psi(\TT) - \al| < M} \prod_{\bul\in \TT^\infty} \frac{1}{\jap{\xi_\bul}^s} d\vec\xi 
&
\le 
C^j M^{1-}, 
\end{align}
...
From Corollary~\ref{COR:types}, it suffices to show \eqref{eq:fre_geral} when $\TT$ is a tree of Types I-IV, and \eqref{eq:fre_geral2} for $\TT$ is of Types V-VI.
...
\smallskip 
\noi
$\bul$ \textbf{Type I: $\TT=\<1>$ }


From \eqref{wint0c} with $\TT= \<1>$, we have
...
If (i) holds, the multiplier in \eqref{type1} is bounded since all frequencies all small, and the integral in \eqref{eq:fre_geral} is uniformly bounded in $M$. 
...
\end{align}
 Thus, from the change of variables $\xi_1 \mapsto \phi $, we obtain
	\begin{align*}
\int_{\G(\TT)} \frac{ \jb{\xi}^{s+ } |\xi| }{\jb{\xi_1}^s \jb{\xi_2}^s } \mul_{\gfrak} (\TT) \ind_{|\Psi(\TT) - \al| < M }
 &
\les 
...
\les 
 \int \frac{\fia }{ \jap{\xi}^{1 + s- }} d\phi \lesssim M, 
	\end{align*}
	for $s>-1$ and \eqref{eq:fre_geral} follows from Lemma~\ref{lem:eta1}.
...
\medskip 
\noi
$\bul$ \textbf{Type $\II$: $\TT^{0,\ff} = \{\star\}$ and $\star\ne \emptyset$}

From \eqref{wint0c}, we are only interested in the contribution with $\ind_{A_\star}$, and we denote that portion of the multiplier by $\mul_{\gfrak}^{\star}(\TT)$. 
Then, we have $j\ge2$, $\TT^0 = \{ a^{1}, \ldots, a^{j} \}$ with
\begin{align}
\label{II0}
\emptyset = a^{1} \le a^{2} \le \cdots \le a^{j} = \star, 
\end{align}
where $a^{k-1}$ is the parent of $a^{k}$ in $\TT$, $k=2, \ldots, j$, and $\TT^\infty = \{ \sbf(a^{2}), \ldots, \sbf(a^{j}), \star1, \star2  \}$, where $\sbf(\bul)$ denotes the sibling node of $\bul$. 
...
\begin{split}
&
\int_{\G(\TT)} \jb{\xi}^s \big| \mul_{\gfrak}^\star(\TT) \big| \ind_{|\Psi(\TT)-\al| \le  M} \prod_{\bul\in\TT^\infty} \frac{1}{\jb{\xi_\bul}^s} d\vec\xi 
\\
&
\le C^j \bigg( \prod_{k=2}^{j-1} \int \frac{1}{ \jb{\xi_{a^{k}}}^{s+2} } d\xi_{a^k} \bigg) \sup_{\substack{\xi_{a^k}, \xi_{\sbf(a^k)}, \xi_{\sbf(a^{j})} \\ k=2, \ldots, j-1}}
\int \frac{\ind_{A_\star} \ind_{A^c_{\pb(\star)}}}{ \jb{\xi_{\star1} }^s \jb{\xi_{\star2}}^s \jb{\xi_\star}^{(s\land0)+1}  } \ind_{|\Psi(\TT) - \al| \le M} d\xi_{\star1} d\xi_{\star} 
\\
&
\le C^j 
\sup_{\substack{\xi_{a^k}, \xi_{\sbf(a^k)}, \xi_{\sbf(a^{j})} \\ k=2, \ldots, j-1}}
\int \frac{\ind_{A_\star} \ind_{|\xi_\star|\ge 1}}{ \jb{\xi_{\star1} }^s \jb{\xi_{\star2}}^s \jb{\xi_\star}^{(s\land0)+1}  } \ind_{|\Psi(\TT) - \al| \le M} d\xi_{\star1} d\xi_{\star} , 
\end{split}
\end{align}
for some constant $C>0$, given that $s+2>1 \iff s>-1$. 
To estimate the remaining integral, suppose that
$|\xi_{\star1} | \ge |\xi_{\star2}|$.
If $|\xi_{\star1}| \les 1$, then the integral on the RHS of \eqref{IIbb} is trivially bounded since all frequencies are $\les 1$. Otherwise, from the restriction to $A_\star$ (see \eqref{defA}), we have 
\begin{align}
\big| |\xi_{\star1} |  - |\xi_{\star2}| \big| \sim |\xi_{\star1}| \sim |\xi_\star| \ges 1. 
\end{align}
Then, for fixed $\xi_\star$, we have $|\partial_{\xi_{\star1}} \Psi(\star) | = 3| \xi_\star  (\xi_{\star2} - \xi_{\star1})| \sim |\xi_{\star}|^2$. Thus, performing the change of variables $\xi_{\star1} \mapsto \phi = \Psi(\star)$, gives
\begin{align}\label{IIb}
\begin{split}
\int \frac{\ind_{A_\star} \ind_{A^c_{\pb(\star)}}}{ \jb{\xi_{\star1} }^s \jb{\xi_{\star2}}^s \jb{\xi_\star}^{(s\land0)+1}  } \ind_{|\Psi(\TT) - \al| \le M} d\xi_{\star1} d\xi_{\star}
&
\sim
\int \frac{\ind_{A_\star} \ind_{A^c_{\pb(\star)}}}{ \jb{\xi_\star}^{(3s\land0)+1}  } \ind_{|\Psi(\TT) - \al| \le M} d\xi_{\star1} d\xi_{\star}
\\
&
\les 
\int \frac{\ind_{|\phi - \wt\al| \le M} }{ \jb{\xi_\star}^{(3s\land0)+3}  } d\phi d\xi_{\star} \les M,
\end{split}
\end{align}
where $\wt\al = \Psi(\TT) - \Psi(\star) = \sum_{k=1}^{j-1} \Psi(a^{k})$ which does not depend on $\xi_{\star}$, given that $s>-\frac23$. 
Then,  \eqref{eq:fre_geral} follows from \eqref{IIbb}, \eqref{IIb}, and Lemma~\ref{lem:eta1}.
...
\noi
\underline{Case 1: $|\xi_{11}| \ll |\xi_2|$}

In this case, we have $|\xi| \sim |\xi_2| \gg  |\xi_{11}| \sim |\xi_{12}| \ges |\xi_1|$, which implies
\begin{align}\label{IIIb}
\begin{split}
\big| 
\partial_{\xi_{2}}  (\xi^3 - \xi_{11}^3 - (\xi-\xi_{11}-\xi_2)^3 - \xi_2^3)
\big|
&
...
\end{split}
\end{align}
Thus, we perform the change of variables $\xi_2 \mapsto \Phi = \Psi(1) + \Psi(11) = \xi^3 - \xi^3_{11} - (\xi-\xi_{11}-\xi_2)^3 - \xi_2^3$, to obtain
\begin{align}
&
\int \frac{ \ind_{A_1^c} \ind_{\xi=\xi_1+\xi_2} \jb{\xi}^{(s\lor 0)+1 + }  }{ \jb{\xi_{11}}^{s+1  } \jb{\xi_{12}}^{s+1} \jb{\xi_{2}}^s }
\ind_{|\Psi(1) + \Psi(11) - \al| \le M}
d\xi_{11} d\xi_2
\\
&
\sim \int \frac{ \jb{\xi}^{(s\lor0)  - s +  1 + } }{ \jb{\xi_{11}}^{2s+2} |\xi|^2  } \ind_{|\Phi - \al|\le M } d\Phi d\xi_2 
\\
&
\les \int \frac{\ind_{|\Phi - \al|\le M }}{ \jb{\xi_2}^{3s+3- (s \lor 0) - } } d\Phi d\xi_2 \les M, 
\end{align}
given that $s>-\frac23$, with \eqref{IIIa} following by Lemma~\ref{lem:eta1}. 



\noi
\underline{Case 2: $|\xi_{11}| \ges |\xi_2|$ and $\big| |\xi_{11}| - |\xi_2| \big| \sim |\xi_{11}|$}
...
\end{align}
which gives to the following estimate after the change of variables $\xi_{2} \mapsto \phi = \Psi(1) + \Psi(11)$:
\begin{align}
\int \frac{\ind_{A_1^c} \ind_{\xi=\xi_1+\xi_2} \jb{\xi}^{(s \lor 0 )+ 1+}}{ \jb{\xi_{11}}^{s+1} \jb{\xi_{12}}^{s+1} \jb{\xi_2}^s}
\ind_{|\Psi(1) + \Psi(11) - \al| \le M}
d\xi_{11} d\xi_2
&
\les \int \frac{\jb{\xi}^{(s\lor0) - s + 1 +} }{ \jb{\xi_{11}}^{2s+4}  } \ind_{|\phi - \al | \le M } d\phi d\xi_2
\\
&
\les \int \frac{\ind_{|\phi - \al|\le M }}{ \jb{\xi_2}^{3s + 3 -(s \lor 0)-} } d\phi d\xi_2 \les M, 
\end{align}
...
\noi
\underline{Case 3:  $\big| |\xi_{11}| - |\xi_2| \big| \ll |\xi_{11}| \sim |\xi_2|$}
...
Looking at the Hessian $D^2 \Phi(1, p_{11}, p_2)$, we note that $\det (D^2 \Phi) = -36 (p_{11} p_{12} + p_{11} p_2 + p_{12} p_2) \ne0 $ on the critical points above. Thus, applying Morse's Lemma (Lemma~\ref{LEM:morse}) around each point implies the existence of a local change of coordinates $(p_{11},p_2) \mapsto (q_{11}, q_2)$ such that $|q_{11}|, |q_2| \les 1$ and 
\begin{align}
\Phi(1, p_{11},p_2) = \Phi(1, c) + q_{11}^2 + q_2^2, 
\end{align}
for $c\in\{(1,-1), (-1,1), (\frac13, \frac13)\}$. For the critical point $(p_{11}, p_2) =(1,1)$, instead we consider $\wt\Phi(1, p_{12}, p_2) = \Phi(1, 1-p_{12}-p_2, p_2)$ where this corresponds to the critical point $(p_{12}, p_2) = (-1,1)$, and proceeding as above, there exists a local change of variables that guarantees that
\begin{align}
\wt\Phi(1, p_{12}, p_2) = q_{12}^2 + q_2^2, 
\end{align}
where we note that $\wt\Phi(1,-1,1)=0$. 
Consequently, applying the change of variables $(\xi_{11},\xi_2) \mapsto (p_{11}, p_2)$ followed by $(p_{11}, p_2) \mapsto (q_{11}, q_2)$ (with suitable change around $(1,1)$), together with \eqref{eq:posdef}, we obtain
\begin{align}
&
\int \frac{\ind_{A_1^c} \ind_{\xi=\xi_1+\xi_2} \jb{\xi}^{(s \lor 0 )+ 1}}{ \jb{\xi_{11}}^{s+1} \jb{\xi_{12}}^{s+1} \jb{\xi_2}^s}
\ind_{|\Psi(1) + \Psi(11) - \al| \le M}
...
&
\les 
\jb{\xi}^{(s\lor 0) - 3 s - 1   } \int \ind_{|\xi^3 \Phi(1, p_{11}, p_2) - \al| \le M} \xi^2 dp_{11} dp_2
\\
&
\les 
\jb{\xi}^{(s\lor 0)- 3s + 1  } \int_{|q_{11}|, |q_2| \les 1} \ind_{| \xi^3( \Phi(1,c) + q_{11}^2 +  q_2^2 )- \al | \le M  } dq_{11} d q_2
\\
&
\les 
\jb{\xi}^{(s\lor 0)- 3s -2   } M^{1-} \les M^{1-}
\end{align}
since $\Psi(1) + \Psi(11) = \Phi(\xi, \xi_{11},\xi_2) = \xi^3 \Phi(1, p_{11}, p_2)$ and for $s>-\frac23$. 


\smallskip 

\noi
$\bul$ \textbf{Types IV and VI: }

Let $j\in \N$ and $\TT\in \Tr_{j+2}$ with $\star\in \TT^0$ and $\TT^{0,\ff} = \{\star1, \star2\}$. Then, we can write $\TT^0 = \{b^1, \ldots, b^j, b^j1, b^j2\}$ with
\begin{align}
\label{II1}
    \emptyset = b^1 \le b^2 \le \cdots \le b^j = \star, 
    \quad 
    b^j_1 = \star1, 
    \quad 
    \text{and}
    ...
    b^j_2 = \star2, 
\end{align}
where $b^{k-1}$ is the parent of the node $b^k$ in $\TT$, $k=2, \ldots, j$, and $\TT^\infty = \{ \sbf(b^2), \ldots, \sbf(b^j), \star11, \star12, \star21, \star22 \}$, with $\sbf(\bul)$ denoting the sibling of $\bul$. 
...
\end{align}
where $\wt\TT = \<33>$. 
To show \eqref{eq:frequadd}, we consider the different cases based on the relation between the pairs of frequencies $|\xi_{11}|\sim |\xi_{12}|$ and $|\xi_{21}| \sim |\xi_{22}|$. By symmetry, we assume that $|\xi_{11}| \ges |\xi_{21}|$.

\smallskip
\noi
\underline{Case 1: $|\xi_{11}| \gg |\xi_{21}|$}

Fixing $\xi_{11}$ and $\xi_{21}$, we perform the change of variables $\xi_1 \mapsto \phi = \Phi(\wt\TT) = \xi^3 - \xi_{11}^3 - (\xi_1-\xi_{11})^3 - \xi_{21}^3 - (\xi-\xi_1-\xi_{21})^3$, noting that
\begin{align}\label{CV1}
|\partial_{\xi_1} \phi |
& =
| -3 (\xi_1 - \xi_{11})^2 + 3 ( \xi-\xi_1 - \xi_{21})^2|
\\
&
= 3
\big| |\xi_{22}| - |\xi_{12}| \big| \cdot   \big | | \xi_{22}| + |\xi_{12}| \big|
\\
&
\sim |\xi_{11}|^2, 
\end{align}
...
& 
\int_{\G(\wt\TT)}
\frac{\jb{\xi}^{s\lor 0 + 1 } \jb{\xi_{11}}^{0+}  }{\jb{\xi_{11}}^{2s+2} \jb{\xi_{21}}^{2s+2} } \ind_{| \Psi(\wt\TT) - \al | \le M} d\xi_1 d\xi_{11} d\xi_{21}
\\
&
\les \int \frac{1}{\jb{\xi_{11}}^{2s+3 - s\lor 0 -} \jb{\xi_{21}}^{2s+2}} \ind_{| \phi - \al  | \le M} d\phi d\xi_{11} d\xi_{21} 
\\
&
\les M 
\end{align}
...
\smallskip
\noi
\underline{Case 2: $|\xi_{11}| \sim |\xi_{21}|$ and $\big| |\xi_{12}| - |\xi_{22}| \big| \lor \big| |\xi_{11}| - |\xi_{21}| \big| \lor \big| |\xi_{11}| - |\xi_{22}| \big| \lor \big| |\xi_{12}| - |\xi_{21}| \big|\ges |\xi| $}
...
\smallskip
\noi
\underline{Case 3: $|\xi_{11}| \sim |\xi_{21}|$ and $\big| |\xi_{12}| - |\xi_{22}| \big| \lor \big| |\xi_{11}| - |\xi_{21}| \big| \lor \big| |\xi_{11}| - |\xi_{22}| \big| \lor \big| |\xi_{12}| - |\xi_{21}| \big|\ll |\xi| $}


If $\xi_{11} \xi_{12}, \xi_{21}\xi_{22}<0$, then $|\xi| \les |\xi_1|, |\xi_2|\ll |\xi_{11}|$. Assuming that $\xi_{11},\xi_{21}>0 > \xi_{12}, \xi_{22} $, we have that
\begin{align}
    |\xi_{11} + \xi_{22} | = \big| |\xi_{11}| - |\xi_{22}| \big| 
    \ll |\xi| \sim |\xi_{12} + \xi_{21}| = \big| |\xi_{12}| - |\xi_{21}| \big|, 
\end{align}
which cannot happen.
The same observation applies if $\xi_{11}\xi_{12}, \xi_{21}\xi_{22}>0$ but $\xi_{11}\xi_{21}<0$. 
Thus, we must have at most one frequency with a different sign, and $|\xi| \sim |\xi_{11}|$. 
...
\begin{align}
   |\xi|\sim |\xi_{11}| \sim |\xi_{12}| \sim |\xi_{21}| \sim |\xi_{22}| \gg 
    \big| |\xi_{k_1}| - |\xi_{k_2}| \big|, \quad k_1,k_2 \in \{11,12,21,22\}, \ k_1 \ne k_2, 
\end{align}
and we cannot proceed as in Cases 2-3. Instead we consider the normalization $p_\bul = \frac{\xi_\bul}{\xi}$ for $\bul\in\{11, 12, 21, 22\}$, and 
\begin{align}
    \Psi(\wt\TT) 
    &
    ...
    \\
    & = : \xi^3 P(p_{12}, p_{21}, p_{22} ).
\end{align}
From the frequency assumptions, we have $1\sim |1 - p_{12}-p_{21} - p_{22}| \sim |p_{12}| \sim |p_{21} |\sim |p_{22}|$, and fixing $p_{12}$, the variables $(p_{21}, p_{22})$ are around one of the stationary points of $P(p_{12}, p_{21}, p_{22})$:
\begin{align}\label{crit}
    (1-p_{12}, 1-p_{12}), \quad (\pm(1-p_{12}), \mp(1-p_{12})), \quad \tfrac13(1-p_{12}, 1-p_{12}). 
\end{align}
Calculating the Hessian $D^2 P(p_{12}, p_{21}, p_{22})$, we note that 
\begin{align}
 \det(D^2 P)(p_{12}, p_{21}, p_{22}) 
 &
 = 36(1-p_{12}-p_{21})(1-p_{12}-p_{22}) - 36(1-p_{12}-p_{21}-p_{22})^2
 \\
 &
 = 36 (p_{11}+p_{22})(p_{11} + p_{21}) - 36 p_{11}^2
 \\
 &
 = 36 (p_{11} p_{21} + p_{11} p_{22} + p_{21} p_{22} ), 
\end{align}
from which we note that $|\det(D^2P)(p_{12}, p_{21}, p_{22})| = (1-p_{12})^2 \ne 0$ at the critical points, since $p\ne 1$. Note that $p_{12}=1$ would imply $p_{21}=p_{22} = 0$, breaking the size relation. 
Consequently, by Morse's Lemma (Lemma~\ref{LEM:morse}), there exists a local change of coordinates $( p_{21}, p_{22}) \mapsto (q_{21}, q_{22})$ such that $|q_{21}|, |q_{22}| \les 1 $ and 
\begin{align}
P(p_{12},p_{21}, p_{22})
 = P(p_{12}, c) + \ld_{21} q_{21}^2 + \ld_{22} q_{22}^2,
\end{align}
for $c$ being one of critical points in \eqref{crit} and $\ld_{21}, \ld_{22} \in \{-1,1\}$ depending on the sign of the eigenvalues of $D^2P$ at $(p_{12}, c)$.
...
% \\
\text{LHS } \eqref{eq:frequadd}
&
\les 
\int \frac{ 1 }{ \jb{\xi_{12}}^{4s+3 - s \lor 0} } 
\bigg( 
\int \ind_{|\xi^3 P(\frac{\xi_{12}}{\xi}, p_{21}, p_{22}) - \al| \le M} \xi^2 dp_{21} dp_{22} 
\bigg)
d\xi_{12}
\\
&
\les 
\int \frac{1}{\jb{\xi_{12}}^{4s+1 - s \lor 0}} 
\bigg( 
\int_{|q_{21}| ,  |q_{22}| \les 1} 
\ind_{|\xi^3 (P(p_{12},c) + \ld_{21} q_{12} + \ld_{22} q_{22}) - \al| \le M } 
d q_{21} dq_{22}
\bigg) 
d\xi_{12}
\\
&
\les M^{1-} \int \frac{1}{\jb{\xi_{12}}^{4s+4-s\lor 0 -}} d \xi_{12}
\les M^{1-},
\end{align}
given that $s> -\frac34$.
...
Combining the estimates for the terms of Types $\I$-$\VI$ completes the proof of Proposition~\ref{prop:bourgLinfty}.~\qed
...
From the proof of Proposition~\ref{prop:bourgLinfty}, we note that the frequency restriction $s> -\frac23$ comes from the cubic in $u$ terms of Type $\II$ (see Corollary~\ref{COR:types}), namely those associated with trees $\<21>$ and $\<22>$. Note that the same restriction is imposed by every Type $\II$ term in the proof of Proposition~\eqref{prop:bourgLinfty} due to the same trilinear sub-structure appearing in \eqref{IIbb}. 
% 
...
\begin{remark}
    \rm 
    Comment on being able to choose same $\be$ for all
\end{remark}
...
Given $j\in \N$ and $\TT\in \Tr_j$, as in Subsection~\ref{SUB:Linfty}, 
we use Lemma~\ref{lem:fre_bi} and Lemma~\ref{lem:fre_tri} to obtain \eqref{eq:bourg_bi_l2}  from suitable frequency-restricted  estimates. Based on Corollary~\ref{COR:types}, it suffices to consider $\TT$ and contributions $\NN^\gfrak(\TT)$ and $\NN^\bfrak(\TT)$ as in \eqref{wint0b} for $\TT$ of Type $\I$-$\VI$. 
...
from Lemma~\ref{lem:fre_bi}, we consider the multiplier
\begin{align}
\label{L21}
       \frac{\jb{\xi}^{2s} |\xi|^2 }{\jb{\xi_1}^{2s}\jb{\xi_2}^{2s}} 
    \ind_A \ind_{|\Psi(\TT) - \al| < M } , 
\end{align}
 for $A$ as in \eqref{A}, and we assume $|\xi_1| \ge |\xi_2|$ by symmetry. 
...
&
\les
\jb{\al}^{\frac12 + (-\frac23 s - \frac12 -) \land 0 
+ } M
\end{align}
where we used $|\xi_2| \les \jb{\al}^{\frac13}$ in the last step. 
...
&
\les 
\sup_{\xi_1} \jb{\xi_1}^{\frac32 -2s + } \ind_{|\xi_1|^3 \le \jb{\al}} M^\frac12
\les 
\jb{\al}^{\frac12 - \frac23 s +} M^\frac12,
\end{align}
where we used \eqref{eq:quadratica}, and we require $\frac12 - \frac23 s < 1 \iff s> -\frac34$. 
...
\jb{\xi}^s \mul_\gfrak(\TT) \prod_{\bul \in \TT^\infty} \frac{1}{\jb{\xi_\bul}^s}
&
\les \frac{\jb{\xi}^s \ind_{A_\star} \ind_{A^c_{\pb(\star)}} }{ \jb{\xi_{\star1}}^s \jb{\xi_{\star2}}^s  \jb{\xi_{\sbf(\star)}}^{s+1} } \prod_{\bul \in \TT^{\infty}\setminus \{\sbf(\star), \star1, \star2\}} \frac{1}{\jb{\xi_{\bul}}^{s+2}} 
. 
\end{align}
By symmetry, we assume $|\xi_{\star1}| \ge |\xi_{\star2}|$. Due to the frequency restrictions, we have
\begin{align}
|\xi| \lor 1 
 \les |\xi_{\star}| \sim |\xi_{\sbf(\star)}| 
\sim 
|\xi_{\star1}| 
\sim 
...
\big| |\xi_{\star2}| - | \xi_{\sbf(\star)}| \big| , 
\end{align}
contradicting the earlier assumption. This completes the proof for Type $\II$ terms. 
...
\jb{\xi}^s | \mul_\gfrak(\TT) + \mul_\bfrak(\TT) | 
\prod_{\bul \in \TT^\infty} \frac{1}{\jb{\xi_\bul}^s}
\les 
C^{j-3}
\frac{\jb{\xi}^{s\lor 0} |\xi_\star| \ind_{A_\star} \ind_{A^c_{\star1}} \ind_{A^c_{\star2}} }{ 
(\jb{\xi_{\star11}} \jb{\xi_{\star12}} \jb{\xi_{\star21}} \jb{\xi_{\star22}})^{s+1} } \prod_{\bul\in \TT^\infty_\star} \frac{1}{\jb{\xi_\bul}^{s+2}}, 
\end{align}
where $\TT^\infty_\star = \TT^\infty \setminus\{ \star11, \star12, \star21, \star22\}$, and we used that $|\xi| \le C^{j-3} |\xi_\star|$ and the following frequency relations hold for the last 3 generations:
\begin{align}\label{L24freq}
| \xi_\star| \sim |\xi_{\star1}| \lor |\xi_{\star2}| \sim \big| |\xi_{\star1}| - |\xi_{\star2}| \big| , 
\quad 
...
\les 
\M_1 \M_2
=: \bigg( \frac{\jb{\xi_\star}
\ind_{A_\star \cap A^c_{\star1} \cap A^c_{\star2}}
...
\noi
From the frequency assumptions in \eqref{L24freq}, we note that $|\xi_{\star11}| \simeq |\xi_{\star22}|$, i.e.
\begin{align}
|\xi_{\star11}| \sim |\xi_{\star11} - \xi_{\star22}| \sim |\xi_{\star11} + \xi_{\star22}|.  
\end{align}
...
\sup_{\xi_{\star11}, \xi_{\star22}, \xi} 
\int 
\frac{ \jb{\xi}^{2(s\lor0)}  }{ \jb{\xi_{\star11}}^{\frac52 + 2s -} \jb{\xi_{\star21}}^{2+2s} } \ind_{|\phi - \wt\al| < M} d\phi 
\prod_{\bul\in \TT^\infty_\star} \frac{1}{\jb{\xi_\bul}^{2s+4}}
d\xi_{\bul \in \TT^\infty_\star}
\\
&
\les C^{j-3} M  \sup_\xi \jb{\xi}^{2(s\lor 0) - \frac92 - 4s + } \les C^{j-3} M 
, 
\end{align}
...
\smallskip
\noi
\underline{Case 2: $|\xi_{\star11}| \simeq |\xi_{\star21}|$}

\noi 
Since $|\xi_{\star11}| \simeq |\xi_{\star12} | \simeq |\xi_{\star21}| \simeq |\xi_{\star22}|$ and $\xi_\star = \xi_{\star11} + \xi_{\star12} + \xi_{\star21} + \xi_{\star22}$, we have that $|\xi_{\star11}| \not\simeq |\xi_\star|$.

If $j=3$, then $\xi_\star = \xi$. If $j\ge 4$, then we also have $|\xi_{\sbf(\star)}| \not \simeq |\xi_{\star11}|$, where $\sbf(\star)$ denotes the sibling node of $\star$. We delineate how to proceed in the latter case, which can be easily modified to handle $j=3$ by swapping the roles of $\xi$ and $\xi_{\sbf(\star)}$.

\andreia{\textbf{Extra details for the conclusion above:}

Assume that $|\xi_\star| \simeq |\xi_{\star11}|$. Then, 
\begin{align}
    |\xi_{\star11}| \sim |\xi_\star| \gg \big| |\xi_\star| - |\xi_{\star11}| \big|.
\end{align}
If $\xi_\star \xi_{\star11} >0$, then
\begin{align}
    \big| |\xi_\star| - |\xi_{\star11}| \big| 
    =
    |\xi_\star - \xi_{\star11}|
    = 
    |\xi_{\star12} + \xi_{\star21} + \xi_{\star22}|
    \sim |\xi_{\star11}| , 
\end{align}
since either (i) $\xi_{\star11}, \xi_{\star21}, \xi_{\star22}$ have the same sign (and the conclusion follows), or (ii) exactly two frequencies have the same sign, say $\xi_{12}\xi_{21}>0$, and the conclusion follows from the fact that $|\xi_{12}| \simeq |\xi_{22}|$:
\begin{align}
    |\xi_{\star12} + \xi_{\star21} + \xi_{\star22}| 
    = 
    \big| |\xi_{\star12}| - |\xi_{\star22}| + |\xi_{\star21}|  \big|
    \sim |\xi_{\star22}| \sim |\xi_{\star11}|. 
\end{align}
\noi 
If $\xi_\star \xi_{\star11}<0$, then
\begin{align}
    ...
    | \xi_{\star} + \xi_{\star11}|
    =
    | 2 \xi_{\star11} + \xi_{\star12} + \xi_{\star21} + \xi_{\star22}|, 
\end{align}
which since there are is an odd number of terms above and $|\xi_{11}| \simeq |\xi_{12}| \simeq |\xi_{21}| \simeq |\xi_{22}|$, either all frequencies have the same sign, or we can always write the sum above as at least one large term + smaller ones. 


I do not see why we can always conclude that $|\xi_{\star11}| \not \simeq |\xi|$, it seems that this is still a valid case. However, from the proof, I believe this hypothesis is not being used, so I have omitted it. 

What I think we  can do, in order to do the change of variables in the first integral being considered below is to divide into two cases: $j=3$ (where $\xi_\star = \xi$ and thus the assumption on $\xi$ holds trivially) or $j\ge 4$. 
In the latter case, I want to use the change of variables with $\xi_{\sbf(\star)}$ instead.
}


Fixing $\xi_{\star12}, \xi_{\star21}, \xi_{\star22}$, with $\phi = -\xi^3_{\sbf(\star11)} - \xi^3_{\star11}$, 
note that further fixing $\xi_{\bul\in \TT^\infty_\star \setminus\{\sbf(\star)\}}, \xi$, we have $ | \partial_{\xi_{\star11} } \phi |  \sim |\xi_{\sbf(\star)} - \xi_{\star11}| \sim |\xi_{\star11}|^2$, which with $\jb{\xi} \les \jb{\xi_{\sbf(\star)}}$ gives
\begin{align}
    & \sup_{\xi_{\star12}, \xi_{\star21}, \xi_{\star22}
    } \int \M_2 \ind_{| \Psi(\TT) - \al| <M} d\xi_{\star11} d\xi_{\bul \in \TT^\infty_\star\setminus\{\sbf(\star)\}} d\xi 
    \\
    &
    \quad 
    \les 
    ...
    \int \frac{ \jb{\xi}^{2(s\lor0) - 2s - 4 } }{\jb{\xi_{\star11}}^{\frac92 + 4s - }} \ind_{|\phi - \wt\al| < M} d\phi d\xi \prod_{\bul \in \TT^\infty_\star \setminus\{\sbf(\star)\}} \frac{1}{\jb{\xi_\bul}^{2s+4}} d\xi_{\bul \in \TT^\infty_{\star}\setminus\{\sbf(\star)\}}
    \\
    &
    \quad 
    \les C^{j-3} M, 
 \end{align}
given that $s> -\frac{15}{8}$. \andreia{This means we do not need as much weight as we are putting here}

For the remaining integrals, we fix $\xi_{\star11}, \xi_{\bul\in \TT^\infty_{\star}}, \xi$, and normalize as follows 
\begin{align}
    p_\bul = \frac{\xi_\bul}{\xi_{\star11}}, \quad P(\xi_{\star12}, \xi_{\star21}) = p^3_\star - 1 - p^3_{\star12} - p^3_{\star21} - p^3_{\star22}, 
\end{align}
which implies that
\begin{align}
    \Psi(\TT) - \al = \xi^3 - \sum_{\star \in \TT^\infty_\star} - \xi^3_\star + \xi^3_{\star11} P(p_{\star12}, p_{\star21}) - \al 
    =:
    \xi^3_{\star11} P(p_{\star12}, p_{\star21}) - \wt\al, 
\end{align}
where $\wt\al$ does not depend on $\xi_{\star12}, \xi_{\star21}$. Since $|\xi_{\star11}| \simeq |\xi_{\star12}| \simeq |\xi_{\star21}| \simeq |\xi_{\star22}|$, then we are near a (non-degenerate) critical point of $P$, and can thus apply Morse's Lemma (Lemma~\ref{LEM:morse}) to obtain a change of coordinates $(p_{\star12},p_{\star21}) \mapsto (q_{\star12}, q_{\star21}$, with $|q_{\star12}|, |q_{\star21}| \ll 1$, and 
\begin{align}
    P(p_{\star12}, p_{\star21}) = c + \ld_{\star12} q^2_{\star12} + \ld_{\star21} q_{\star21}^2, 
\end{align}
for some $\ld_{\star12},\ld_{\star21} \in \{\pm1\}$. Therefore, we obtain
\begin{align}
&
    \sup_{\xi_{\star11}, \xi, \xi_{\bul\in \TT^\infty_\star}
    }
    \int \M_1 \ind_{|\Psi(\TT) - \al | < M } d\xi_{\star12} d\xi_{\star21}
    \\
    &
    \quad 
    \les 
    ...
    \jb{\xi_{\star11}}^{- \frac72 - 4s + 2 +}
    \int 
    \ind_{|\xi_{\star11}^3 P(p_{\star12}, p_{\star21}) - \wt\al | < M } dp_{\star12} dp_{\star21}
    \\
    &
    \quad 
    \les 
    ...
    \jb{\xi_{\star11}}^{- \frac32 - 4s  +}
    \int_{|q_{\star12}| + |q_{\star21}| \le 1} \ind_{| \xi_{\star11}^3 (c + \ld_{\star12} q_{\star12}^2 + \ld_{\star21} q_{\star21}^2 ) - \wt\al | < M } dq_{\star12} dq_{\star21} 
    \\
    &
    \quad 
    \les 
    M
    \sup_{\xi_{\star11}} \jb{\xi_{\star11}}^{- \frac92 - 4s +}
    \les M 
\end{align}
where we also used Lemma~\ref{lem:quadraticas}, given that $s > - \frac98$. 
Combining the estimate for both integrals with Lemma~\ref{lem:fre_tri} and Lemma~\ref{lem:eta1}, yields the estimate for Type $\IV$ and $\VI$ terms.
...
\noi
$\bul$ \textbf{Types $\III$ and $\5$ for $j=3$:}


Here, there are only two options for trees, namely $\<21>$ and $\<22>$, and we consider the first one. The result follows for the second by symmetry. 
From \eqref{wint0c}, the relevant multiplier is controlled as follows
\begin{align}
    \jb{\xi}^s \big| \mul_\gfrak^{\pb(\star)} (\TT) + \mul_\bfrak(\TT) \big| 
    \prod_{\bul \in \TT^\infty} \frac{1}{\jb{\xi_\bul}^s}
    &
    \les 
    \frac{\jb{\xi}^{s} |\xi|  \ind_{A_1^c} }{ \jb{\xi_{11}}^{s+1} \jb{\xi_{12}}^{s+1} \jb{\xi_{2}}^s},
\end{align}
where $\mul_\gfrak^{\pb(\star)}(\TT)$ corresponds to the portion of the multiplier $\mul_\gfrak(\TT)$ restricted to $A_{\pb(\star)} = A$. From \eqref{A}, we have $|\xi_{11}| \simeq |\xi_{12}|$, and consider different cases.


\smallskip
\noi
\underline{Case 1: $|\xi_{11}| \simeq |\xi_{12}| \ll |\xi|$}
...
&
    ...
    \les 
    \sup_{\xi_{11},\xi} \jb{\xi}^{-2s -3 +} \int \ind_{|\phi-\al|<M} d\phi \les M 
    , 
    \\
    &
    \sup_{\xi_{12}, \xi_2} 
    ...
    \les M , 
\end{align}
given that $s>-\frac32$, with the intended estimate following from Lemma~\ref{lem:fre_tri} and Lemma~\ref{lem:eta1}.


\smallskip
\noi
\underline{Case 2: $|\xi_{11}| \simeq |\xi_{12}| \ges |\xi|$}
...
\smallskip
\noi
$\bul$ \textbf{Types $\III$ and $\5$ for $j\ge 4$:}

Given $\TT \in \Tr_j$ with $j\ge4$, from \eqref{wint0c}, we have the following bound for the multiplier:
\begin{align}
    \jb{\xi}^s \big| \mul_\gfrak^{\pb(\star)} (\TT) + \mul_\bfrak(\TT) \big| 
    \prod_{\bul \in \TT^\infty} \frac{1}{\jb{\xi_\bul}^s}
    &
...
\end{align}
where $\TT^\infty_\star : = \TT^\infty \setminus \{\star1, \star2, \sbf(\star), \sbf(\pb(\star))\}$. 
...
\smallskip
\noi
\underline{Case 1: $|\xi_{\star1}| \simeq |\xi_{\star2}| \ll |\xi_{\pb(\star)}|$}

In this case, we have $|\xi_{\sbf(\pb(\star))}| \sim |\xi_{\pb(\star)}| \sim |\xi_{\sbf(\star)}| \gg |\xi_{\star1}| \sim |\xi_{\star2}| \ges |\xi_\star|$, where we recall that $\pb(\bul)$ and $\sbf(\bul)$ denote the parent and sibling node of $\bul$, respectively. Therefore, $|\xi_{\star1}| \not \simeq |\xi_{\pb(\sbf(\star))}| $ and $|\xi_{\star2}| \not \simeq |\xi_{\sbf(\star)}|$, which allows us to perform the following integrals with the change of variables $\xi_{\star1}, \xi_{\star2} \mapsto \phi = \Psi(\TT)$:
...
\smallskip
\noi
\underline{Case 2: $|\xi_{\star1}| \simeq |\xi_{\star2}| \ges |\xi_{\pb(\star)}|$}
...
It remains to consider $|\xi_{\star1}| \simeq |\xi_{\star2}| \simeq |\xi_{\sbf(\star)}| \simeq |\xi_{\sbf(\pb(\star))}| $. Here we consider two cases depending on the signs of the four frequencies: (i) two are positive and two are negative, or (ii) at least 3 frequencies have the same sign. 
...
In Case (ii), assume that $\xi_{\star1}, \xi_{\star2}, \xi_{\sbf(\star)}$ have the same sign. Fixing $\xi, \xi_{\sbf(\pb(\star)}, \xi_{\bul\in\TT^\infty_\star}$, let $p_\bul = \frac{\xi_\bul}{\xi_{\sbf(\pb(\star))}}$ and $P(p_{\star1}, p_{\star2}) = p^3_{\star1} + p^3_{\star2} + p_{\sbf(\star)}^3$. Then, 
\begin{align}
...
\end{align}
Thus, integrating in $|\xi_{\star1}| \simeq |\xi_{\star2}|\simeq |\xi_{\sbf(\star)}|$ where the frequencies have the same sign, amounts to integrating around a non-degenerate critical point of $P$ defined above. Therefore, by Morse's Lemma (Lemma~\ref{LEM:morse}), there exists a change of variables $(p_{\star1}, p_{\star2}) \mapsto (q_{\star1}, q_{\star2})$ with $|q_{\star1}|, |q_{\star2}| \ll 1$ and 
\begin{align}
    P(p_{\star1}, p_{\star2}) = c + \ld_{\star1} q_{\star1}^2 + \ld_{\star2} q_{\star2}, 
\end{align}
...
\begin{align}
    ...
    \sup_{\xi, \xi_{\sbf(\pb(\star))}, \xi_{\bul\in\TT^\infty_\star}}
     ...
     \int
    \ind_{|\xi_{\sbf(\pb(\star))}^3 P(p_{\star1, \star2}) - \wt\al| < M } d p_{\star1} dp_{\star2} 
    \\
    &
    \quad 
    \les 
...
\end{align}
where we used \eqref{eq:posdef} in Lemma~\ref{lem:quadraticas} on the first estimate, given that $s>-\frac78$. 



\andreia{The strategy used for (i) can be used for (ii), without using Morse on the second integral. This imposes the restriction $s>-\frac34$, which can be improved to $s>-\frac78$ if proceeding as above. }
...
\andreia{Add remark on regularity restriction of $s>-\frac34$ coming
from trilinear terms of Types $\III$ and $\5$.

For simplicity we present the estimate for these types even for higher multilinearity terms all together, under the stronger assumption, but this can be improved for $j\ge4$ terms. For the quadrilinear term this can already be improved to $s>-\frac78$, where the restriction comes from the stationary case $|\xi_{111}| \simeq |\xi_{112}| \simeq |\xi_{12}| \simeq |\xi_2| \gg |\xi| $, for the tree $\<31>$.
}
...
\subsection{Proof of Proposition \ref{prop:bourgLp}} The proof of \eqref{eq:bourg_bi_lp} follows from an interpolation argument between the $L^2$ and $L^\infty$ estimates \eqref{eq:bourg_bi_l2}-\eqref{eq:Linftybourg1}. However, since the latter involves the $X_{\infty,1}^{s,\tilde{b}}$ norm, the argument is slightly more delicate, as we present below.
...
Using the multilinear estimates of Section \ref{sec:multi}, we now prove local well-posedness of the gauged KdV equation \eqref{z1} using standard arguments (see, for example, \cite{Grunrock_mkdv}). To that end, given $T>0$, we define the local Fourier restriction space\\
\andreia{Restricted spaces can be defined earlier}
$$
X^{s,b}_{p,q}(0,T) = \left\{ z\big|_{[0,T]} : z\in X^{s,b}_{p,q}\right\}
$$
with the induced norm
$$
\|z\|_{X^{s,b}_{p,q}(0,T)} = \inf\left\{\|z'\|_{X^{s,b}_{p,q}}: z'\big|_{[0,T]}\equiv z\right\}.
$$
...
\begin{equation}\label{eq:Theta}
        ...
        + e^{-t\partial_x^3} \NN_1(\<1>; e^{t\partial_x^3}z) 
        + 
        \sum_{j=2}^\infty \sum_{\TT \in \Tr_j} \sum_{\l=2}^4 
        \NN_\l (\TT; e^{t\dx^3} z).
\end{equation}
and
\begin{equation}\label{eq:Theta_eta}
	\Theta_\psi[z](t) := \psi(t)e^{-t\partial_x^3}z_0
    + \psi(t) e^{-t\partial_x^3} \NN_1(\<1>; e^{t\partial_x^3}z)
    +  \sum_{j=2}^\infty \sum_{\TT \in \Tr_j}
    \sum_{\l=2}^4
    \psi_T(t)
        \NN_\l (\TT; e^{t\dx^3} z)
\end{equation}
recalling the definition of $\NN_\l$, $\l=1, \ldots, 4$, in \eqref{wint0b}.
...
Let $p\in[2, \infty]$, $T>0$, and consider the gauge transform $\Gc$ in \eqref{gauge}. Then, 

\noi {\rm(i)}
Given $s>-\frac12 - \frac1p$, the gauge transformation $\Gc$ is a continuous bijection on open balls $B_\dl \subset C([0,T]; \FL^{s,p}(\R))$ for $\dl>0$ sufficiently small. 
Consequently, its inverse $\Gc^{-1}$ is a continuous map on $B_\dl$. 

\smallskip
\noi {\rm(ii)} For any $t\in\R$, the map $\Gc_t$ is unbounded on $\FL^{s,p}(\R)$ for $s< - \frac12 - \frac1p$. 
...
\begin{remark}
    ...
    (i) Add comment about the endpoint regularity $s= - \frac12 - \frac1p $?
...
From \eqref{w} \andreia{Replace with reference to gauge definition}, we have that
\begin{align}
    \Gc_t[z] = z(t) + \D[z,z](t), 
\end{align}
for $\D$ as in \eqref{w}.
% 
% 
We start by showing that $\Gc$ is a small perturbation of the identity on $B_\dl \subset L^\infty_T \FL^{s, p}(\R)$, for $\dl>0$ sufficiently small. In particular, this follows once we show that $\D$ is a contraction on this set. 
...
\begin{align}
\|\D[z_{1},z_{2}]\|_{L^\infty_T \mathcal{F}L^{s,\infty}}   
&
\les
\sup_{0\le t \le T } \sup_\xi \jb{\xi}^s \int_{A^c} 
...
\bigg) 
\|z_{1}\|_{L^\infty_T \mathcal{F}L^{s,\infty}}\|z_{2}\|_{L^\infty_T \mathcal{F}L^{s,\infty}}
\notag
\\
& \les
\|z_{1}\|_{L^\infty_T \mathcal{F}L^{s,\infty}}
\|z_{2}\|_{L^\infty_T \mathcal{F}L^{s,\infty}}
\label{gaugeLinfty}
\end{align}
for $s>-\frac12$.
Similarly, for $p=2$, by duality and Cauchy-Schwarz inequality, we obtain
\begin{align}
\|\D[z_{1},z_{2}]\|_{L^\infty_T \mathcal{F}L^{s,2}}   
&
\les 
\sup_{\|z\|_{L^2} \le 1 }
...
\|z_{1}\|_{L^\infty _T \mathcal{F}L^{s,2}}\|z_{2}\|_{L^\infty_T \mathcal{F}L^{s,2}}\|z\|_{L^2}
\notag
\\
&
\lesssim\  \|z_{1}\|_{L^\infty_T \mathcal{F}L^{s,2}}\|z_{2}\|_{L^\infty_T \mathcal{F}L^{s,2}}, 
\label{gaugeL2}
\end{align}
given that $s>-1$. 
...
It follows from \eqref{eq:est_D} that $\D$ is a contraction on $B_\dl \subset L^\infty_T \FL^{s,p}(\R)$, for $\dl>0$ sufficiently small.
...
Lastly, we fix $t\in\R$, and show the unboundedness of the map $\Gc_t$ on $\FL^{s,p}(\R)$ for small $s$. Let $f \in \FL^{s, p}(\R)$ with 
\begin{align}
    \ft{f}(\xi) = \jb{\xi}^{-s-\frac1p-}. 
\end{align}
If $2 \le p <\infty$, from \eqref{w}, we have
\begin{align}
\| \D[f,f] (t) \|_{\FL^{s,p}}^p 
\ges 
\int_{|\xi|\le 1} \bigg| \int_{A^c} \frac{\ft{f}(\xi_1) \ft{f}(\xi_2) }{\xi_1\xi_2} d\xi_1 \bigg|^p d\xi
\sim \bigg( \int_{|\xi_1| \ge 1} \frac{1}{\jb{\xi_1}^{2s+2 + \frac2p + }} d\xi_1\bigg)^p , 
\end{align}
\andreia{The proof is ignoring the time dependence on the operator $\D$, which adds and oscillatory factor which we need to take into account . Could just do $t=0$ which would remove this issue?
 
Should come back to this and see if we want to define the gauge transform as depending on $\D[w,w]$ or $\D[e^{t\dx^3}w, ... ]$, since for the latter, the above result should hold for any $t$, but need to think harder for the former, because need to consider what happens from the oscillations}
\\
which diverges for $s<-\frac12 - \frac1p$, and for $p=\infty$, we have
\begin{align}
\| \D[f,f] (t) \|_{\FL^{s,\infty }}
\ges 
\sup_{|\xi| \le 1} 
\bigg| \int_{A^c} \frac{\ft{f}(\xi_1) \ft{f}(\xi_2) }{\xi_1\xi_2} d\xi_1 \bigg|
\sim 
\int_{|\xi_1| \ge 1 } \frac{1}{\jb{\xi_1}^{2s+2+}} d\xi_1  ,
\end{align}
which also diverges, allowing us to conclude the unboundedness of $\D$ on $\FL^{s, p}(\R)$. \qedhere 

\andreia{Does the above not work for $s=-\frac12$ when $p=\infty$, by removing the $0-$ extra power in defining $f$?}
...
\begin{equation}\label{eq:molinet2_v2}
 M(u_{0,N}) \to \theta \mbox{ strongly in }\mathcal{F}L^{s,p}, \ s+\frac1p<-\frac12.
\end{equation}
\begin{lemma}
...
\begin{equation}
	M(u_{9,N})= (u_{0,N})_x + (u_{0,N})^2 \to \frac{e^{4i\xi}-e^{2i\xi}}{2i\xi} =: \theta \quad \mbox{in }\mathcal{F}L^{s,p}(\R).
\end{equation}
as required.
...
In fact, Proposition~\ref{prop:s>1/4} follows from a standard fixed point argument in the space $X^{s, b}_{\infty, \infty} \cap X^{s, \wt b }_{\infty, 1}$ (see Definition~\ref{defi:bourg_space}) for some  suitable $b,\wt b\in \R$,
...
\begin{align}
\label{bilinear0}
\left\|\int_0^te^{-(t-t')\partial_x^3}\partial_x(u_1u_2) (s)ds\right\|_{X^{s,b}_{\infty, \infty}\cap X^{s,\tilde{b}}_{\infty, 1} } & \les 
...
\noi
$\bul$ 
\textbf{Case A: $|\xi_1^2-\xi_2^2| \gtrsim |\xi_1^2|$}
...
\noi
$\bul$ 
\textbf{Case B: $|\xi| \ll |\xi_1|$} In this region, we still perform the change of variables $\xi_2\mapsto \phi=\Phi$, since $|\partial_{\xi_2}\Phi|\gtrsim |\xi\xi_1|$. If $|\xi|>1$, since $|\xi|^{2+2s}|\xi_1|^{-4s+}\lesssim |\Phi|^{\frac{1-2s}{3}+} $
\begin{align*}
...
\noi
$\bul$ 
\textbf{Case C: $\xi_1\simeq \xi_2 \simeq \frac{\xi}{2}$.} 
...
\noi
$\bul$ 
\textbf{Case A: $|\xi|\gtrsim |\xi_1|$.} If $|\xi_2|>1$, then
$$
...
\smallskip\noi
$\bul$ 
\textbf{Case B: $|\xi|\ll |\xi_1|\simeq |\xi_2|$.} We bound
...
% where we used \eqref{eq:quadratica}. For $s> \frac12$, the estimate follows, while for $s\le\frac12$ we use the bound on $\xi$ to obtain
% \begin{align}
% \text{LHS\eqref{est1}} \les (\jap{\al} + M )^{\frac13(\frac12-s+)} M^\frac12 
% \end{align}
...
% Next, we estimate the integral $\I(\tau,\xi)$ on the LHS\eqref{bilin0a} from below. Let $|\xi| \ge 1$ and consider the integration restricted to $|\xi_1 - \xi/2| \ll |\xi|$ and $|\s_1| < 1$. Then, we have
% \begin{align}
% \label{bilin0b}
% \begin{split}
% \I(\tau,\xi) 
