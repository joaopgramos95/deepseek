...
as in the pointwise bound in Lemma~\ref{lem:Fock-basic}(ii).
...
namely for arbitrary bounded simply connected domains, with no boundary smoothness hypothesis. In
fact, simple connectivity is not used in this argument either, so the same proof yields the
slightly stronger statement that any bounded domain admitting a Hermite eigenfunction must be a
disk.
\end{remark}
...
\eqref{eq:uF-translate} shows that its associated densities drift to
