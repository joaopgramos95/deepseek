...
In summary, the results of this paper establish a bridge between time--frequency analysis, inverse spectral theory, and free-boundary problems in potential theory. This perspective not only resolves several open problems, but also provides a flexible and robust framework for studying a wide range of questions in phase--space analysis. :contentReference[oaicite:0]{index=0}
...
We begin with the perturbative free-boundary statement used below. The domain
is chosen so that the exterior Cauchy transform of the source vanishes; this
is the null-quadrature form of the one-phase problem and, for smooth
boundaries, is equivalent to
\[
-\Delta u=f \quad\text{in }\Omega,\qquad
u=\partial_\nu u=0 \quad\text{on }\partial\Omega,\qquad
u=0 \quad\text{on }\C\setminus\Omega .
\]
This is the local Cauchy-transform version of the perturbative free-boundary
method of Isakov~\cite[Chapter~5]{Isakov}, and is closely related to the
quadrature-domain constructions of
...
\[
\partial\Omega=\{x+\varphi(x)\nu(x):x\in\partial\Omega_0\},
\]
where \(\nu\) is the outer unit normal and
...
\]
Then there exist \(\varepsilon>0\) and \(\rho>0\) such that, whenever
\[
...
\|\varphi(f)\|_{C^2}<\rho,
\]
and a compactly supported function
\[
u_f\in C^1(\C)\cap C^{2,\alpha}(\overline{\Omega_f})
\]
such that
...
\]
Moreover, \(f\mapsto\varphi(f)\) is \(C^1\) from a neighborhood of \(f_0\) in
\(C^5(\overline{\Omega'})\) into \(C^2(\partial\Omega_0)\).
\end{proposition}

\begin{proof}
We prove the result by imposing the vanishing of the exterior Cauchy transform
as the free-boundary equation.

\medskip
\noindent\emph{Step 1: the Cauchy-transform equation.}
Choose a bounded \(C^2\) domain \(\Omega_2\) such that
\[
\overline{\Omega_0}\subset\Omega_2,\qquad
\overline{\Omega_2}\subset\Omega',
\]
and restrict to normal graphs with \(\overline{\Omega_\varphi}\subset\Omega_2\).
For such \(\varphi\), define
\[
...
\]
If \(\mathcal C(\varphi,f)\) vanishes on \(\partial\Omega_2\), then it vanishes
on \(\C\setminus\overline{\Omega_\varphi}\) by holomorphic continuation, since
it is holomorphic there and vanishes at infinity.

For the reference pair this condition holds. Indeed, from
\(-\Delta u_0=f_0\mathbf 1_{\Omega_0}\) and \(u_0=0\) on
\(\C\setminus\Omega_0\), the function \(\partial_z u_0\) vanishes outside
\(\Omega_0\) and solves
\[
\partial_{\bar z}(\partial_z u_0)=-\frac14f_0\,\mathbf 1_{\Omega_0}.
\]
The Cauchy transform of \(f_0\mathbf 1_{\Omega_0}\) solves the same
\(\bar\partial\)-equation up to the universal constant and vanishes at
infinity. Their difference is entire and vanishes at infinity, hence is zero.
Thus
\[
\mathcal C(0,f_0)=0
...
\medskip
\noindent\emph{Step 2: smooth dependence and linearization.}
...
\right\}.
\]
We equip \(Y\) with the quotient norm inherited from \(C^2(\partial\Omega_0)\);
by the exterior Cauchy inversion theorem this is equivalent to the
\(C^2\)-norm of the exterior holomorphic representative on any fixed annulus
between \(\partial\Omega_0\) and \(\partial\Omega_2\). The standard exterior
balayage property for the Cauchy transform shows that
\(\mathcal C(\varphi,f)|_{\partial\Omega_2}\in Y\): any \(C^2\) volume density
supported in \(\Omega_2\) has, in the exterior of \(\Omega_2\), the same Cauchy
transform as a unique \(C^2\) density on \(\partial\Omega_0\).
...
\to Y
\]
is \(C^1\) near \((0,f_0)\). This follows by writing
\(\Omega_\varphi=\Phi_\varphi(\Omega_0)\):
\[
\mathcal C(\varphi,f)(z)
=
\int_{\Omega_0}
...
\qquad z\in\partial\Omega_2,
\]
where the denominator stays uniformly away from zero.

For \(\psi\in C^2(\partial\Omega_0)\), the Hadamard variation formula gives
\begin{equation}\label{eq:appendix-shape-derivative-null}
D_\varphi\mathcal C(0,f_0)[\psi](z)
=
\int_{\partial\Omega_0}\frac{f_0(w)\psi(w)}{z-w}\,d\sigma(w),
\qquad z\in\partial\Omega_2 .
\end{equation}
...
If \(L\psi=0\) on \(\partial\Omega_2\), then the Cauchy transform of the
boundary density \(f_0\psi\) vanishes on the exterior of \(\Omega_0\) by the
identity theorem. Taking boundary values on \(\partial\Omega_0\) and using the
Sokhotski--Plemelj jump relation gives \(f_0\psi=0\), hence \(\psi=0\) because
\(f_0>0\) on \(\partial\Omega_0\). Thus \(L\) is injective.

Conversely, if \(h\in Y\), let \(H\) be its holomorphic extension to
\(\C\setminus\overline{\Omega_0}\), vanishing at infinity. The exterior
Cauchy inversion problem on the \(C^2\) curve \(\partial\Omega_0\) gives a
unique density \(\sigma\in C^2(\partial\Omega_0)\) such that
\[
H(z)=\int_{\partial\Omega_0}\frac{\sigma(w)}{z-w}\,d\sigma(w),
\qquad z\in\C\setminus\overline{\Omega_0}.
\]
Setting \(\psi=\sigma/f_0\) gives \(L\psi=h\). The Fredholm theory of the
Cauchy singular integral on \(C^2\) curves gives continuous dependence of
\(\sigma\), hence \(L^{-1}\) is bounded.

\medskip
\noindent\emph{Step 4: implicit function theorem and construction of \(u_f\).}
Since \(\mathcal C(0,f_0)=0\) and \(D_\varphi\mathcal C(0,f_0)=L\) is an
isomorphism, the implicit function theorem gives a unique small
\(\varphi(f)\) solving
\[
\mathcal C(\varphi(f),f)=0
\qquad\text{on }\partial\Omega_2 .
\]
Let \(\Omega_f=\Omega_{\varphi(f)}\). By holomorphic continuation,
\[
\int_{\Omega_f}\frac{f(w)}{z-w}\,dA(w)=0
\qquad\text{for }z\in\C\setminus\overline{\Omega_f}.
\]
Define
\[
N_f(z):=-\frac1{2\pi}\int_{\Omega_f}\log|z-w|\,f(w)\,dA(w).
\]
Then \(-\Delta N_f=f\mathbf 1_{\Omega_f}\), and the vanishing of the Cauchy
transform says that \(\partial_zN_f=0\) on the exterior. Thus \(N_f\) is
constant on \(\C\setminus\overline{\Omega_f}\). Subtracting this constant
gives a compactly supported solution \(u_f\) of
\eqref{eq:appendix-perturbed-problem}, with \(u_f=0\) on the exterior.
Regularity of logarithmic potentials with bounded compactly supported density
gives \(u_f\in C^1(\C)\), while interior Schauder estimates give
\(u_f\in C^{2,\alpha}(\overline{\Omega_f})\). Since \(u_f\equiv0\) outside,
also \(\partial_\nu u_f=0\) on \(\partial\Omega_f\).

\medskip
\noindent\emph{Step 5: preservation of negativity.}
On compact subsets of \(\Omega_0\), the pullbacks
\(u_f\circ\Phi_{\varphi(f)}\) converge uniformly to \(u_0\), so strict
negativity persists away from the boundary. Near the boundary, use the
second-order nondegeneracy. Since \(u_f=\partial_\nu u_f=0\) on
\(\partial\Omega_f\) and \(-\Delta u_f=f\), while all tangential derivatives
of the boundary trace vanish, one has
\[
\partial_{\nu\nu}u_f=-f
\qquad\text{on }\partial\Omega_f .
\]
Because \(f_0>0\) on \(\partial\Omega_0\), after shrinking \(\varepsilon\)
there is \(c_0>0\) such that
\(\partial_{\nu\nu}u_f\le -c_0\) on \(\partial\Omega_f\). Taylor expansion
along inward normal segments gives
\[
u_f(x-t\nu_f(x))\le -\frac{c_0}{4}t^2<0
\]
for \(0<t<t_0\), uniformly in \(x\in\partial\Omega_f\). Thus \(u_f<0\) in a
boundary collar and hence throughout \(\Omega_f\). The uniqueness and \(C^1\)
dependence of \(\varphi(f)\) are those supplied by the implicit function
theorem.
\end{proof}
...
We postpone the proof of Theorem~\ref{thm:eig-loc-U} until after the perturbative
inverse theorem. The same inverse/free-boundary mechanism used to construct
domains for prescribed eigenfunctions also recovers the classical
Abreu--D\"orfler disk theorem in the simply connected setting; see
...
\qquad \text{in }\C,
\]
with \(v_0=0\) on \(\C\setminus D_0\) and \(v_0<0\) in \(D_0\). Since \(\widetilde u_0\) was constructed as the compactly supported radial solution of \eqref{eq:unperturbed-free-boundary-new}, it is \(C^1\) across \(\partial D_0\); equivalently \(v_0=\partial_\nu v_0=0\) on \(\partial D_0\). Thus the hypotheses
...
\]
Applying Proposition \ref{prop:isakov-tailored}, we obtain a \(C^2\) domain \(U_0\), represented as a small normal deformation of \(D_0\), and a compactly supported function \(v\) such that
\[
...
\begin{theorem}\label{thm:local-perturbative} Given $\lambda \in (0,1)$ and $N \in \mathbb{N},$ there is $\varepsilon_0(\lambda,N)>0$ such that, if a function $f_0 = h_0 + \sum_{k=0}^N a_k h_k$ satisfies $\max |a_k| < \varepsilon_0,$ then there is a domain $U_{\lambda}$ such that 
\[
...
\section{Proof of Theorem \ref{thm:local-perturbative}} 
...
According to Theorem \ref{thm:local-perturbative}, we know that, for each $\varepsilon > 0$ sufficiently small, there is a domain $U_{\varepsilon},$ which moreover may be taken to be a perturbation of the form 
...
\end{equation}
The assertion about $U_{\varepsilon}$ being a perturbation of the unit ball of the form \eqref{eq:def-u-eps} may be proved as follows: first, notice that the method of proof of Theorem \ref{thm:isakov} yields directly that the domains so obtained are of the desired form. A thorough inspection of the proof of Proposition \ref{prop:auxiliary} shows that the same holds for the domains constructed there - that is, the domain $\tilde{U}$ from Proposition \ref{prop:auxiliary} may be taken to be a normal perturbation of the domain $U_0$ in that result. If both perturbations are small enough in absolute norm, we readily conclude that the domains constructed in Theorem \ref{thm:local-perturbative} are still a smooth perturbation over the unit disk. 
