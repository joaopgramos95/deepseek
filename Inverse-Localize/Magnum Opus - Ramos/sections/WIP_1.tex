...
% TODO: consider adding \label{sec:preliminaries}, \label{sec:perturb-proof}, \label{sec:GGRT-sharp}, \label{sec:local-max}, \label{sec:global-max} to the corresponding \section commands in main_expanded.tex so these references can be made symbolic instead of hard-coded numbers.
The paper is organized as follows. In Section~2 we review the Bargmann--Fock representation and establish several auxiliary results, including the bridge from time--frequency analysis to free boundary problems via the key divisibility lemma. Section~3 proves Theorem~\ref{thm:perturb}; the self-contained disk inverse theorem is established there as part of the construction. Section~\ref{sec:classical-rigidity} explains how the same machinery recovers the classical theorem of Abreu and D\"orfler in the simply connected case (Theorem~\ref{thm:eig-loc-U}). Section~5 contains the sharpness result, Theorem~\ref{thm:GGRT-sharp}. Section~6 establishes the local rigidity statement, showing that local maximizers of the Faber--Krahn problem in the Gaussian STFT setting are disks. Finally, Section~7 proves existence of global maximizers via concentration--compactness in the Bargmann--Fock space and combines this with the local rigidity to give a new proof of the Nicola--Tilli theorem.
