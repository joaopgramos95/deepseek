...
\cite{Hormander,Folland})%
\footnote{TODO: the bib key \texttt{Hormander} currently points to
H\"{o}rmander's 1991 \emph{Ark.\ Mat.}\ note on Beurling's theorem rather
than to Volume~I of \emph{The Analysis of Linear Partial Differential
Operators}, where Theorem~7.3.1 (Paley--Wiener--Schwartz) lives. A
dedicated bib entry for Vol.~I should be added; analogously, the present
\texttt{Folland} key is to \emph{Harmonic Analysis in Phase Space}, which
does not contain a textbook proof of Paley--Wiener--Schwartz at \S 9.3 --
\emph{Real Analysis} or Rudin's \emph{Functional Analysis} would be more
appropriate.}, the Fourier
...
\footnote{TODO: the present \texttt{Garnett} key is to \emph{Bounded
Analytic Functions} (GTM 236), which does not contain the Ch.~II,
Thm~1.3 statement on continuity of logarithmic potentials of bounded
densities originally cited here; an entry for, e.g., Ransford's
\emph{Potential Theory in the Complex Plane} or Landkof's
\emph{Foundations of Modern Potential Theory} should be added to the
bibliography.} The difference $u-v$ is then a tempered
