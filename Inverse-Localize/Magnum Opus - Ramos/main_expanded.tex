...
The proof proceeds in three stages, combining tools from free-boundary problems and complex analysis. First, we reformulate the eigenvalue equation in the Bargmann--Fock space as a nonlinear integral identity for the squared modulus of an entire function with polynomial growth. Applying a suitable differential operator, we reduce the problem to that of constructing a domain satisfying a weighted identity that is reminiscent of a quadrature condition.

Next, we produce an approximate solution by solving a free-boundary problem of obstacle type. This is achieved through a perturbative theorem of Isakov \cite{Isakov}, which provides a domain whose associated logarithmic potential satisfies a prescribed equation under small perturbations of the source. This step yields a domain for which the desired identity holds up to a controlled error.

In the third stage, we refine this construction using a weighted quadrature-domain theorem in the spirit of \cite{Gustafsson,Gustafsson-Shahgholian,Shahgholian}. Viewing the problem through the lens of complex analysis, we interpret the identity as a condition on a weighted Cauchy transform, and a perturbative implicit-function argument allows us to deform the approximate domain so that the identity becomes exact. Finally, we show that the resulting function satisfies the original eigenvalue equation by analyzing an associated differential operator and ruling out nontrivial obstructions through growth estimates. The combination of these steps yields a robust local inverse theory.
...
The paper is organized as follows. In Section~2 we review the Bargmann--Fock representation and establish several auxiliary results, including a key divisibility lemma and perturbative free-boundary theorems. Section~3 is devoted to the perturbative inverse construction underlying Theorem~\ref{thm:perturb}. Section~4 explains how the same machinery recovers the classical theorem of Abreu and D\"orfler in the simply connected case (Theorem~\ref{thm:eig-loc-U}). Section~5 contains the sharpness result, Theorem~\ref{thm:GGRT-sharp}, while the remaining sections are devoted to variational applications, culminating in the new proof of the Nicola--Tilli theorem.
...
\]
with $f\in L^1(\R^2)$ compactly supported, $c_j\in \R\setminus\{0\}$, and
...
\]
The first summand is the logarithmic potential of the $L^1$, compactly
...
Theorem~\ref{thm:perturb}. The first is a free-boundary stability
...
\qquad \text{in }\C,
\]
with $v_0=0$ on $\partial D_0$ and $v_0<0$ in $D_0$, by the positivity of $\widetilde u_0$ established above. Thus all hypotheses of Proposition~\ref{prop:isakov-tailored} are satisfied with $\Omega_0=D_0$.
...
\]
where $\varepsilon_1$ is the smallness threshold furnished by Proposition~\ref{prop:isakov-tailored}. Applying that proposition, we obtain a $C^2$ domain $U_0$ and a function $v$ such that
\[
