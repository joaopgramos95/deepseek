...
\]
If some \(R_j\) were nonzero, choose one of maximal \(|\zeta_j|\) and call it
\(\zeta_1\). Equal roots have already been grouped into the same polynomial
coefficient \(R_1\), so for every competing root \(\zeta_j\ne\zeta_1\) one has
\(\Re(\zeta_j\overline{\zeta_1})<|\zeta_1|^2\). Along
\(w=t\overline{\zeta_1}\), \(t\to+\infty\), the term
\(R_1(w)e^{\zeta_1w}\) dominates the remaining exponential-polynomial terms,
so
\[
