...
In the present work, we address the question of stability of Minkowski bodies. That is, can we estimate a suitable distance between two Minkowski bodies $E_\mu, E_\nu$ with a suitable distance between $\mu,\nu$? It is well known that Minkowski bodies can be obtained as solutions to an appropriated variational problem, which will be discussed further on, so that the question of stability can be seen as stability of minimizers with respect to a parameter of the energy functional. 

In the present work, we prove stability results assuming the form
\begin{equation}\label{eq.BM_stability_prototype}
    {\alpha(E_\mu, E_\nu)}^2 \leq C {\dc(\mu,\nu)}^{1 + \frac{1}{n}},
\end{equation}
for some constant $C$, and an exponent that is dependent on the dimension $n$ but uniformly bounded from below by its asymptotic $1/2$. Here $\alpha$ denotes the \textit{Fraenkel asymmetry}, used for stability statements by~\cite{fusco2008sharp} in the case of the isoperimetric inequality and by \cite{figalli2010stability} in the case of the more general anisotropic isoperimetric inequality, defined  as 
\begin{equation}\label{eq.fraenkel_asymmetry}
    ...
    \right\}.
\end{equation}
It holds that $\alpha(E,F)=\alpha(F,E)$, and $\alpha(E,F)=0$ if and only if $E$ and $F$ are homothetic, so that $\alpha$ becomes a distance on the equivalence class of homotheties. The \textit{convex-dual distance} $\dc$ is defined for two measures $\mu,\nu$ over $\mathbb{S}^{n-1}$ as 
\begin{equation}\label{eq.convex_dual_distance}
    ...
    \right|,
\end{equation}
where the supremum is taken over all convex bodies included in the unit ball, and $h_K$ corresponds to the support function of $K$, defined in~\eqref{eq.support_function}. See~\cite{abdallah2015reconstruction} for the basic properties of $\dc$. 
...
\begin{figure}[ht]\label{fig.degenerate_minko_bodies}
    \centering
    \input{degenerate_minko_tikz.tex}
    \caption{Family of ellipsoids collapsing to a line as $\Theta$ goes to $0$.}
\end{figure}

% {\color{red} QUESTIONS: 
% \begin{itemize}
% \item Do we really want to restrict the Theorem to probability measures? A big advantage of the convex dual distance is that it makes sense even in the case of different total measures. This setup may come up naturally in applications. Then $\Theta(\mu)$ can be defined relative to $\mu(S^{n-1})$.
% \item Is the Fraenkel asymmetry  sufficient to measure the distance between two convex bodies? It allows convex bodies having very different volumes. Maybe, we should also state an estimate on $\max\{\frac{|E|}{|F|},\frac{|F|}{|E|}\}$ (like in Figalli, et al), and eventually have an estimate on
% $$
% \inf\left\{\frac{|E\Delta (x+F)|}{\min\{|E|,|F|\}}:x\in \mathbb{R}^n\right\}
% $$
% (which quantity is not a metric I think).
% \end{itemize}
% }
...
Now we are ready to prove our main stability result for Minkowski's problem. Our proof relies on first leveraging the rigidity conferred by anisotropic isoperimetry, and then using it to reinforce a stability principle based on a variational formulation of Minkowski problem, for which the qualitative stability theory of the Brunn-Minkowski inequality plays a major role. Our major reference for these tools are~\cite{diskant1972bounds,figalli2010stability}, whose results we review briefly in the sequel. 

Any convex body $F$ containing the origin induces an anisotropy as the quantity 
\[
    \norm{v}_F 
    \eqdef 
    \sup \{v\cdot x: x \in F\}, \quad v\in\mathbb{S}^{n-1}. 
\]
Of course, this is just the geometric measure theory notation for the support function, $\norm{\cdot}_F = h_F$. Whereas the anisotropic perimeter of a set $E$
\begin{equation}
    P_F(E) \eqdef 
    \int_{\partial E} \norm{\nu_E(x)}_F\dd \mathcal{H}^{n-1}(x) 
    =
    \int_{\mathbb{S}^{n-1}} h_F \dd S_E = nV_1(E,F)
\end{equation}
and can also be equivalently written by means of the first mixed volume. 

One of the first approach to reinforce this inequality with quantitative version is due to Diskant~\cite{diskant1972bounds}, where a control of the Hausdorff distance between two convex bodies can be obtained assuming bounds on the geometry of the convex bodies involved. Recalling that the Hausdorff distance of convex bodies can be written as the $L_\infty$ norm between the corresponding support functions 
\[
    d_H(E,F) = \norm{h_E - h_F}_{\infty}, 
\]
it is shown in~\cite{diskant1972bounds} that, given two convex bodies $E,F$ such that $r \le r_E, r_F$ and $R_E, R_F \le R$, there is a constant $C$ depending on $r,R,n$ such that 
\begin{equation}\label{eq.diskant_isoperimetric}
    {d_H(E,F)}^n \le C \left(V_1(E,F) - |E|^{\frac{n-1}{n}}|F|^{\frac{1}{n}} \right).
\end{equation}

The optimal version of this inequality was then proposed in~\cite{figalli2010stability}, where the dependence on the geometry is removed by considering the following notion of discrepancy for Wulff's inequality 
\begin{align}\label{eq.deficit_ISO}
    0\le \delta_{\text{ISO}}(E,F) 
    &\eqdef 
    \frac{P_F(E)}{n|F|^{\frac{1}{n}}|E|^{\frac{n-1}{n}}} - 1 %\cr
    = 
    \frac{V_1(E,F)}{|F|^{\frac{1}{n}}|E|^{\frac{n-1}{n}}} - 1,
\end{align}
...
\begin{equation}\label{eq.quantitative_isoperimetric}
    ...
    \right\}. 
\end{equation}
In other words, there is a dimensional constant $C_n$, for which the anisotropic isoperimetric inequality can be strengthened as 
\begin{equation}
    {\alpha(E,F)}^2
    \le 
    C_n\delta_{\text{ISO}}(E,F). 
\end{equation}

Our second approach to the stability of Minkowski bodies employs the sharp quantitative stability of the Brunn-Minkowski inequality. As we will only need this inequality with the convexity parameter given by $1/2$, we define the deficit of the Brunn-Minkowski inequality for two convex sets $E,F$ as 
\begin{equation}\label{eq.deficit_BM}
    ...
    - 1.
\end{equation}
Differently from the isoperimetric inequality, in this case the roles of $E$ and $F$ are completely symmetric, so it is only natural to compensate with ratio between the volumes of the sets 
\[
    \sigma_{E,F} 
    \eqdef 
    \max 
    ...
    \right\},
\]
then there exists a constant $C_n$ depending only on the dimension for which the following stronger version holds
...
\begin{lemma}\label{lemma.variational_problem}
    ...
    \begin{enumerate}
        ...
        \[
            ...
            C_{\mu,\nu} \dc(\mu,\nu),
        \]
        where $W_1$ corresponds to the $1$-Wasserstein distance and $C_{\mu,\nu} =  \max\left\{\frac{R(E_\mu)}{|E_\mu|^{\frac{1}{n}}}, \frac{R(E_\nu)}{|E_\nu|^{\frac{1}{n}}}  \right\}$. 
    \end{enumerate}
\end{lemma}
\begin{proof}
...
\begin{proof}[Proof of Theorem~\ref{thm.quantitative_stability_p=1}]
    Given $\mu,\nu \in \mathscr{P}(\mathbb{S}^{n-1})$ such that $\Theta(\mu),\Theta(\nu) \ge \vartheta > 0$, the strict positivity of the functional $\Theta$ implies the existence of centered Minkowski bodies $E_\mu,E_\nu$. Let $\varphi_\mu, \varphi_\nu$ be their corresponding support functions. We first obtain a weaker stability result by bounding the isoperimetric deficit. As a consequence of Lemma~\ref{lemma.rR}, there are universal constants $C_{n,\vartheta},c_{n,\vartheta}>0$ depending only on $n$ and $\vartheta$ such that
    \[
        c_{n,\vartheta} \le r(E_\mu) \le R(E_\mu) \le C_{n,\vartheta},
    \]
    and similarly for $E_\nu$. As a result, up to replacing the constants above, it follows that 
    \[
        c_{n,\vartheta} \le |E_\mu|,|E_\nu|, \quad 
        \Lip(\varphi_{\mu}), \Lip(\varphi_{\nu}) \le C_{n,\vartheta}. 
    \]

    Without loss of generality we can assume that $|E_\mu| \ge |E_\nu|$, which implies with the anisotropic isoperimetric inequality that 
    \begin{align*}
        V_1(E_\mu,E_\nu) - |E_\nu|
        &\ge 
        V_1(E_\mu,E_\nu) - |E_\nu|^{\frac{1}{n}}|E_\mu|^{\frac{n-1}{n}}. %\\
        % &= 
        % n|E_\nu|^{\frac{1}{n}}|E_\mu|^{\frac{n-1}{n}} \, 
        % \delta_{\text{ISO}}(E_\mu,E_\nu)
    \end{align*}
    Next we recall the following identities for the mixed volumes
    \[  
        V_1(E_\mu,E_\nu) = \frac{1}{n}\int_{\mathbb{S}^{n-1}} \varphi_{\nu}\dd S_{E_\mu},
        \quad
        |E_\nu| = \frac{1}{n}\int_{\mathbb{S}^{n-1}} \varphi_{\nu}\dd S_{E_\nu},
    \]
    which combined with Diskant's quantitative isoperimetric inequality~\eqref{eq.diskant_isoperimetric}, and using that $S_{E_\mu}=\mu,S_{E_\nu}=\nu$, it follows from the definition of the dual-convex distance $\dc$~\eqref{eq.convex_dual_distance} that
    \begin{equation}\label{eq.premilinary_stability_p=1}
        \norm{\varphi_\mu - \varphi_\nu}_\infty \le C \dc(\mu,\nu)^{\frac{1}{n}},
    \end{equation}
    where $C$ is a constant depending only on $\vartheta$ and $n$. 

    We can obtain a stronger version by employing the Figalli-Maggi-Pratelli quantitative version of the isoperimetric inequality~\eqref{eq.quantitative_isoperimetric}, obtaining an estimate of the form $\alpha(E_\mu,E_\nu) \le C_{n,\vartheta} \dc(\mu,\nu)^{1/2}$. This is also achievable with the variational argument that follows, this time using the Brunn-Minkowski inequality, but the latter combined with~\eqref{eq.premilinary_stability_p=1} yields a better exponent. 
    ...
    \begin{align*}
        ...
        C_{n,\vartheta}\, {\dc(\mu,\nu)}^{1 + \frac{1}{n}},
    \end{align*}
    where we have used the quantitative stability of the constant $\mu \mapsto \lambda_\mu$ with respect to $\dc(\mu,\nu)$, proved in Lemma~\ref{lemma.variational_problem}, combined with the uniform estimates on the inradius and circumradius from~\ref{lemma.rR}. Combining the above estimate with~\eqref{eq.def_delta} and the quantitative stability of the Brunn-Minkowski inequality~\eqref{eq.quantitative_BM}, we conclude that there exists a constant $C_{\vartheta,n}$ such that 
    \[
...
\end{equation}
where we have equality if and only if $E$ and $F$ are dilates.  
...
\begin{lemma}\label{lemma.quantitative_Lp_isoperimetric}
    ...
    \begin{equation}\label{eq.hausdorff_Lp_isoperimetry}
        \norm{h_E - h_F}_\infty^n\le C \left(V_p(E,F) - |E|^{\frac{n-p}{n}}|F|^{\frac{p}{n}}\right).
    \end{equation}
\end{lemma}
\begin{proof}
...
\begin{proof}[Proof of Thm.~\ref{thm.quantitative_stability_p}]
    ...
    As in the $p = 1$ case, to control the quantity $\delta$ above we use the concavity of the function $t \mapsto F(t) \eqdef \mathscr{F}_{\mu,p}(\varphi_{t})$, for $\varphi_t \eqdef {\left( (1-t)\varphi_{\nu,p}^p + t\varphi_{\mu,p}^p \right)}^{1/p}$, along with the optimality of $\mathscr{F}_{\mu,p}(\varphi_{1/2}) \le \mathscr{F}_{\mu,p}(\varphi_{\mu,p})$. Using the stability in Hausdorff distance from~\eqref{eq.premilary_Lp_stability}, along with the analogous arguments from the $p=1$ case, we have that
    \begin{align*}
        \delta 
        &= 
        \mathscr{F}_{\mu,p}(\varphi_{1/2}) 
    ...
    \end{align*}
    where we have used the quantitative stability of the constant $\mu \mapsto \lambda_{\mu,p}$ with respect to $\dc(\mu,\nu)$, from Lemma~\ref{lemma.variational_problem_Lp}. Combining this estimate with~\eqref{eq.control_Fraenkel_Lp} the result follows. 
\end{proof}
