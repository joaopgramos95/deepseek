...
based on a contraction mapping argument in spaces tailored to the dispersion of the equation. This seminal paper was later improved in \cite{kpv_bilin} to the threshold $s>-\frac34$. The endpoint case $s=-\frac34$ was first attained by Christ, Colliander, and Tao \cite{CCT_ill} and revisited by Guo \cite{Guo_gwp} and Kishimoto \cite{Kishimoto_lwp_crit}. It is also important to note that the aforementioned results are based on contraction arguments, which can be extended to globally-in-time by means 
...
In light of the ill-posedness in in $L^2$-based Sobolev spaces \cite{Molinet_ill_line}, to bridge the gap between the well-posedness threshold of $H^{-1}(\R)$ in \cite{KillipVisan_kdv} and the critical regularity $s=-\frac32$, one may consider a different functional framework. 
...
For space-time functions $f: \R^2 \to \R$, we may write $\Ft_t (f)$ and $\Ft_x(f)$ to denote the Fourier transform with respect to $t$ and $x$, respectively. When clear from context, we also use $\ft{f}$ to denote either the spatial Fourier transform or the space-time Fourier transform. }
	\begin{equation}
		\|f \|_{\FL^{s,p}(\R)}=\|(1+|\xi|^2)^{s/2}\ft{f }(\xi)\|_{L^p(\R)}.
	\end{equation} 
These spaces agree with the classical $H^s(\R)$ spaces when $p=2$ and
and have been considered in many works \cite{adams, BO94, Chapouto_FL_mkdv, Christ_power,CC24,Grunrock_mkdv, Grunrock_ldv_hier, HerrGrunrock_FL,  GrunrockVega}, for example.
\\
%
\andreia{
- Added Grunrock (2004) paper since it predates some of those listed.

- Added Bourgain's 94 paper where he does well-posedness in $H^{s_1} \cap \FL^{s_2,\infty}$ for periodic mKdV. You can tell me if you disagree
(he also has a paper on invariant measures for periodic Zakharov where he uses $\FL^{s, \infty}$, Duke '94)

}
%
%

As a general rule of thumb, it is important to understand the behaviour of the (homogeneous) $\dot{\FL}^{s,p}$-norm under the natural scaling \eqref{scaling} of \eqref{kdv}. 
Since 
$$
\|u_\ld \|_{\dot{\FL}^{s,p}(\R)} \sim \lambda^{1+s+\frac1p}
\| u \|_{\dot{\FL}^{s,p}(\R)}, 
$$ 
we say that the Fourier-Lebesgue space $\FL^{s,p}(\R)$ with 
$s=s_c(p)=-1-\frac1p$ is \emph{critical}, as the corresponding norm is left invariant under scaling. For comparison, we can also define $s_{H^{-1}}(p) = - \frac12 - \frac1p$ as the Fourier-Lebesgue regularity at the $H^{-1}$-scale.
%
...
\label{eq:sp_grunrock}
	\end{equation}
The result in \cite{Grunrock_ldv_hier} remains the current state-of-the art in this functional framework, leaving a substantial gap from the scaling critical regularity $s_c(p)$ and even from the $H^{-1}$-scale $s_{H^{-1}}(p)$. The regularity constraint \eqref{eq:sp_grunrock} comes from the main bilinear estimate, in particular, the high-high-to-low frequency interactions $|\xi_1| \sim |\xi_2| \gg \max(\big||\xi_1| - |\xi_2|\big|, 1)$ (see Appendix \ref{sec:-14}). 
{\color{blue}To go beyond this result, our first main contribution is the introduction of a gauge transform that 
successfully removes these interactions and allows for a local well-posedness theory via a contraction argument at lower regularity, surpassing even the $H^{-1}$ scale.}

\andreia{Morally agree with the sentence above, but the potential issue is that the relevant result we are mentioning that goes beyond the $H^{-1}$-scale is not for KdV, but for a different equation, which is not made clear from the statement above. Do we want to be more careful?}
...
{\color{blue}$A^c$ captures the problematic high-high-to-low frequency interactions; see \eqref{A} for a definition. }
...
\\
\andreia{I think there is an issue above as a consequence of a rewriting I suggested, which was to mention $\Gc$ and its inverse above, which makes the if and only if statement not true, because we do not prove that $\Gc$ is not well-defined at low reg. } 
    \\
    Moreover,
...
z=\mathcal{G}[u]
    \end{equation}
solves the following \emph{gauged KdV equation}:
\begin{equation}
...
N_j[z].
	\end{equation}
Here, 
each $N_j$, $j\ge 2$, is a nonlocal nonlinear operator with homogeneity $j$ in $z$. In particular, the quadratic nonlinear terms $N_2[z]$ in \eqref{rkdv} do not contain the bad high-high-to-low frequency interactions, at the cost of more involved cubic and higher order contributions. 
  Despite the apparent increased complexity of the gauged KdV equation \eqref{rkdv}, it is in fact \emph{better behaved} than \eqref{kdv}, and we can push its well-posedness theory to lower regularities, namely beyond the $H^{-1}$-scale.
...
accounting for careful algebraic cancellations together with the effects of the map $\Gc^{-1}$ in \eqref{gauge}.
We postpone the details of this derivation to Subsection~\ref{SUBSEC:method}. For the precise description of \eqref{rkdv}, see Section \ref{sec:infr} and \eqref{z1}.
...
\eqref{kdv} \overset{\Gc}{\longleftrightarrow} \eqref{rkdv}, 
    $$
while we will see that we can extend the well-posedness theory for \eqref{rkdv} beyond that of \eqref{kdv} at low regularity. 
%
%
...
To {\jp the best of} our knowledge, the gauge transform $\Gc$ is a new transform for the \eqref{kdv} on the real line. As will become clear in Subsection~\ref{SUBSEC:method}, one advantage of $\Gc$ is being independent of \emph{any} completely integrable structure for \eqref{kdv}, and thus being easily generalized for other dispersive equations with quadratic nonlinearities. 
...
{ \color{blue} For a more precise statement of Theorem \ref{thm:lwp_rkdv}, we refer to Proposition \ref{thm:lwp_lp}.}
\andreia{Is this sentence needed ? It seems that the Prop. says essentially the same?}
...
%
Similarly, there are no scaling critical exponents for \eqref{rkdv}, thus, motivated by the equivalence between the two equations, we use the notion of criticality of \eqref{kdv} as a guide. In particular, we note that Theorem~\ref{thm:lwp_rkdv} establishes sub-critical well-posedness of the gauged equation, and goes beyond the $H^{-1}$-scale when $p$ is sufficiently large, namely $p>{5}$. See Figure~\ref{fig:rkdv} for a visual depiction of the range covered in the result, compared with the scaling indices of \eqref{kdv}. 
\andreia{Add comparison with Grunrocks regularity too and comment on how his strategy also applies here, in a convoluted way}
...
We do so systematically by reducing these to Fourier restricted estimates, as introduced by the second author in \cite{}. 
We also emphasize that the main technical difficulty in this work comes from \emph{deriving} the gauged equation in \eqref{rkdv}, not solving it. This process relies on an infinite normal form reduction, 
and a thorough exploitation of algebraic cancellations coming from the phase function as well as from the gauge transform $\Gc$, detailed in Section \ref{sec:multi}. 
...
We postpone further details on both the derivation of \eqref{rkdv} and the strategy of proof of Theorem~\ref{thm:lwp_rkdv} to Subsection~\ref{SUBSEC:method}.
	...
	\begin{figure}[h]
		...
		\caption{Well-posedness for \eqref{rkdv} in the $(s,\frac1p)$-plane. The dark-red region corresponds to the supercritical region. The black line represents the $H^{-1}$-scale, which is the threshold for the boundedness of $\Gc^{-1}$. The green region corresponds to the well-posedness of Theorem \ref{thm:lwp_rkdv}.}\label{fig:rkdv}
	\end{figure}
...
\andreia{Can we say anything for finite $p$? Should we comment that it may be possible to use a different strategy of proof that may allow well-posedness below, but given the complexity of the equation and the apparent lack of special structure such as integrability, we do not pursue this direction?}
...
\andreia{Justin also makes this comment in his thesis when referring to this not being a suitable space for well-posedness for this reason. Could consider citing}
\end{remark}
	...
	\begin{remark}
		...
		In particular, the choice of the gauge transform $\Gc$ defined through \eqref{gauge} is inferred by the dispersion and quadratic nonlinearity of \eqref{kdv} but unrelated to its integrability. Thus, it can be extended to small perturbations of KdV, with distinct nonlinearity or dispersions. For instance, the exact result in Theorem~\ref{thm:lwp_rkdv} holds for the following equation
		\begin{equation}
			\partial_t u + \partial_x^3 u = m(\partial_x)\partial_x(u^2),
		\end{equation}
		where $m$ is any pseudo-differential operator of order zero, such as the Hilbert transform $\mathcal{H}$, with no essential changes to the proof.
 One may also consider generalized dispersions, with the necessary adaptations on the regularity $s$ for Theorem~\ref{thm:lwp_rkdv}, which we detail in Subsection~\ref{sec:dgbo}.
	\end{remark}
...
\noi{\rm(i)} If $2 \le p<\infty$, given $R>0$, there exists $T=T(R)>0$ and a map
\begin{align}
...
{\noi{\rm(iii)} The data-to-solution map $\wt{\Theta}$ is analytic.}
	\end{theorem}

\andreia{HERE}

\begin{remark}
 In the case $p=\infty$, we cannot show that $\wt{\Theta}$ is the \emph{unique} extension of the flow defined for smooth data, simply because smooth decaying functions are not dense in $\FL^{s,p}(\R)$.
\end{remark}
