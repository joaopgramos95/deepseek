...
While KdV and the gauged equation are equivalent for smooth solutions, the latter is better behaved at low regularity in Fourier-Lebesgue spaces. In particular, the admissible regularities go beyond the $H^{-1}$-scale, which is a well-known threshold for KdV. 
% 
...
The \eqref{kdv} equation is a canonical dispersive equation, with a rich structure, a variety of special solutions (such as solitons) and it is completely integrable \cite{ZakharovFadeev}. As such, it admits infinitely many conservation laws, one at each positive integer level of $H^s$ regularity. Some of the most relevant conserved quantities include
\begin{align*}
...
Our focus in this work is the low-regularity local well-posedness problem for \eqref{kdv}, which requires a brief overview of the known literature {\footnote{Which is by no means exaustive, as it spans more than half a century.}}. In the 1970s, local well-posedness for \eqref{kdv} in $H^s(\R)$, $s>\frac32$ was shown by viewing \eqref{kdv} as a quasilinear hyperbolic equation \cite{BonaSmith, Kato_kdv, SautTemam, TsutsumiMukasa}. These works did not rely on the dispersive character of \eqref{kdv}. The first {author} to incorporate dispersion was Kato \cite{Kato_kdv_disp}, which then led to the works of Kenig, Ponce and Vega \cite{KPV91, KPV_gkdv}, where {the threshold $s>\frac34$ was reached}. Their technique relied on a suitable combination of maximal functions, Kato smoothing, and Strichartz estimates. 

\medskip


Soon after, Bourgain \cite{bourg2} reached $s=0$ by introducing Fourier restriction spaces (see also  \cite{KlainermanMachedon} in the context of wave equations), which are tailored to the dispersion of the equation. This seminal paper was later improved in \cite{kpv_bilin} to the threshold $s>-\frac34$. The endpoint case $s=-\frac34$ was first attained by Christ, Colliander and Tao \cite{CCT_ill} and later improved independently by Guo \cite{Guo_gwp} and Kishimoto \cite{Kishimoto_lwp_crit}. { It is also important to mention that the} aforementioned local well-posedness results can then lead to global results \cite{bourg2, CKSTT_sharp_kdv, Guo_gwp} by means of (modified) conservation laws associated with \eqref{kdv}. For $s<-\frac34$, the initial-value problem is \emph{ill-posed} in the sense of the failure of uniform continuity of the data-to-solution map \cite{KPV_ill}. As such, $s=-\frac34$ is a threshold for contraction arguments, which is in contrast with the scaling-critical regularity $s=-\frac32$. 
% {\jp This is at stark contrast with the properties of the equation itself: recalling that \eqref{kdv} is invariant by the scaling
% \begin{equation}\label{scaling}
% u_\lambda(t,x)=\lambda^{2}u(\lambda^3 t, \lambda x),
% \end{equation}
...
% \text{Energy:}& \int_\R \frac{(\dx u)^2}{2} + \frac{u^3}{3}  dx.\\
% \end{align*}
% In fact, the \eqref{kdv} equation can be shown to be ``completely integrable'' \cite{ZakharovFadeev}. As a consequence, \eqref{kdv} possesses an infinite number of conservation laws, one at each integer level of regularity $s$. 
\medskip

In order to surpass the $s=-\frac34$ threshold, one strategy is to exploit more deeply the structure of \eqref{kdv}, namely its complete integrability. This has permitted remarkable improvements to the local well-posedness theory. Kappeler \cite{Kappeler_rough} and Tsutsumi \cite{Tsutsumi_kdv_measures} were able to construct solutions with Radon measures as initial data. In \cite{BuckmasterKoch}, Buckmaster and Koch proved the existence of $H^{-1}(\R)$ weak solutions. Later, Killip and Visan \cite{KillipVisan_kdv} showed the (global) well-posedness of \eqref{kdv} in $H^{-1}(\R)$, where the solution is understood as the unique limit of smooth solutions. This result is \emph{optimal}, as it is not possible to define a continuous flow in $H^s(\R)$, for $s<-1$ \cite{Molinet_ill_line}. In the periodic case, the complete integrability structure has lead to even more substantial improvements to the  local result of Kenig, Ponce and Vega \cite{kpv_bilin}, {as the reader may find in }\cite{KMMT16, KappelerMolnar18, KT06, KillipVisan_kdv}.

	
	\medskip

    
	As such, complete integrability structures have emerged as one of the most powerful tools in the field of nonlinear dispersive equations. However, their rigidity implies that even very small perturbations, either in the dispersion or in the nonlinearity, break down the sharper methods available in the literature and force one to go back to contraction arguments. 
    For this reason, {as a primary postulate of the present work,} we avoid at all costs the complete integrability of \eqref{kdv}. Our goal is {\jp rather to improve the contraction methods which allow us to encompass perturbations of \eqref{kdv}, offering new insights into its rich dynamics.}

\medskip

One approach to improve on the regularity threshold is to work with the Fourier-Lebesgue spaces $\mathcal{F}L^{s,p}(\R)$, which are defined through the norm\footnote{
% Throughout this text, $\hat{\cdot}$ denotes the Fourier transform in the spatial variables.\\
...
For space-time functions $f: \R^2 \to \R$, we may write $\Ft_t (f)$ and $\Ft_x(f)$ to denote the Fourier transform with respect to $t$ and $x$, respectively. When clear from context, we also use $\ft{f}$ to denote either the spatial Fourier transform or the space-time Fourier transform. }
	...
	\end{equation} 
These spaces reduce to the classical $H^s(\R)$ spaces when $p=2$ and have been considered in \cite{adams, Chapouto_FL_mkdv, CC24, Grunrock_ldv_hier, HerrGrunrock_FL,  GrunrockVega}, where larger values of $p$ tend to close the gap between the critical and the local well-posedness regularities. 
...
% N_2[z] + N_3[z] + N_4[z]+ \dots
    ...
    We note crucially that \eqref{kdv} and \eqref{rkdv} are \textit{equivalent} for small\footnote{The smallness assumption is harmless in the context of the \eqref{kdv} since we are working in the scaling-subcritical regime.} smooth solutions, due to the mapping properties of the gauge transform $\Gc$ in \eqref{gauge} (see Proposition \ref{prop:equivalencia}):
    $$
...
To {\jp the best of} our knowledge, the gauge transform $\Gc$ is a new transform for the \eqref{kdv} on the real line, with the advantage of being independent of \emph{any} complete integrability structure.
...
there exist $b>1/p'$ and  a unique
	$$z\in X^{s,b}_{p, p}(0,T)\hookrightarrow C([0,T]; \mathcal{F}L^{s,p}(\R))$$ satisfying \eqref{rkdv} on $[0,T]$, where we refer to \eqref{Xsb} for the definition of $X^{s,b}_{p,q}(0,T)$;
...
\noi For a more precise statement of Theorem \ref{thm:lwp_rkdv}, we refer to Proposition \ref{thm:lwp_lp}.

	\begin{figure}[h]
		\centering
		\includegraphics[height=4.4cm]{plot_rkdv.pdf}
		\caption{Well-posedness for \eqref{rkdv} in the $(s,\frac1p)$-plane. The dark-red region corresponds to the supercritical region. The black line represents the $H^{-1}$-scale, which is the threshold for the boundedness of $\Gc$. The green region corresponds to the well-posedness of Theorem \ref{thm:lwp_rkdv}.}\label{fig:rkdv}
	\end{figure}
...
The proof of Theorem \ref{thm:lwp_rkdv} follows from a standard fixed-point argument in  Fourier restriction spaces,  once we derive the necessary multilinear estimates for each of the nonlinear terms in \eqref{rkdv} (with some nontrivial adaptations in the case $p=\infty$, see Section \ref{sec:fourier_restr}). This requires a thorough exploitation of algebraic cancellations present in \eqref{rkdv} and is the focus of Section \ref{sec:multi}. 
	...
	\begin{remark}
		...
		\end{equation}
		where $m$ is any pseudo-differential operator of order zero. One may also consider more general dispersions, with the necessary adaptations on the regularity $s$, see Subsection \ref{sec:dgbo}.
	\end{remark}
...
{\noi{\rm(iii)} The data-to-solution map $\tilde{\Theta}$ is analytic.}
	\end{theorem}

\begin{remark}
 In the case $p=\infty$, we cannot show that $\tilde{\Theta}$ is the \emph{unique} extension of the flow defined for smooth data, simply because smooth decaying functions are not dense in $\FL^{s,p}(\R)$.
\end{remark}
...
\simao{Strees the differences between the lwp theories for both equations and link with figure.}


\begin{figure}[h]
		\centering
		\includegraphics[height=4.4cm]{plot_kdv.pdf}
		\caption{Well-posedness for \eqref{kdv} in the $(s,\frac1p)$-plane. The red and yellow regions corresponds to ill-posedness (Proposition \ref{prop:ill_intro}). The dark-green region is the state-of-the-art result by Grünrock \cite{Grunrock_ldv_hier}. The light-green region corresponds to the well-posedness of Theorem \ref{thm:wp_kdv_intro}. In the yellow region, \eqref{kdv} is ill-posed while \eqref{rkdv} is well-posed.}\label{fig:kdv}
	\end{figure}
...
\subsection{Ill-posedness} We now move to the proofs of Propositions \ref{prop_C3_ill} and \ref{prop:ill_intro}. The first follows the (standard) arguments of \cite{taobejenaru, bou_ill_kdv, MolTzeSaut_ill, Tzvetkov_ill}.
...
\noi
    and let $z^\delta$ be the solution to \eqref{z1} with initial data $\delta \varphi_N $ constructed in Proposition~\ref{thm:lwp_rkdv}. From the analiticity of the solution map, we can write $z^\delta$ as follows
    $$
...
Finally, we prove Proposition \ref{prop:ill_intro}, which follows from an adaptation of the ill-posedness result in $H^s(\R)$, $s<-1$, shown by Molinet in \cite{Molinet_ill_line}. The argument is based on the fact that the Miura transform $\MMb[u]$ in \eqref{eq:defiMiura} 
maps solutions to \eqref{mkdv} to solutions of \ref{kdv},
...
In this section, we prove the well-posedness of \eqref{kdv}
 in $\FL^{s, \infty}(\R)$ for $s>-\frac14$, complementing the results of \cite{Grunrock_ldv_hier}, without the gauge transform. 
