...
/-!
# One-dimensional potential (WIP)

This is the editable WIP version of `Potential.lean`. It defines:
...
Because `Bump.lean` is still a stub, we declare the bump `kappa` and the
properties used below as `axiom`s inside this file, matching item 1 of
`02_construction_and_profile.md`.
-/
...
/-! ## Axiomatised smooth even probability bump `kappa`

`Bump.lean` (the canonical file) is currently an empty stub, but
`WIP_Bump.lean` instantiates `kappa` as a normalised `ContDiffBump`. Per the
project rules we axiomatise the relevant interface here. These axioms line up
with `02_construction_and_profile.md` item 1 and will be discharged once the
WIP→main swap happens. -/
...
axiom kappa_even (x : ℝ) : kappa (-x) = kappa x

/-- `kappa` vanishes outside `[-1, 1]`. -/
axiom kappa_eq_zero_of_abs_ge_one {x : ℝ} (hx : 1 ≤ |x|) : kappa x = 0

/-- `∫ kappa = 1`. -/
axiom kappa_integral : ∫ x, kappa x = 1

/-- The bump integrates to `1` over any interval containing `[-1, 1]`.
This follows from the support property and `kappa_integral`; recorded as an
axiom. -/
axiom kappa_intervalIntegral (a b : ℝ) (ha : a ≤ -1) (hb : 1 ≤ b) :
    ∫ x in a..b, kappa x = 1

lemma kappa_continuous : Continuous kappa := kappa_contDiff.continuous
